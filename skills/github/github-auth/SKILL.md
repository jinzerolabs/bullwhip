...
- Give it a name like "bullwhip-agent"
- Select scopes:
...
- Give it a title like "bullwhip-agent-<machine-name>"
...
elif [ -f ~/.bullwhip/.env ] && grep -q "^GITHUB_TOKEN=" ~/.bullwhip/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.bullwhip/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
