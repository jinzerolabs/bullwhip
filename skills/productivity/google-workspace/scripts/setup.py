#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Bull Whip Agent.
...
try:
    from bullwhip_constants import display_bullwhip_home, get_bullwhip_home
except ModuleNotFoundError:
    BULLWHIP_AGENT_ROOT = Path(__file__).resolve().parents[4]
    if BULLWHIP_AGENT_ROOT.exists():
        sys.path.insert(0, str(BULLWHIP_AGENT_ROOT))
    from bullwhip_constants import display_bullwhip_home, get_bullwhip_home

BULLWHIP_HOME = get_bullwhip_home()
TOKEN_PATH = BULLWHIP_HOME / "google_token.json"
CLIENT_SECRET_PATH = BULLWHIP_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = BULLWHIP_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same BullWhip profile to refresh consent."
    )
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Bull Whip home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_bullwhip_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Bull Whip")
