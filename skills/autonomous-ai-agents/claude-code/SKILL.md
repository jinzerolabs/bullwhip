...
metadata:
  bullwhip:
    ...
    related_skills: [codex, bullwhip-agent, opencode]
---

# Claude Code — Bull Whip Orchestration Guide

Delegate coding tasks to [Claude Code](https://code.claude.com/docs/en/cli-reference) (Anthropic's autonomous coding agent CLI) via the Bull Whip terminal. Claude Code v2.x can read files, write code, run shell commands, spawn subagents, and manage git workflows autonomously.
...
Bull Whip interacts with Claude Code in two fundamentally different ways. Choose based on the task.
...
1. **Interactive mode REQUIRES tmux** — Claude Code is a full TUI app. Using `pty=true` alone in Bull Whip terminal works but tmux gives you `capture-pane` for monitoring and `send-keys` for input, which is essential for orchestration.
