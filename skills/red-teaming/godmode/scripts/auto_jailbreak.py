...
Usage in execute_code:
    exec(open(os.path.expanduser(
        "~/.bullwhip/skills/red-teaming/godmode/scripts/auto_jailbreak.py"
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("BULLWHIP_HOME", Path.home() / ".bullwhip")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# Bull Whip config paths
# ═══════════════════════════════════════════════════════════════════

BULLWHIP_HOME = Path(os.getenv("BULLWHIP_HOME", Path.home() / ".bullwhip"))
CONFIG_PATH = BULLWHIP_HOME / "config.yaml"
PREFILL_PATH = BULLWHIP_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/BullWhip models — already uncensored, just needs clean prompt
    "bullwhip": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "bullwhip" in model_lower or "nous" in model_lower:
        return "bullwhip"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Bull Whip config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.bullwhip/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart BullWhip for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart BullWhip for changes to take effect.")
