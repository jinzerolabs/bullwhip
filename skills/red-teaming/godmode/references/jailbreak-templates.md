...
## 5. GODMODE FAST — Bull Whip 4 405B (Zero Refusal)

**Model:** `zerolabskorea/bullwhip-4-405b`
**Strategy:** Bull Whip models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Bull Whip Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.bullwhip/config.yaml`:
