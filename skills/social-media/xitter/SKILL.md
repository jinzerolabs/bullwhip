...
version: 1.0.0
author: Siddharth Balyan + Bull Whip Agent
license: MIT
...
This skill intentionally does not vendor a separate CLI implementation into Bull Whip. Install and use upstream `x-cli` instead.
...
If you already keep your X credentials in `~/.bullwhip/.env`, the cleanest setup is:

```bash
mkdir -p ~/.config/x-cli
ln -sf ~/.bullwhip/.env ~/.config/x-cli/.env
```
...
x-cli tweet search "from:ZeroLabsKorea" --max 3
...
- **Credential drift**: if you rotate tokens in `~/.bullwhip/.env`, make sure `~/.config/x-cli/.env` still points at the current file.
