"""
Gateway subcommand for bullwhip CLI.

Handles: bullwhip gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_bullwhip_home is imported lazily at call sites to avoid ImportError
# when bullwhip_constants is cached from a pre-update version during `bullwhip update`.
from bullwhip_cli.setup import (
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "bullwhip-gateway*",
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``bullwhip update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            BullWhip profile are returned.
    """
    _exclude = exclude_pids or set()
    pids = [pid for pid in _get_service_pids() if pid not in _exclude]
    patterns = [
        "bullwhip_cli.main gateway",
        "bullwhip_cli.main --profile",
        "bullwhip_cli.main -p",
        "bullwhip_cli/main.py gateway",
        "bullwhip_cli/main.py --profile",
        "bullwhip_cli/main.py -p",
        "bullwhip gateway",
        "gateway/run.py",
    ]
    current_home = str(get_bullwhip_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        if current_profile_name:
            return (
                ...
                or f"BULLWHIP_HOME={current_home}" in command
            )

        if "--profile " in command or " -p " in command:
            return False
        if "BULLWHIP_HOME=" in command and f"BULLWHIP_HOME={current_home}" not in command:
            return False
        return True
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (BULLWHIP_HOME-scoped).
...
from bullwhip_constants import is_container, is_termux, is_wsl
...
_SERVICE_BASE = "bullwhip-gateway"
SERVICE_DESCRIPTION = "Bull Whip Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current BULLWHIP_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (BULLWHIP_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from bullwhip_constants import get_defaultbullwhip_root
    home = get_bullwhip_home().resolve()
    default = get_defaultbullwhip_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary BULLWHIP_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(bullwhip_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when BULLWHIP_HOME is a named profile.

    For ``~/.bullwhip/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        bullwhip_home: Optional explicit BULLWHIP_HOME path. Defaults to the current
            ``get_bullwhip_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
    """
    import re
    from bullwhip_constants import get_defaultbullwhip_root
    home = Path(bullwhip_home or str(get_bullwhip_home())).resolve()
    default = get_defaultbullwhip_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this BULLWHIP_HOME.

    Default ``~/.bullwhip`` returns ``bullwhip-gateway`` (backward compatible).
    Profile ``~/.bullwhip/profiles/coder`` returns ``bullwhip-gateway-coder``.
    Any other BULLWHIP_HOME appends a short hash for uniqueness.
    """
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    bullwhip gateway uninstall")
    print_info("    sudo bullwhip gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            print_warning("  System service install requires sudo, so BullWhip can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo bullwhip gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo bullwhip gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo bullwhip gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode BullWhip often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.bullwhip`` → ``ai.bullwhip.gateway.plist`` (backward compatible).
    Profile ``~/.bullwhip/profiles/coder`` → ``ai.bullwhip.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.bullwhip.gateway-{suffix}" if suffix else "ai.bullwhip.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.bullwhip/bullwhip-agent  -> /home/alice/.bullwhip/bullwhip-agent
      /opt/bullwhip                 -> /opt/bullwhip  (kept as-is)
...
def _bullwhip_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current BULLWHIP_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_bullwhip_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.bullwhip                    → /home/alice/.bullwhip
      /root/.bullwhip/profiles/coder     → /home/alice/.bullwhip/profiles/coder
      /opt/custom-bullwhip               → /opt/custom-bullwhip  (kept as-is)
    """
    current_bullwhip = get_bullwhip_home().resolve()
    current_default = (Path.home() / ".bullwhip").resolve()
    target_default = Path(target_home_dir) / ".bullwhip"

    # Default ~/.bullwhip → remap to target user's default
    if current_hermes == current_default:
        return str(target_default)

    # Profile or subdir of ~/.bullwhip → preserve the relative structure
    try:
        relative = current_bullwhip.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.bullwhip) — keep as-is
        return str(current_bullwhip)
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        bullwhip_home = _bullwhip_home_for_target_user(home_dir)
        profile_arg = _profile_arg(bullwhip_home)
...
User={username}
Group={group_name}
ExecStart={python_path} -m bullwhip_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
Environment="BULLWHIP_HOME={bullwhip_home}"
Restart=on-failure
RestartSec=30
...
"""
    ...
    bullwhip_home = str(get_bullwhip_home().resolve())
    profile_arg = _profile_arg(bullwhip_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current BullWhip install")
    return True
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}bullwhip gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}bullwhip gateway status{scope_flag}             # Check status")
...
def systemd_restart(system: bool = False):
    ...
    if pid is not None and _request_gateway_self_restart(pid):
        ...
        print(
            ...
            f"  Check status: {'sudo ' if system else ''}bullwhip gateway status\n"
...
def systemd_status(deep: bool = False, system: bool = False):
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}bullwhip gateway install{scope_flag}")
        return
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}bullwhip gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}bullwhip gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.bullwhip.gateway-{suffix}" if suffix else "ai.bullwhip.gateway"
...
def generate_launchd_plist() -> str:
    ...
    working_dir = str(PROJECT_ROOT)
    bullwhip_home = str(get_bullwhip_home().resolve())
    log_dir = get_bullwhip_home() / "logs"
    ...
    prog_args = [
        ...
        "<string>-m</string>",
        "<string>bullwhip_cli.main</string>",
    ]
    if profile_arg:
...
<dict>
    ...
    <dict>
        ...
        <key>BULLWHIP_HOME</key>
        <string>{bullwhip_home}</string>
    </dict>
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current BullWhip install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  bullwhip gateway status             # Check status")
...
def launchd_stop():
    ...
    # `bullwhip gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current BullWhip install")
    else:
        print("⚠ Service definition is stale relative to the current BullWhip install")
        print("  Run: bullwhip gateway start")
    ...
    if deep:
        log_file = get_bullwhip_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           🐂 Bull Whip Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and BullWhip will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @bullwhip:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. bullwhip) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where BullWhip delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Bull Whip agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address BullWhip will use (e.g., bullwhip@gmail.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. BullWhip connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: bullwhip pairing generate bluebubbles",
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_bullwhip_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'bullwhip pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: bullwhip pairing approve <platform> <code>")
                else:
                    print_info("  Skipped — configure later with 'bullwhip gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from bullwhip_cli.main import cmd_whatsapp
    import argparse
...
def _setup_weixin():
    ...
    print()
    print_info("  1. BullWhip will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info("  3. BullWhip will store the returned account_id/token in ~/.bullwhip/.env.")
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `bullwhip gateway setup`.")
        return
    ...
    import asyncio
    try:
        credentials = asyncio.run(qr_login(str(get_bullwhip_home())))
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown DM users can request access and you approve them with `bullwhip pairing approve`.")
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info("  Unknown users can request access; approve with `bullwhip pairing approve`.")
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"BullWhipAgent\"")
        ...
        if service_running:
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: bullwhip gateway")
                except subprocess.CalledProcessError as e:
            ...
            if supports_systemd_services() or is_macos():
                ...
                if prompt_yes_no(f"  Install the gateway as a {platform_name} service?{wsl_note} (runs in background, starts on boot)", True):
                    ...
                    print_info("  Or run in foreground:  bullwhip gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: bullwhip gateway run")
                print_info("  For persistence:   tmux new -s bullwhip 'bullwhip gateway run'")
                ...
                if is_termux():
                    ...
                    print_info("  Run in foreground: bullwhip gateway run")
                    print_info(f"  Or start it manually in the background (best effort): nohup bullwhip gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
                else:
                    print_info("  Service install not supported on this platform.")
                    print_info("  Run in foreground: bullwhip gateway run")
    else:
        print()
        print_info("No platforms configured. Run 'bullwhip gateway setup' when ready.")
...
def gateway_command(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: bullwhip gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning("WSL detected — systemd services may not survive WSL restarts.")
                print_info("  Consider running in foreground instead: bullwhip gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s bullwhip 'bullwhip gateway run'")
                print()
            ...
            print()
            print("  bullwhip gateway run                              # direct foreground")
            print("  tmux new -s bullwhip 'bullwhip gateway run'         # persistent via tmux")
            print("  nohup bullwhip gateway run > ~/.bullwhip/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
            ...
            print()
            print("To run the gateway: bullwhip gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: bullwhip gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: bullwhip gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: bullwhip gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if supports_systemd_services():
                ...
                if linger_ok is not True:
                    ...
                    print()
                    print("  Then restart the gateway:")
                    print("    bullwhip gateway restart")
                    return
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: bullwhip gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  bullwhip gateway install")
                    print("  sudo bullwhip gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  bullwhip gateway run      # Run in foreground")
                if is_termux():
                    print("  nohup bullwhip gateway run > ~/.bullwhip/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s bullwhip 'bullwhip gateway run'         # persistent via tmux")
                    print("  nohup bullwhip gateway run > ~/.bullwhip/logs/gateway.log 2>&1 &  # background")
                else:
                    print("  bullwhip gateway install  # Install as user service")
                    print("  sudo bullwhip gateway install --system  # Install as boot-time system service")
