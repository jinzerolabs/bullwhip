"""
Dump command for bullwhip CLI.

Outputs a compact, plain-text summary of the user's BullWhip setup
...
from bullwhip_cli.config import get_bullwhip_home, get_env_path, get_project_root, load_config
...
def _count_skills(bullwhip_home: Path) -> int:
...
def _cron_summary(bullwhip_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = bullwhip_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"bullwhip_home:      {display_bullwhip_home()}")
    ...
    toolsets = config.get("toolsets", ["bullwhip-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(bullwhip_home)}")
    lines.append(f"  skills:             {_count_skills(bullwhip_home)}")
