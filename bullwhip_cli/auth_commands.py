...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_bullwhip_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:bullwhip_pkce",
    ...
    raise SystemExit(f"`bullwhip auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    if removed.source.startswith("env:"):
        ...
        if env_var:
            from bullwhip_cli.config import remove_env_value
            cleared = remove_env_value(env_var)
            if cleared:
                print(f"Cleared {env_var} from .env")

    # If this was a singleton-seeded credential (OAuth device_code, bullwhip_pkce),
    ...
    # re-seeded on the next load_pool() call.
    ...
    elif removed.source == "bullwhip_pkce" and provider == "anthropic":
        from bullwhip_constants import get_bullwhip_home
        oauth_file = get_bullwhip_home() / ".anthropic_oauth.json"
        if oauth_file.exists():
            oauth_file.unlink()
            print("Cleared BullWhip Anthropic OAuth credentials")

    elif removed.source == "claude_code" and provider == "anthropic":
        from bullwhip_cli.auth import suppress_credential_source
        ...
        print("Run `bullwhip auth add anthropic` to re-enable if needed.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `bullwhip auth` is called bare."""
