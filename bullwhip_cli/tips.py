...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or bullwhip -c.",
    ...
    "Ctrl+Z suspends BullWhip to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "bullwhip -c resumes your most recent CLI session. bullwhip -c \"project name\" resumes by title.",
    "bullwhip -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "bullwhip -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "bullwhip chat -t web,terminal enables only specific toolsets for a focused session.",
    "bullwhip chat -s github-pr-workflow preloads a skill at launch.",
    "bullwhip chat -q \"query\" runs a single non-interactive query and exits.",
    "bullwhip chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "bullwhip chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "bullwhip --yolo bypasses all dangerous command approval prompts for the entire session.",
    "bullwhip chat --source telegram tags the session for filtering in bullwhip sessions list.",
    "bullwhip -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "bullwhip doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "bullwhip dump outputs a compact setup summary — great for bug reports.",
    "bullwhip config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "bullwhip config edit opens config.yaml in your default editor.",
    "bullwhip config check scans for missing or stale configuration options.",
    "bullwhip sessions browse opens an interactive session picker with search.",
    "bullwhip sessions stats shows session counts by platform and database size.",
    "bullwhip sessions prune --older-than 30 cleans up old sessions.",
    "bullwhip skills search react --source skills-sh searches the skills.sh public directory.",
    "bullwhip skills check scans installed hub skills for upstream updates.",
    "bullwhip skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "bullwhip skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "bullwhip mcp add github --command npx adds MCP servers from the command line.",
    "bullwhip mcp serve runs BullWhip itself as an MCP server for other agents.",
    "bullwhip auth add lets you add multiple API keys for credential pool rotation.",
    "bullwhip completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "bullwhip logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "bullwhip backup creates a zip backup of your entire BullWhip home directory.",
    "bullwhip profile create coder creates an isolated profile that becomes its own command.",
    "bullwhip profile create work --clone copies your current config and keys to a new profile.",
    "bullwhip update syncs new bundled skills to ALL profiles automatically.",
    "bullwhip gateway install sets up BullWhip as a system service (systemd/launchd).",
    "bullwhip memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "bullwhip webhook subscribe creates event-driven webhook routes with HMAC validation.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as bullwhip -w).",
    ...
    "BULLWHIP_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call BullWhip tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'bullwhip profile create coder' creates the 'coder' command.",
    "bullwhip profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, BullWhip prints a resume command with session ID and stats.",
    "bullwhip sessions export backup.jsonl exports all sessions for backup or analysis.",
    "bullwhip -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "bullwhip skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .bullwhip/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: bullwhip cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "BullWhip runs on 18 platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, email, and more.",
    "bullwhip gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Bull Whip via a local macOS server.",
    ...
    "Context files (.bullwhip.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.bullwhip/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "BullWhip loads project context from .bullwhip.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — bullwhip tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.bullwhip/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "bullwhip chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "bullwhip plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "bullwhip acp runs BullWhip as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "BULLWHIP_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "bullwhip login supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "bullwhip status --deep runs deeper diagnostic checks across all components.",

    # --- Hidden Gems & Power-User Tricks ---
    "BOOT.md at ~/.bullwhip/BOOT.md runs automatically on every gateway start — use it for startup checks.",
    "Cron jobs can attach a Python script (--script) whose stdout is injected into the prompt as context.",
    "Cron scripts live in ~/.bullwhip/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make BullWhip your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.bullwhip/SOUL.md to customize.",
    ...
    "Before a gateway session reset, BullWhip auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.bullwhip/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at BULLWHIP_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "BULLWHIP_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in BULLWHIP_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.bullwhip/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'bullwhip profile create ls' would be rejected.",
    "bullwhip profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "bullwhip claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "BULLWHIP_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via bullwhip tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.bullwhip/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.bullwhip/skills/.hub/quarantine/ holds skills pending security review.",
]
