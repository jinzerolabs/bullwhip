"""
Interactive setup wizard for Bull Whip Agent.
...
Config files are stored in ~/.bullwhip/ for easy access.
"""
...
_DOCS_BASE = "https://bullwhip-agent.nousresearch.com/docs"
...
from bullwhip_cli.config import (
    ...
    save_env_value,
    get_env_value,
    ensure_bullwhip_home,
)
# display_bullwhip_home imported lazily at call sites (stale-module safety during bullwhip update)

from bullwhip_cli.colors import Colors, color
...
from bullwhip_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("🐂 Bull Whip Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure BullWhip using environment variables or config commands:")
    print_info("  bullwhip config set model.provider custom")
    print_info("  bullwhip config set model.base_url http://localhost:8080/v1")
    print_info("  bullwhip config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'bullwhip setup' in an interactive terminal to use the full wizard.")
    print()
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0) -> int:
    ...
    from bullwhip_cli.curses_ui import curses_radiolist
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from bullwhip_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'bullwhip setup')")


def _print_setup_summary(config: dict, bullwhip_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'bullwhip setup' to configure"))
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'bullwhip setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'bullwhip setup terminal'"))
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'bullwhip setup tools' to configure them,"
        )
        from bullwhip_constants import display_bullwhip_home as _dhh
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {bullwhip_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('bullwhip setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('bullwhip setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('bullwhip setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('bullwhip setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('bullwhip setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('bullwhip config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('bullwhip config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('bullwhip config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('bullwhip', Colors.GREEN)}              Start chatting")
    print(f"   {color('bullwhip gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('bullwhip doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `bullwhip tools` and `bullwhip setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``bullwhip model``)
    ...
    provider added to ``bullwhip model`` is automatically available here.
    ...
    # Delegate to the shared bullwhip model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from bullwhip_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: bullwhip model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from bullwhip_cli.auth_commands import auth_add_command
            ...
            print_info(
                "BullWhip can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one BullWhip can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'bullwhip setup' or configure AUXILIARY_VISION_* settings")
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.bullwhip/.env."
            )
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'bullwhip setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where BullWhip runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("  When using BullWhip via Telegram/Discord, this is where")
        print_info(
...
def _apply_default_agent_settings(config: dict):
    ...
    save_env_value("BULLWHIP_MAX_ITERATIONS", "90")
    ...
    print_info("  Run `bullwhip setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    # ── Max Iterations ──
    current_max = get_env_value("BULLWHIP_MAX_ITERATIONS") or str(
    ...
    try:
        ...
        if max_iter > 0:
            save_env_value("BULLWHIP_MAX_ITERATIONS", str(max_iter))
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where BullWhip delivers cron job results,")
...
def _setup_slack():
    ...
    print()
    print_info("   Full guide: https://bullwhip-agent.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where BullWhip delivers cron job results and notifications.")
...
def _setup_mattermost():
    ...
    print()
    print_info("📬 Home Channel: where BullWhip delivers cron job results and notifications.")
...
def _setup_whatsapp():
    ...
    print_info("Requires Node.js. Run 'bullwhip whatsapp' for guided setup.")
    print()
    if prompt_yes_no("Enable WhatsApp now?", True):
        save_env_value("WHATSAPP_ENABLED", "true")
        print_success("WhatsApp enabled")
        print_info("Run 'bullwhip whatsapp' to choose your mode (separate bot number")
        print_info("or personal self-chat) and pair via QR code.")


def _setup_weixin():
    """Configure Weixin (personal WeChat) via iLink Bot API QR login."""
    from bullwhip_cli.gateway import _setup_weixin as _gateway_setup_weixin
    _gateway_setup_weixin()


def _setup_signal():
    """Configure Signal via gateway setup."""
    from bullwhip_cli.gateway import _setup_signal as _gateway_setup_signal
    _gateway_setup_signal()


def _setup_email():
    """Configure Email via gateway setup."""
    from bullwhip_cli.gateway import _setup_email as _gateway_setup_email
    _gateway_setup_email()


def _setup_sms():
    """Configure SMS (Twilio) via gateway setup."""
    from bullwhip_cli.gateway import _setup_sms as _gateway_setup_sms
    _gateway_setup_sms()


def _setup_dingtalk():
    """Configure DingTalk via gateway setup."""
    from bullwhip_cli.gateway import _setup_dingtalk as _gateway_setup_dingtalk
    _gateway_setup_dingtalk()


def _setup_feishu():
    """Configure Feishu / Lark via gateway setup."""
    from bullwhip_cli.gateway import _setup_feishu as _gateway_setup_feishu
    _gateway_setup_feishu()


def _setup_wecom():
    """Configure WeCom (Enterprise WeChat) via gateway setup."""
    from bullwhip_cli.gateway import _setup_wecom as _gateway_setup_wecom
    _gateway_setup_wecom()


def _setup_wecom_callback():
    """Configure WeCom Callback (self-built app) via gateway setup."""
    from bullwhip_cli.gateway import _setup_wecom_callback as _gw_setup
    _gw_setup()
...
def _setup_qqbot():
    ...
    print_info("Connects BullWhip to QQ via the Official QQ Bot API (v2).")
...
def _setup_bluebubbles():
    ...
    print_info("Connects BullWhip to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from bullwhip_cli.gateway import _PLATFORMS
    qq_platform = next((p for p in _PLATFORMS if p["key"] == "qqbot"), None)
    if qq_platform:
        from bullwhip_cli.gateway import _setup_standard_platform
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://bullwhip-agent.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://bullwhip-agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  bullwhip config edit")
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with Bull Whip from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'bullwhip setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     bullwhip config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                try:
                    ...
                    print_info("  You can try manually: bullwhip gateway install")
            else:
                print_info("  You can install later: bullwhip gateway install")
                if supports_systemd:
                    print_info("  Or as a boot-time service: sudo bullwhip gateway install --system")
                print_info("  Or run in foreground:  bullwhip gateway")
        else:
            from bullwhip_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   bullwhip gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   bullwhip gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `bullwhip setup tools` and `bullwhip tools` use the same flow:
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from bullwhip_cli.config
    ...
    if section_key == "model":
        ...
        if not has_key:
            ...
            try:
                from bullwhip_cli.auth import get_active_provider
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_bullwhip.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_bullwhip migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_bullwhip", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and BullWhip.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure BullWhip to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point BullWhip at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point BullWhip at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point BullWhip at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point BullWhip at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Bull Whip equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing BullWhip config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in BullWhip.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ BullWhip's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(bullwhip_home: Path) -> bool:
    ...
    print_info("BullWhip can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: bullwhip claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=bullwhip_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: bullwhip claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing BullWhip configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing BullWhip config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in BullWhip (use bullwhip claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      bullwhip setup           — full or quick (auto-detected)
      bullwhip setup model     — just model/provider
      bullwhip setup tts       — just text-to-speech
      bullwhip setup terminal  — just terminal backend
      bullwhip setup gateway   — just messaging platforms
      bullwhip setup tools     — just tool configuration
      bullwhip setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     🐂 Bull Whip Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             🐂 Bull Whip Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Bull Whip Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have BullWhip configured.")
        print()
        ...
        if choice == 0:
            # Quick setup
            _run_quick_setup(config, bullwhip_home)
            return
        elif choice == 1:
            # Full setup — fall through to run all sections
            pass
        elif choice == 7:
            print_info("Exiting. Run 'bullwhip setup' again when ready.")
            return
        elif 2 <= choice <= 6:
            ...
            if section:
                ...
                func(config)
                save_config(config)
                _print_setup_summary(config, bullwhip_home)
            return
    else:
        # ── First-Time Setup ──
        print()
        ...
        migration_ran = _offer_openclaw_migration(bullwhip_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice("How would you like to set up BullWhip?", [
            "Quick setup — provider, model & messaging (recommended)",
            "Full setup — configure everything",
        ], 0)

        if setup_mode == 0:
            _run_first_time_quick_setup(config, bullwhip_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {bullwhip_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'bullwhip config edit'")
...
def _resolvebullwhip_chat_argv() -> Optional[list[str]]:
    """Resolve argv for launching ``bullwhip chat`` in a fresh process."""
    bullwhip_bin = shutil.which("bullwhip")
    if bullwhip_bin:
        return [bullwhip_bin, "chat"]

    try:
        if importlib.util.find_spec("bullwhip_cli") is not None:
            return [sys.executable, "-m", "bullwhip_cli.main", "chat"]
    except Exception:
        pass
...
def _offer_launch_chat():
    ...
    print()
    if not prompt_yes_no("Launch bullwhip chat now?", True):
        return

    chat_argv = _resolvebullwhip_chat_argv()
    if not chat_argv:
        print_info("Could not relaunch BullWhip automatically. Run 'bullwhip chat' manually.")
        return

    os.execvp(chat_argv[0], chat_argv)


def _run_first_time_quick_setup(config: dict, bullwhip_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'bullwhip setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    bullwhip setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  bullwhip setup gateway")
    print()
...
def _run_quick_setup(config: dict, bullwhip_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'bullwhip setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect BullWhip to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'bullwhip setup gateway'.")
