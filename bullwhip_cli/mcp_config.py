"""
MCP Server Management CLI — ``bullwhip mcp`` subcommand.

Implements ``bullwhip mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.bullwhip/config.yaml under the ``mcp_servers`` key.
"""
...
from bullwhip_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_bullwhip_home,  # noqa: F401 — used by test mocks
)
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from bullwhip_cli.cli_output import prompt as _shared_prompt
...
def cmd_mcp_add(args):
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  bullwhip mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  bullwhip mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  bullwhip mcp add myserver --preset mypreset')
        return
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_bullwhip_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info("Fix the issue, then: bullwhip mcp test " + name)
        return
    ...
    print()
    _success(f"Saved '{name}' to {display_bullwhip_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  bullwhip mcp add <name> --url <endpoint>')
        _info('  bullwhip mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
# ─── bullwhip mcp configure ────────────────────────────────────────────────────

def cmd_mcp_configure(args):
    """Reconfigure which tools are enabled for an existing MCP server."""
    import sys as _sys
    if not _sys.stdin.isatty():
        print("Error: 'bullwhip mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``bullwhip mcp`` subcommands."""
    ...
    if handler:
        ...
        _info("bullwhip mcp serve                              Run as MCP server")
        _info("bullwhip mcp add <name> --url <endpoint>        Add an MCP server")
        _info("bullwhip mcp add <name> --command <cmd>         Add a stdio server")
        _info("bullwhip mcp add <name> --preset <preset>       Add from a known preset")
        _info("bullwhip mcp remove <name>                      Remove a server")
        _info("bullwhip mcp list                               List servers")
        _info("bullwhip mcp test <name>                        Test connection")
        _info("bullwhip mcp configure <name>                   Toggle tools")
        print()
