...
from bullwhip_cli.config import get_env_value, load_config
...
def get_nous_subscription_explainer_lines() -> list[str]:
    ...
    return [
        ...
        "Change these later with: bullwhip setup tools, bullwhip setup terminal, or bullwhip status.",
    ]


def apply_nous_provider_defaults(config: Dict[str, object]) -> set[str]:
    """Apply provider-level Nous defaults shared by `bullwhip setup` and `bullwhip model`."""
    if not managed_nous_tools_enabled():
        return set()
