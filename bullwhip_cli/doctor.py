"""
Doctor command for bullwhip CLI.

Diagnoses issues with Bull Whip Agent setup.
"""
...
from bullwhip_cli.config import get_project_root, get_bullwhip_home, get_env_path
from bullwhip_constants import display_bullwhip_home

PROJECT_ROOT = get_project_root()
BULLWHIP_HOME = get_bullwhip_home()
_DHH = display_bullwhip_home()  # user-facing display path (e.g. ~/.bullwhip or ~/.bullwhip/profiles/coder)

# Load environment variables from ~/.bullwhip/.env so API key checks work
...
from bullwhip_constants import OPENROUTER_MODELS_URL
...
from bullwhip_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.bullwhip/.env contains provider auth/base URL settings."""
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `bullwhip`.
    os.environ.setdefault("BULLWHIP_INTERACTIVE", "1")
    ...
    print(color("│                 🩺 BullWhip Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Check ~/.bullwhip/.env (primary location for user config)
    env_path = BULLWHIP_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'bullwhip setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'bullwhip setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'bullwhip setup' to create one")
                issues.append("Run 'bullwhip setup' to create .env")
    
    # Check ~/.bullwhip/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = BULLWHIP_HOME / 'config.yaml'
    ...
    if config_path.exists():
        try:
            from bullwhip_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'bullwhip setup' to migrate config")
                else:
                    issues.append("Run 'bullwhip doctor --fix' or 'bullwhip setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'bullwhip doctor --fix'")
        except Exception:
            pass
    ...
    try:
        from bullwhip_cli.auth import get_nous_auth_status, get_codex_auth_status
    ...
    bullwhip_home = BULLWHIP_HOME
    if bullwhip_home.exists():
        check_ok(f"{_DHH} directory exists")
    else:
        if should_fix:
            bullwhip_home.mkdir(parents=True, exist_ok=True)
    ...
    for subdir_name in expected_subdirs:
        subdir_path = bullwhip_home / subdir_name
    ...
    soul_path = bullwhip_home / "SOUL.md"
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give BullWhip a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Bull Whip Agent Persona\n\n"
                "<!-- Edit this file to customize how BullWhip communicates. -->\n\n"
                "You are BullWhip, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = bullwhip_home / "memories"
    ...
    state_db_path = bullwhip_home / "state.db"
    ...
    wal_path = bullwhip_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'bullwhip doctor --fix' to checkpoint")
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'bullwhip setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    hub_dir = BULLWHIP_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: bullwhip skills list)")
    ...
    try:
        ...
        _mem_cfg_path = BULLWHIP_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                check_warn("Honcho config not found", "run: bullwhip memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                check_fail("Honcho API key or base URL not set", "run: bullwhip memory setup")
                issues.append("No Honcho API key — run 'bullwhip memory setup'")
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                check_fail("Mem0 API key not set", "(set MEM0_API_KEY in .env or run bullwhip memory setup)")
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: bullwhip memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: bullwhip memory setup")
        except Exception as _e:
    ...
    # =========================================================================
    try:
        from bullwhip_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
                ...
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "bullwhip -p" in content:
                            _m = _re.search(r"bullwhip -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'bullwhip doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
