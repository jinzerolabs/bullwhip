#!/usr/bin/env python3
"""
BullWhip CLI - Main entry point.

Usage:
    bullwhip                     # Interactive chat (default)
    bullwhip chat                # Interactive chat
    bullwhip gateway             # Run gateway in foreground
    bullwhip gateway start       # Start gateway as service
    bullwhip gateway stop        # Stop gateway service
    bullwhip gateway status      # Show gateway status
    bullwhip gateway install     # Install gateway service
    bullwhip gateway uninstall   # Uninstall gateway service
    bullwhip setup               # Interactive setup wizard
    bullwhip logout              # Clear stored authentication
    bullwhip status              # Show status of all components
    bullwhip cron                # Manage cron jobs
    bullwhip cron list           # List cron jobs
    bullwhip cron status         # Check if cron scheduler is running
    bullwhip doctor              # Check configuration and dependencies
    bullwhip honcho setup                    # Configure Honcho AI memory integration
    bullwhip honcho status                   # Show Honcho config and connection status
    bullwhip honcho sessions                 # List directory → session name mappings
    bullwhip honcho map <name>               # Map current directory to a session name
    bullwhip honcho peer                     # Show peer names and dialectic settings
    bullwhip honcho peer --user NAME         # Set user peer name
    bullwhip honcho peer --ai NAME           # Set AI peer name
    bullwhip honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    bullwhip honcho mode                     # Show current memory mode
    bullwhip honcho mode [hybrid|honcho|local]  # Set memory mode
    bullwhip honcho tokens                   # Show token budget settings
    bullwhip honcho tokens --context N       # Set session.context() token cap
    bullwhip honcho tokens --dialectic N     # Set dialectic result char cap
    bullwhip honcho identity                 # Show AI peer identity representation
    bullwhip honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    bullwhip honcho migrate                  # Step-by-step migration guide: OpenClaw native → BullWhip + Honcho
    bullwhip version             Show version
    bullwhip update              Update to latest version
    bullwhip uninstall           Uninstall Bull Whip Agent
    bullwhip acp                 Run as an ACP server for editor integration
    bullwhip sessions browse     Interactive session picker with search

    bullwhip claw migrate --dry-run  # Preview migration without changes
"""
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (bullwhip tools, bullwhip setup, bullwhip model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'bullwhip {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any bullwhip module import.
#
# Many modules cache BULLWHIP_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("BULLWHIP_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.bullwhip/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set BULLWHIP_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # 2. If no flag, check active_profile in the bullwhip root
    if profile_name is None:
        try:
            from bullwhip_constants import get_defaultbullwhip_root
            active_path = get_defaultbullwhip_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set BULLWHIP_HOME
    if profile_name is not None:
        try:
            from bullwhip_cli.profiles import resolve_profile_env
            bullwhip_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent bullwhip from starting
            print(f"Warning: profile override failed ({exc}), using default", file=sys.stderr)
            return
        os.environ["BULLWHIP_HOME"] = bullwhip_home
...
# Load .env from ~/.bullwhip/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from bullwhip_cli.config import get_bullwhip_home
from bullwhip_cli.env_loader import load_bullwhip_dotenv
load_bullwhip_dotenv(project_env=PROJECT_ROOT / '.env')

# Initialize centralized file logging early — all `bullwhip` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from bullwhip_logging import setup_logging as _setup_logging
...
try:
    from bullwhip_cli.config import load_config as _load_config_early
    from bullwhip_constants import apply_ipv4_preference as _apply_ipv4
...
from bullwhip_cli import __version__, __release_date__
from bullwhip_constants import OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from bullwhip_cli.config import get_env_path, get_bullwhip_home, load_config
    from bullwhip_cli.auth import get_auth_status

    # Determine whether BullWhip itself has been explicitly configured (model
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _hasbullwhip_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from bullwhip_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_bullwhip_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if BullWhip has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants BullWhip to use their tokens.
    if _hasbullwhip_config:
        try:
...
def _resolve_last_cli_session() -> Optional[str]:
    ...
    try:
        from bullwhip_state import SessionDB
        db = SessionDB()
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, bullwhip_bin
        cli_args: the original CLI arguments (everything after 'bullwhip')
    """
    import shutil
    ...
    bullwhip_bin = container_info["bullwhip_bin"]
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'bullwhip sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print("It looks like BullWhip isn't configured yet -- no API keys or providers found.")
        print()
        print("  Run:  bullwhip setup")
        print()

        from bullwhip_cli.setup import is_interactive_stdin, print_noninteractive_setup_guidance
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'bullwhip setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from bullwhip_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["BULLWHIP_YOLO_MODE"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["BULLWHIP_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from bullwhip_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    import subprocess
    from pathlib import Path
    from bullwhip_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Bull Whip?")
        print()
    ...
    session_dir = get_bullwhip_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: bullwhip gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  bullwhip gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '🐂 Bull Whip Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  bullwhip gateway")
        ...
        print()
        print("  Or install as a service: bullwhip gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'bullwhip whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from bullwhip_cli.setup import run_setup_wizard
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``bullwhip model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from bullwhip_cli.auth import (
        resolve_provider, AuthError, format_auth_error,
    )
    from bullwhip_cli.config import get_compatible_custom_providers, load_config, get_env_value
    ...
    effective_provider = (
        config_provider
        or os.getenv("BULLWHIP_INFERENCE_PROVIDER")
        or "auto"
    )
    try:
    ...
    from bullwhip_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    # a leftover OPENAI_BASE_URL in ~/.bullwhip/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.bullwhip/.env if the active provider is not 'custom'.
    ...
    """
    from bullwhip_cli.config import get_env_value, save_env_value, load_config
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from bullwhip_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from bullwhip_cli.auth import _prompt_model_selection, _save_model_choice, deactivate_provider
    from bullwhip_cli.config import get_env_value, save_env_value
    ...
    from bullwhip_cli.models import model_ids, get_pricing_for_provider
    ...
    if selected:
        ...
        from bullwhip_cli.config import load_config, save_config
        cfg = load_config()
        model = cfg.get("model")
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from bullwhip_cli.config import get_env_value, save_config, save_env_value
    from bullwhip_cli.nous_subscription import (
    ...
    from bullwhip_cli.models import (
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from bullwhip_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from bullwhip_cli.codex_models import get_codex_model_ids
    import argparse
    ...
    _codex_token = None
    # Prefer credential pool (where `bullwhip auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from bullwhip_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from bullwhip_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from bullwhip_cli.auth import _save_model_choice, deactivate_provider
    from bullwhip_cli.config import get_env_value, load_config, save_config
    ...
    from bullwhip_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `bullwhip model` to set a model.")
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from bullwhip_cli.curses_ui import flush_stdin
        flush_stdin()
        print()
...
from bullwhip_cli.models import _PROVIDER_MODELS
...
def _model_flow_copilot(config, current_model=""):
    ...
    from bullwhip_cli.config import save_env_value, load_config, save_config
    from bullwhip_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from bullwhip_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from bullwhip_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates BullWhip turns to `copilot --acp`.")
    print("  BullWhip currently starts its own ACP subprocess for each request.")
    print("  BullWhip uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print("  Set BULLWHIP_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere.")
        return
...
def _model_flow_kimi(config, current_model=""):
    ...
    from bullwhip_cli.config import get_env_value, save_env_value, load_config, save_config
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    from bullwhip_cli.config import get_env_value, save_env_value, load_config, save_config
    from bullwhip_cli.models import fetch_api_models, opencode_model_api_mode, normalize_opencode_model_id
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from bullwhip_constants import display_bullwhip_home as _dhh_fn
            print(f"    BullWhip will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env.")
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from bullwhip_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'bullwhip model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate BullWhip CLI with a provider."""
    from bullwhip_cli.auth import login_command
    login_command(args)


def cmd_logout(args):
    """Clear provider authentication."""
    from bullwhip_cli.auth import logout_command
    logout_command(args)


def cmd_auth(args):
    """Manage pooled credentials."""
    from bullwhip_cli.auth_commands import auth_command
    auth_command(args)


def cmd_status(args):
    """Show status of all components."""
    from bullwhip_cli.status import show_status
    show_status(args)


def cmd_cron(args):
    """Cron job management."""
    from bullwhip_cli.cron import cron_command
    cron_command(args)


def cmd_webhook(args):
    """Webhook subscription management."""
    from bullwhip_cli.webhook import webhook_command
    webhook_command(args)


def cmd_doctor(args):
    """Check configuration and dependencies."""
    from bullwhip_cli.doctor import run_doctor
    run_doctor(args)


def cmd_dump(args):
    """Dump setup summary for support/debugging."""
    from bullwhip_cli.dump import run_dump
    run_dump(args)


def cmd_debug(args):
    """Debug tools (share report, etc.)."""
    from bullwhip_cli.debug import run_debug
    run_debug(args)


def cmd_config(args):
    """Configuration management."""
    from bullwhip_cli.config import config_command
    config_command(args)


def cmd_backup(args):
    """Back up BullWhip home directory to a zip file."""
    if getattr(args, "quick", False):
        from bullwhip_cli.backup import run_quick_backup
        run_quick_backup(args)
    else:
        from bullwhip_cli.backup import run_backup
        run_backup(args)


def cmd_import(args):
    """Restore a BullWhip backup from a zip file."""
    from bullwhip_cli.backup import run_import
    run_import(args)


def cmd_version(args):
    """Show version."""
    print(f"Bull Whip Agent v{__version__} ({__release_date__})")
    ...
    try:
        from bullwhip_cli.banner import check_for_updates
        from bullwhip_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Bull Whip Agent."""
    _require_tty("uninstall")
    from bullwhip_cli.uninstall import run_uninstall
    run_uninstall(args)
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``bullwhip update --gateway`` so interactive prompts (stash restore,
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``bullwhip web``).
    ...
    if r1.returncode != 0:
        print(f"  {'✗' if fatal else '⚠'} Web UI npm install failed"
              + ("" if fatal else " (bullwhip web will not be available)"))
        if fatal:
...
def _update_via_zip(args):
    """Update Bull Whip Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = f"https://github.com/NousResearch/bullwhip-agent/archive/refs/heads/{branch}.zip"
    
    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="bullwhip-update-")
        zip_path = os.path.join(tmp_dir, f"bullwhip-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to bullwhip-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"bullwhip-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime("bullwhip-update-autostash-%Y%m%d-%H%M%S")
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if BullWhip behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes bullwhip completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print("⚠ Local changes were restored, but BullWhip couldn't find the stash entry to drop.")
        ...
        if drop.returncode != 0:
            print("⚠ Local changes were restored, but BullWhip couldn't drop the saved stash entry.")
    ...
    print("  Review `git diff` / `git status` if BullWhip behaves unexpectedly.")
    return True

# =========================================================================
# Fork detection and upstream management for `bullwhip update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/bullwhip-agent.git",
    "git@github.com:NousResearch/bullwhip-agent.git",
    "https://github.com/NousResearch/bullwhip-agent",
    "git@github.com:NousResearch/bullwhip-agent",
}
OFFICIAL_REPO_URL = "https://github.com/NousResearch/bullwhip-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    from bullwhip_constants import get_bullwhip_home
    return (get_bullwhip_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        from bullwhip_constants import get_bullwhip_home
        (get_bullwhip_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official BullWhip repository.")
        print("  This means you may miss updates from NousResearch/bullwhip-agent.")
        print()
        try:
        ...
        if response in ("", "y", "yes"):
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print("  ✓ Added upstream: https://github.com/NousResearch/bullwhip-agent.git")
                has_upstream = True
            else:
                print("  ✗ Failed to add upstream remote. Skipping upstream sync.")
                return
        else:
            print("  Skipped. Run 'git remote add upstream https://github.com/NousResearch/bullwhip-agent.git' to add later.")
...
def _invalidate_update_cache():
    ...
    ``bullwhip update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from bullwhip_constants import get_defaultbullwhip_root
    default_home = get_defaultbullwhip_root()
...
def _load_installable_optional_extras() -> list[str]:
    ...
    # Entries look like "bullwhip-agent[matrix]" or "package-name[extra]".
...
def cmd_update(args):
    """Update Bull Whip Agent to the latest version."""
    import shutil
    from bullwhip_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Bull Whip Agent")
        return
    ...
    print("🐂 Updating Bull Whip Agent...")
    print()
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print("  curl -fsSL https://raw.githubusercontent.com/NousResearch/bullwhip-agent/main/scripts/install.sh | bash")
            sys.exit(1)
    ...
    try:
        ...
        # the old bytecode (e.g. get_bullwhip_home added to bullwhip_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}")
        ...
        # modules in this process.  Reload bullwhip_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_bullwhip_home() added since the last release.
        try:
            import importlib
            import bullwhip_constants as _hc
        ...
        try:
            from bullwhip_cli.profiles import list_profiles, get_active_profile_name, seed_profile_skills
        ...
        if needs_migration:
            ...
            if gateway_mode:
                ...
                print("    Run 'bullwhip config migrate' later to apply any new config/env options.")
                response = "n"
            else:
                try:
            ...
            if response in ('', 'y', 'yes'):
                ...
                if gateway_mode and missing_env:
                    print("  ℹ API keys require manual entry: bullwhip config migrate")
            else:
                print()
                print("Skipped. Run 'bullwhip config migrate' later to configure.")
        else:
        ...
        # When running as ``bullwhip update --gateway`` (spawned by the gateway's
        # /update command), this process lives inside the gateway's systemd
        # cgroup.  ``systemctl restart bullwhip-gateway`` kills everything in the
        ...
        if gateway_mode:
            _exit_code_path = get_bullwhip_home() / ".update_exit_code"
            try:
        ...
        try:
            from bullwhip_cli.gateway import (
            ...
            # Discover all bullwhip-gateway* units (default + profiles)
            if supports_systemd_services():
                ...
                for scope, scope_cmd in [("user", ["systemctl", "--user"]), ("system", ["systemctl"])]:
                    try:
                        result = subprocess.run(
                            scope_cmd + ["list-units", "bullwhip-gateway*", "--plain", "--no-legend", "--no-pager"],
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            if not parts:
                                continue
                            unit = parts[0]  # e.g. bullwhip-gateway.service or bullwhip-gateway-coder.service
            ...
            if is_macos():
                try:
                    from bullwhip_cli.gateway import launchd_restart, get_launchd_label, get_launchd_plist_path
            ...
            if restarted_services or killed_pids:
                ...
                if killed_pids:
                    ...
                    print("    Restart manually: bullwhip gateway run")
                    # Also restart for each profile if needed
                    if len(killed_pids) > 1:
                        print("    (or: bullwhip -p <profile> gateway run  for each profile)")
        ...
        print("  bullwhip model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``bullwhip -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from bullwhip_cli.profiles import (
    ...
    from bullwhip_constants import display_bullwhip_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `bullwhip profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_bullwhip_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → bullwhip -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.bullwhip)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(f"  Choose a custom alias:  bullwhip profile alias {name} --name <custom>")
                    print(f"  Or access via flag:     bullwhip -p {name} chat")
                else:
        ...
        name = args.profile_name
        from bullwhip_cli.profiles import get_profile_dir, profile_exists, _read_config_model, _check_gateway_running, _count_skills
        ...
        from bullwhip_cli.profiles import profile_exists
        if not profile_exists(name):
            print(f"Error: Profile '{name}' does not exist.")
            sys.exit(1)
        ...
        if remove:
            ...
            if wrapper_path:
                ...
                if custom_name:
                    wrapper_path.write_text(f'#!/bin/sh\nexec bullwhip -p {name} "$@"\n')
                print(f"✓ Alias created: {wrapper_path}")
                if not _is_wrapper_dir_in_path():
                    print(f"⚠ {_get_wrapper_dir()} is not in your PATH.")

    elif action == "rename":
        from bullwhip_cli.profiles import rename_profile
        try:
        ...
        from bullwhip_cli.profiles import export_profile
        name = args.profile_name
        ...
        from bullwhip_cli.profiles import import_profile
        try:
...
def cmd_dashboard(args):
    ...
    try:
        ...
        print("Install them with:  pip install bullwhip-agent[web]")
        sys.exit(1)

    if not _build_web_ui(PROJECT_ROOT / "web", fatal=True):
        sys.exit(1)

    from bullwhip_cli.web_server import start_server
    start_server(
        host=args.host,
        port=args.port,
...
def cmd_completion(args, parser=None):
    ...
    from bullwhip_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    """View and filter BullWhip log files."""
    from bullwhip_cli.logs import tail_log, list_logs
...
def main():
    """Main entry point for bullwhip CLI."""
    parser = argparse.ArgumentParser(
        prog="bullwhip",
        description="Bull Whip Agent - AI assistant with tool-calling capabilities",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    bullwhip                        Start interactive chat
    bullwhip chat -q "Hello"        Single query mode
    bullwhip -c                     Resume the most recent session
    bullwhip -c "my project"        Resume a session by name (latest in lineage)
    bullwhip --resume <session_id>  Resume a specific session by ID
    bullwhip setup                  Run setup wizard
    bullwhip logout                 Clear stored authentication
    bullwhip auth add <provider>    Add a pooled credential
    bullwhip auth list              List pooled credentials
    bullwhip auth remove <p> <t>    Remove pooled credential by index, id, or label
    bullwhip auth reset <provider>  Clear exhaustion status for a provider
    bullwhip model                  Select default model
    bullwhip config                 View configuration
    bullwhip config edit            Edit config in $EDITOR
    bullwhip config set model gpt-4 Set a config value
    bullwhip gateway                Run messaging gateway
    bullwhip -s bullwhip-agent-dev,github-auth
    bullwhip -w                     Start in isolated git worktree
    bullwhip gateway install        Install gateway background service
    bullwhip sessions list          List past sessions
    bullwhip sessions browse        Interactive session picker
    bullwhip sessions rename ID T   Rename/title a session
    bullwhip logs                   View agent.log (last 50 lines)
    bullwhip logs -f                Follow agent.log in real time
    bullwhip logs errors            View errors.log
    bullwhip logs --since 1h        Lines from the last hour
    bullwhip debug share             Upload debug report for support
    bullwhip update                 Update to latest version

For more help on a command:
    bullwhip <command> --help
"""
    )
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Bull Whip Agent"
    )
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: bullwhip-cli)"
    )
    model_parser.add_argument(
        "--scope",
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Bull Whip Agent with an interactive wizard. "
                    "Run a specific section: bullwhip setup model|tts|terminal|gateway|tools|agent"
    )
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Bull Whip CLI"
    )
    ...
    login_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use (default: bullwhip-cli)"
    )
    login_parser.add_argument(
        "--scope",
        default=None,
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Bull Whip Agent components"
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Bull Whip Agent setup"
    )
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your BullWhip setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Bull Whip Agent. Use 'bullwhip debug share' to "
                    "upload a debug report (system info + recent logs) to a paste "
                    "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    bullwhip debug share              Upload debug report and print URL
    bullwhip debug share --lines 500  Include more log lines
    bullwhip debug share --expire 30  Keep paste for 30 days
    bullwhip debug share --local      Print report locally (no upload)
""",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up BullWhip home directory to a zip file",
        description="Create a zip archive of your entire BullWhip configuration, "
                    "skills, sessions, and data (excludes the bullwhip-agent codebase). "
                    "Use --quick for a fast snapshot of just critical state files."
    )
    backup_parser.add_argument(
        "-o", "--output",
        help="Output path for the zip file (default: ~/bullwhip-backup-<timestamp>.zip)"
    )
    backup_parser.add_argument(
        "-q", "--quick",
        action="store_true",
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a BullWhip backup from a zip file",
        description="Extract a previously created BullWhip backup into your "
                    "BullWhip home directory, restoring configuration, skills, "
                    "sessions, and data"
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Bull Whip Agent configuration"
    )
    ...
    def cmd_pairing(args):
        from bullwhip_cli.pairing import pairing_command
    ...
    def cmd_skills(args):
        ...
        if getattr(args, 'skills_action', None) == 'config':
            ...
            from bullwhip_cli.skills_config import skills_command as skills_config_command
            skills_config_command(args)
        else:
            from bullwhip_cli.skills_hub import skills_command
            skills_command(args)
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/bullwhip-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
    ...
    def cmd_plugins(args):
        from bullwhip_cli.plugins_cmd import plugins_command
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            from bullwhip_cli.memory_setup import memory_command
            memory_command(args)
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'bullwhip tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # bullwhip tools list [--platform cli]
    ...
    # bullwhip tools disable <name...> [--platform cli]
    ...
    # bullwhip tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            from bullwhip_cli.tools_config import tools_disable_enable_command
            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from bullwhip_cli.tools_config import tools_command
            tools_command(args)
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run BullWhip as an MCP server",
        description=(
            "Manage MCP server connections and run BullWhip as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'bullwhip mcp add' to connect to a new server, or\n"
            "'bullwhip mcp serve' to expose BullWhip conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run BullWhip as an MCP server (expose conversations to other agents)",
    )
    ...
    def cmd_mcp(args):
        from bullwhip_cli.mcp_config import mcp_command
        mcp_command(args)
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch bullwhip --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            import shutil
            bullwhip_bin = shutil.which("bullwhip")
            if bullwhip_bin:
                os.execvp(bullwhip_bin, ["bullwhip", "--resume", selected_id])
            else:
                # Fallback: re-invoke via python -m
                os.execvp(
                    sys.executable,
                    [sys.executable, "-m", "bullwhip_cli.main", "--resume", selected_id],
                )
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Bull Whip"
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Bull Whip",
    ...
    def cmd_claw(args):
        from bullwhip_cli.claw import claw_command
        claw_command(args)
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Bull Whip Agent to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Bull Whip Agent",
        description="Remove Bull Whip Agent from your system. Can keep configs/data for reinstall."
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Bull Whip Agent as an ACP (Agent Client Protocol) server",
        description="Start Bull Whip Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )

    def cmd_acp(args):
        """Launch Bull Whip Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated BullWhip instances",
    )
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Bull Whip Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter BullWhip log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    bullwhip logs                    Show last 50 lines of agent.log
    bullwhip logs -f                 Follow agent.log in real time
    bullwhip logs errors             Show last 50 lines of errors.log
    bullwhip logs gateway -n 100     Show last 100 lines of gateway.log
    bullwhip logs --level WARNING    Only show WARNING and above
    bullwhip logs --session abc123   Filter by session ID
    bullwhip logs --component tools  Only show tool-related lines
    bullwhip logs --since 1h         Lines from the last hour
    bullwhip logs --since 30m -f     Follow, starting from 30 min ago
    bullwhip logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``bullwhip -c Pokemon Agent Dev`` → ``bullwhip -c 'Pokemon Agent Dev'``
