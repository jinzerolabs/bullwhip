...
- ``bullwhip_cli.providers``        -- canonical provider identity + overlays
- ``bullwhip_cli.model_normalize``  -- per-provider name formatting
...
from bullwhip_cli.providers import (
...
_BULLWHIP_MODEL_WARNING = (
    "ZeroLabs Korea BullWhip 3 & 4 models are NOT agentic and are not designed "
    "for use with Bull Whip Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real ZeroLabs Korea BullWhip 3 / BullWhip 4 chat families.
# The previous substring check (`"bullwhip" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``bullwhip-brain:qwen3-14b-ctx16k`` that just
# happen to carry "bullwhip" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/BullWhip-3-Llama-3.1-70B, bullwhip-4-405b, openrouter/hermes3:70b
# Negative examples it must NOT match:
#   bullwhip-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_BULLWHIP_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])bullwhip[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_bullwhip_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Bull Whip 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_BULLWHIP_NON_AGENTIC_RE.search(model_name))


def _checkbullwhip_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Bull Whip 3/4 chat model."""
    if is_nous_bullwhip_non_agentic(model_name):
        return _BULLWHIP_MODEL_WARNING
    return ""
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'bullwhip model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'bullwhip doctor' — config issues detected:"
    ...
    hermes_warn = _checkbullwhip_model_warning(new_model)
    if hermes_warn:
        warnings.append(bullwhip_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from bullwhip_cli/models.py (OPENROUTER_MODELS,
    ...
    from bullwhip_cli.models import OPENROUTER_MODELS, _PROVIDER_MODELS
    ...
    # Build curated model lists keyed by bullwhip provider ID
    ...
    # --- 1. Check BullWhip-mapped providers ---
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(bullwhip_id)
        ...
        model_ids = curated.get(bullwhip_id, [])
        total = len(model_ids)
    ...
    # --- 2. Check BullWhip-only providers (nous, openai-codex, copilot, opencode-go) ---
    from bullwhip_cli.providers import BULLWHIP_OVERLAYS
    from bullwhip_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → BullWhip provider ID.
    # BULLWHIP_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use BullWhip IDs ("copilot").
    _mdev_to_bullwhip = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in BULLWHIP_OVERLAYS.items():
        if pid in seen_slugs:
            continue

        # Resolve BullWhip slug — e.g. "github-copilot" → "copilot"
        hermes_slug = _mdev_to_bullwhip.get(pid, pid)
        ...
        if not has_creds:
            try:
                from bullwhip_cli.auth import _load_auth_store
        ...
        if not has_creds and hermes_slug == "anthropic":
            try:
                from agent.anthropic_adapter import (
                    ...
                    read_bullwhip_oauth_credentials,
                )
                hermes_creds = read_bullwhip_oauth_credentials()
                cc_creds = read_claude_code_credentials()
                if (bullwhip_creds and hermes_creds.get("accessToken")) or \
        ...
        # Use curated list — look up by Bull Whip slug, fall back to overlay key
        model_ids = curated.get(bullwhip_slug, []) or curated.get(pid, [])
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": hermes_slug,
            "name": get_label(bullwhip_slug),
        ...
        seen_slugs.add(pid)
        seen_slugs.add(bullwhip_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or BULLWHIP_OVERLAYS (keeps /model in sync
    # with `bullwhip model`).
    try:
        from bullwhip_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
