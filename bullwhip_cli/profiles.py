"""
Profile management for multiple isolated BullWhip instances.

Each profile is a fully independent BULLWHIP_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.bullwhip/profiles/<name>/`` by default.

The "default" profile is ``~/.bullwhip`` itself — backward compatible,
zero migration needed.

Usage::

    bullwhip profile create coder          # fresh profile + bundled skills
    bullwhip profile create coder --clone  # also copy config, .env, SOUL.md
    bullwhip profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    bullwhip -p coder chat                 # or via flag
    bullwhip profile use coder             # set as sticky default
    bullwhip profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See bullwhip_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# Directories/files to exclude when exporting the default (~/.bullwhip) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "bullwhip-agent",         # repo checkout (multi-GB)
...
_RESERVED_NAMES = frozenset({
    "bullwhip", "default", "test", "tmp", "root", "sudo",
})

# Bull Whip subcommands that cannot be used as profile names/aliases
_BULLWHIP_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the bullwhip root, NOT to the current BULLWHIP_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where BULLWHIP_HOME points outside
    ``~/.bullwhip``, profiles live under ``BULLWHIP_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_bullwhip_home() / "profiles"


def _get_default_bullwhip_home() -> Path:
    """Return the default (pre-profile) BULLWHIP_HOME path.

    In standard deployments this is ``~/.bullwhip``.
    In Docker/custom deployments where BULLWHIP_HOME is outside ``~/.bullwhip``
    (e.g. ``/opt/data``), returns BULLWHIP_HOME directly.
    """
    from bullwhip_constants import get_defaultbullwhip_root
    return get_defaultbullwhip_root()


def _get_active_profile_path() -> Path:
    """Return the path to the sticky active_profile file."""
    return _get_default_bullwhip_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name == "default":
        return  # special alias for ~/.bullwhip
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its BULLWHIP_HOME directory."""
    if name == "default":
        return _get_default_bullwhip_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, bullwhip subcommands, existing binaries in PATH.
    """
    if name in _RESERVED_NAMES:
        return f"'{name}' is a reserved name"
    if name in _BULLWHIP_SUBCOMMANDS:
        return f"'{name}' conflicts with a bullwhip subcommand"
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_bullwhip_home()
...
def create_profile(
    ...
    if name == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.bullwhip)."
        )
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches BULLWHIP_HOME at module level.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "BULLWHIP_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if name == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.bullwhip).\n"
            "To remove everything, use: bullwhip uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set BULLWHIP_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("BULLWHIP_HOME")
    try:
        os.environ["BULLWHIP_HOME"] = str(profile_dir)
        from bullwhip_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["BULLWHIP_HOME"] = old_home
        elif "BULLWHIP_HOME" in os.environ:
            del os.environ["BULLWHIP_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.bullwhip/active_profile``. Use ``"default"`` to clear.
    """
    validate_profile_name(name)
    if name != "default" and not profile_exists(name):
        raise FileNotFoundError(
            f"Profile '{name}' does not exist. "
            f"Create it with: bullwhip profile create {name}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from BULLWHIP_HOME.

    Returns ``"default"`` if BULLWHIP_HOME is not set or points to ``~/.bullwhip``.
    Returns the profile name if BULLWHIP_HOME points into ``~/.bullwhip/profiles/<name>``.
    Returns ``"custom"`` if BULLWHIP_HOME is set to an unrecognized path.
    """
    from bullwhip_constants import get_bullwhip_home
    bullwhip_home = get_bullwhip_home()
    resolved = bullwhip_home.resolve()

    default_resolved = _get_default_bullwhip_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if name == "default":
        # The default profile IS ~/.bullwhip itself — its parent is ~/ and its
        # directory name is ".bullwhip", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: bullwhip profile import <archive> --name <name>"
        )

    # Archives exported from the default profile have "default/" as top-level
    # dir.  Importing as "default" would target ~/.bullwhip itself — disallow
    # that and guide the user toward a named profile.
    if inferred_name == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.bullwhip). "
            "Specify a different name: bullwhip profile import <archive> --name <name>"
        )
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for bullwhip profile names."""
    return '''# Bull Whip Agent profile completion
# Add to ~/.bashrc: eval "$(bullwhip completion bash)"

bullwhip_profiles() {
    local profiles_dir="$HOME/.bullwhip/profiles"
...
_bullwhip_completion() {
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(bullwhip_profiles)" -- "$cur"))
        return
    fi
...
complete -F _bullwhip_completion bullwhip
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for bullwhip profile names."""
    return '''#compdef bullwhip
# Bull Whip Agent profile completion
# Add to ~/.zshrc: eval "$(bullwhip completion zsh)"

_hermes() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.bullwhip/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.bullwhip/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a BULLWHIP_HOME path string.

    Called early in the CLI entry point, before any bullwhip modules
    are imported, to set the BULLWHIP_HOME environment variable.
    """
    ...
    if profile_name != "default" and not profile_dir.is_dir():
        raise FileNotFoundError(
            ...
            f"Create it with: bullwhip profile create {profile_name}"
        )
