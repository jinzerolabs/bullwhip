"""``bullwhip logs`` — view and filter BullWhip log files.
...
Usage examples::
    ...
    bullwhip logs                    # last 50 lines of agent.log
    bullwhip logs -f                 # follow agent.log in real time
    bullwhip logs errors             # last 50 lines of errors.log
    bullwhip logs gateway -n 100    # last 100 lines of gateway.log
    bullwhip logs --level WARNING    # only WARNING+ lines
    bullwhip logs --session abc123   # filter by session ID substring
    bullwhip logs --component tools  # only tool-related lines
    bullwhip logs --since 1h         # lines from the last hour
    bullwhip logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_bullwhip_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when BullWhip runs — try 'bullwhip chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_bullwhip_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_bullwhip_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_bullwhip_home()}/logs/")
        return

    print(f"Log files in {display_bullwhip_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'bullwhip chat' to generate logs)")
