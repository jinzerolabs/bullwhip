"""
Bull Whip Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m bullwhip_cli.main web          # Start on http://127.0.0.1:9119
    python -m bullwhip_cli.main web --port 8080
"""
...
try:
    ...
    raise SystemExit(
        ...
        "Run 'bullwhip web' to auto-install, or: pip install bullwhip-agent[web]"
    )

WEB_DIST = Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)

app = FastAPI(title="Bull Whip Agent", version=__version__)
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    BullWhip supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# the canonical ``bullwhip auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    BullWhip resolves Anthropic creds in this order at runtime:
    1. ``~/.bullwhip/.anthropic_oauth.json`` — BullWhip-managed PKCE flow
    ...
    try:
        from agent.anthropic_adapter import (
            ...
            _BULLWHIP_OAUTH_FILE,
        )
    except ImportError:
        read_claude_code_credentials = None  # type: ignore
        read_bullwhip_oauth_credentials = None  # type: ignore
        _BULLWHIP_OAUTH_FILE = None  # type: ignore

    hermes_creds = None
    if read_bullwhip_oauth_credentials:
        try:
    ...
    if hermes_creds and hermes_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "bullwhip_pkce",
            "source_label": f"BullWhip PKCE ({_BULLWHIP_OAUTH_FILE})",
            "token_preview": _truncate_token(bullwhip_creds.get("accessToken")),
            "expires_at": hermes_creds.get("expiresAt"),
            "has_refresh_token": bool(bullwhip_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Bull Whip even
    when they also have a separate BullWhip-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "pkce",
        "cli_command": "bullwhip auth add anthropic",
    ...
    },
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "bullwhip auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "bullwhip auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "bullwhip auth add qwen-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from bullwhip_cli import auth as hauth
        if provider_id == "nous":
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("bullwhip_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same BullWhip-managed PKCE file
    # AND forget the Claude Code import. We don't touch ~/.claude/* directly
    # — that's owned by the Claude Code CLI; users can re-auth there if they
    # want to undo a disconnect.
    if provider_id in ("anthropic", "claude-code"):
        try:
            from agent.anthropic_adapter import _BULLWHIP_OAUTH_FILE
            if _BULLWHIP_OAUTH_FILE.exists():
                _BULLWHIP_OAUTH_FILE.unlink()
        except Exception:
            pass
        # Also clear the credential pool entry if present.
        try:
            from bullwhip_cli.auth import clear_provider_auth
...
#          → persists to ~/.bullwhip/.anthropic_oauth.json AND credential pool
...
# Guarded so bullwhip web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both BullWhip file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``bullwhip auth add anthropic``.
    """
    from agent.anthropic_adapter import _BULLWHIP_OAUTH_FILE
    payload = {
    ...
    _BULLWHIP_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    _BULLWHIP_OAUTH_FILE.write_text(json.dumps(payload, indent=2), encoding="utf-8")
...
def _submit_anthropic_pkce(session_id: str, code_input: str) -> Dict[str, Any]:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "bullwhip-dashboard/1.0",
        },
        method="POST",
    )
    try:
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    from bullwhip_cli import auth as hauth
    if provider_id == "nous":
        from bullwhip_cli.auth import _request_device_code, PROVIDER_REGISTRY
        import httpx
        pconfig = PROVIDER_REGISTRY["nous"]
        portal_base_url = (
            os.getenv("BULLWHIP_PORTAL_BASE_URL")
...
def _nous_poller(session_id: str) -> None:
    ...
    from bullwhip_cli.auth import _poll_for_token, refresh_nous_oauth_from_state
...
async def get_logs(
    ...
    from bullwhip_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_bullwhip_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from bullwhip_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
async def get_skills():
    ...
    from bullwhip_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from bullwhip_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
async def get_toolsets():
    from bullwhip_cli.tools_config import (
...
def mount_spa(application: FastAPI):
    ...
    def _serve_index():
        ...
        token_script = (
            f'<script>window.__BULLWHIP_SESSION_TOKEN__="{_SESSION_TOKEN}";</script>'
        )
...
def start_server(
    ...
    print(f"  BullWhip Web UI → http://{host}:{port}")
