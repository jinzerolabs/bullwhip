"""Shell completion script generation for bullwhip CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(bullwhip_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Bull Whip Agent bash completion
# Add to ~/.bashrc:
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef bullwhip
# Bull Whip Agent zsh completion
# Add to ~/.zshrc:
...
_hermes() {{
    ...
    _arguments -C \\
        ...
        '(-p --profile){{-p,--profile}}[Profile name]:profile:bullwhip_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'bullwhip command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Bull Whip Agent fish completion",
        "# Add to your config:",
        "#   bullwhip completion fish | source",
        "",
        "# Helper: list available profiles",
        "function _bullwhip_profiles",
        "    echo default",
        "    if test -d $HOME/.bullwhip/profiles",
        "        ls $HOME/.bullwhip/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        "# Disable file completion by default",
        "complete -c bullwhip -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c bullwhip -f -s p -l profile"
        " -d 'Profile name' -xa '(_bullwhip_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(_bullwhip_profiles)' -d 'Profile name'"
                )
