"""Slash command definitions and autocomplete for the Bull Whip CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Bull Whip config/state", "Session",
    ...
    CommandDef("update", "Update Bull Whip Agent to the latest version", "Info",
               gateway_only=True),
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``bullwhip skills
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /bullwhip handler.

    Maps both canonical names and aliases so /bullwhip bg do stuff works
    the same as /bullwhip background do stuff.
    """
    overrides = _resolve_config_gates()
