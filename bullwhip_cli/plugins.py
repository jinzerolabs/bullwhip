"""
BullWhip Plugin System
====================

Discovers, loads, and manages plugins from three sources:

1. **User plugins**   – ``~/.bullwhip/plugins/<name>/``
2. **Project plugins** – ``./.bullwhip/plugins/<name>/`` (opt-in via
   ``BULLWHIP_ENABLE_PROJECT_PLUGINS``)
3. **Pip plugins**     – packages that expose the ``bullwhip_agent.plugins``
   entry-point group.
...
ENTRY_POINTS_GROUP = "bullwhip_agent.plugins"
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``bullwhip honcho ...``).
    ...
    def register_skill(
        ...
        ``~/.bullwhip/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self) -> None:
        ...
        # 1. User plugins (~/.bullwhip/plugins/)
        user_dir = get_bullwhip_home() / "plugins"
        manifests.extend(self._scan_directory(user_dir, source="user"))

        # 2. Project plugins (./.bullwhip/plugins/)
        if _env_enabled("BULLWHIP_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".bullwhip" / "plugins"
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``bullwhip tools`` TUI so plugin-provided toolsets appear
