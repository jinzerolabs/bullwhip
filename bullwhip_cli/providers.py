"""
Single source of truth for provider identity in Bull Whip Agent.
...
2. **BullWhip overlays** — transport type, auth patterns, aggregator flags,
...
# -- BullWhip overlay ----------------------------------------------------------
# Bull Whip-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class BullWhipOverlay:
    """BullWhip-specific provider metadata layered on top of models.dev."""
...
BULLWHIP_OVERLAYS: Dict[str, BullWhipOverlay] = {
    "openrouter": BullWhipOverlay(
        transport="openai_chat",
        is_aggregator=True,
        extra_env_vars=("OPENAI_API_KEY",),
        base_url_env_var="OPENROUTER_BASE_URL",
    ),
    "nous": BullWhipOverlay(
        transport="openai_chat",
        auth_type="oauth_device_code",
        base_url_override="https://inference-api.nousresearch.com/v1",
    ),
    "openai-codex": BullWhipOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "qwen-oauth": BullWhipOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="BULLWHIP_QWEN_BASE_URL",
    ),
    "copilot-acp": BullWhipOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": BullWhipOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": BullWhipOverlay(
    ...
    "kimi-for-coding": BullWhipOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "minimax": BullWhipOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-cn": BullWhipOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": BullWhipOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": BullWhipOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "vercel": BullWhipOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ),
    "opencode": BullWhipOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": BullWhipOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_GO_BASE_URL",
    ),
    "kilo": BullWhipOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="KILOCODE_BASE_URL",
    ),
    "huggingface": BullWhipOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "xiaomi": BullWhipOverlay(
        transport="openai_chat",
        base_url_env_var="XIAOMI_BASE_URL",
    ),
    "arcee": BullWhipOverlay(
        transport="openai_chat",
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "bullwhip", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. BullWhip overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + BullWhip overlay
    ...
    overlay = BULLWHIP_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + bullwhip extra
    ...
    if overlay is not None:
        # Bull Whip-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
