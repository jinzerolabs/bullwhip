"""
Unified tool configuration for Bull Whip Agent.

`bullwhip tools` and `bullwhip setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.bullwhip/config.yaml under
...
from bullwhip_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _BULLWHIP_CORE_TOOLS (available at runtime if enabled),
...
def _get_effective_configurable_toolsets():
    ...
    try:
        from bullwhip_cli.plugins import discover_plugins, get_plugin_toolsets
...
from bullwhip_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
    ...
    if post_setup_key in ("agent_browser", "browserbase"):
        ...
        if not node_modules.exists() and shutil.which("npm"):
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_bullwhip_home()}/bullwhip-agent && npm install")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in bullwhip-agent directory)")
...
def _get_platform_tools(
    ...
    *,
    ...
    # "bullwhip-cli" (which include all _BULLWHIP_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # (e.g. "bullwhip-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
    ...
    # A plugin toolset is "known" for a platform once `bullwhip tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by bullwhip tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (bullwhip-cli, bullwhip-telegram, etc.)
    ...
    if not env_vars:
        ...
        if managed_feature:
            ...
            if any(get_env_value(env_var) for env_var in override_envs):
                _print_warning(
                    "  Direct credentials are still configured and may take precedence until you remove them from ~/.bullwhip/.env."
                )
        return
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `bullwhip tools` and `bullwhip setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("🐂 Bull Whip Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://bullwhip-agent.nousresearch.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print()
    from bullwhip_constants import display_bullwhip_home
    print(color(f"  Tool configuration saved to {display_bullwhip_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'bullwhip' or gateway restart.", Colors.DIM))
    print()
