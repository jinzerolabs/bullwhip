...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "bullwhip"
        assert config.workspace_id == "bullwhip"
...
class TestFromGlobalConfig:
    ...
    def test_context_tokens_host_block_wins(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "contextTokens": 1000,
            "hosts": {"bullwhip": {"contextTokens": 2000}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"bullwhip": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"bullwhip": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="bullwhip-agent"
        ):
            result = config.resolve_session_name("/home/user/bullwhip-agent/subdir")
        assert result == "bullwhip-agent"
...
class TestResolveConfigPath:
    def test_prefers_bullwhip_home_when_exists(self, tmp_path):
        bullwhip_home = tmp_path / "bullwhip"
        bullwhip_home.mkdir()
        local_cfg = bullwhip_home / "honcho.json"
    ...
    def test_falls_back_to_global_when_no_local(self, tmp_path):
        bullwhip_home = tmp_path / "bullwhip"
        bullwhip_home.mkdir()
        # No honcho.json in BULLWHIP_HOME — also isolate ~/.bullwhip so
    ...
    def test_falls_back_to_global_without_bullwhip_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()

        with patch.dict(os.environ, {}, clear=False), \
             patch.object(Path, "home", return_value=fake_home):
            os.environ.pop("BULLWHIP_HOME", None)
            result = resolve_config_path()
        assert result == GLOBAL_CONFIG_PATH
...
class TestResolveActiveHost:
    def test_default_returns_hermes(self):
        with patch.dict(os.environ, {}, clear=True):
            os.environ.pop("BULLWHIP_HONCHO_HOST", None)
            os.environ.pop("BULLWHIP_HOME", None)
            assert resolve_active_host() == "bullwhip"

    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"BULLWHIP_HONCHO_HOST": "bullwhip.coder"}):
            assert resolve_active_host() == "bullwhip.coder"

    def test_profile_name_derives_host(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("BULLWHIP_HONCHO_HOST", None)
            with patch("bullwhip_cli.profiles.get_active_profile_name", return_value="coder"):
                assert resolve_active_host() == "bullwhip.coder"

    def test_default_profile_returns_hermes(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("BULLWHIP_HONCHO_HOST", None)
            with patch("bullwhip_cli.profiles.get_active_profile_name", return_value="default"):
                assert resolve_active_host() == "bullwhip"

    def test_custom_profile_returns_hermes(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("BULLWHIP_HONCHO_HOST", None)
            with patch("bullwhip_cli.profiles.get_active_profile_name", return_value="custom"):
                assert resolve_active_host() == "bullwhip"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("BULLWHIP_HONCHO_HOST", None)
            # Temporarily remove bullwhip_cli.profiles to simulate import failure
            saved = sys.modules.get("bullwhip_cli.profiles")
            sys.modules["bullwhip_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "bullwhip"
            finally:
                if saved is not None:
                    sys.modules["bullwhip_cli.profiles"] = saved
                else:
                    sys.modules.pop("bullwhip_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="bullwhip.coder")
        assert config.host == "bullwhip.coder"
        assert config.workspace_id == "bullwhip"  # shared workspace
        assert config.ai_peer == "bullwhip.coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="bullwhip")
        assert config.host == "bullwhip"
        assert config.workspace_id == "bullwhip"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "shared-key",
            "hosts": {
                "bullwhip": {"aiPeer": "bullwhip", "peerName": "alice"},
                "bullwhip.coder": {
                    "aiPeer": "bullwhip.coder",
        ...
        config = HonchoClientConfig.from_global_config(
            host="bullwhip.coder", config_path=config_file,
        )
        assert config.host == "bullwhip.coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "bullwhip.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="bullwhip.dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "bullwhip.dreamer"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"bullwhip": {"enabled": True, "aiPeer": "bullwhip"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"bullwhip": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
...
class TestInitOnSessionStart:
    ...
    def test_host_block_overrides_root(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "initOnSessionStart": True,
            "hosts": {"bullwhip": {"initOnSessionStart": False}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
    ...
    def test_host_block_true_overrides_root_absent(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"bullwhip": {"initOnSessionStart": True}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.init_on_session_start is True
