...
- Path resolution (absolute, relative to BULLWHIP_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp BULLWHIP_HOME."""
    bullwhip_home = tmp_path / ".bullwhip"
    bullwhip_home.mkdir()
    (bullwhip_home / "cron").mkdir()
    (bullwhip_home / "cron" / "output").mkdir()
    (bullwhip_home / "scripts").mkdir()
    monkeypatch.setenv("BULLWHIP_HOME", str(bullwhip_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "BULLWHIP_DIR", bullwhip_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", bullwhip_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", bullwhip_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", bullwhip_home / "cron" / "output")
...
class TestCronjobToolScript:
    ...
    def test_create_with_script(self, cron_env, monkeypatch):
        monkeypatch.setenv("BULLWHIP_INTERACTIVE", "1")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.bullwhip/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("BULLWHIP_SESSION_PLATFORM") is None
        assert os.environ.get("BULLWHIP_SESSION_CHAT_ID") is None
        assert os.environ.get("BULLWHIP_SESSION_CHAT_NAME") is None
