...
- Unlimited timeout (BULLWHIP_CRON_TIMEOUT=0)
- Backward compat: BULLWHIP_CRON_TIMEOUT env var still works
...
class TestInactivityTimeout:
    ...
    def test_unlimited_timeout(self):
        """BULLWHIP_CRON_TIMEOUT=0 means no timeout at all."""
        agent = FakeAgent(idle_seconds=0.0)
    ...
    def test_timeout_env_var_parsing(self, monkeypatch):
        """BULLWHIP_CRON_TIMEOUT env var is respected."""
        monkeypatch.setenv("BULLWHIP_CRON_TIMEOUT", "1200")
    ...
    def test_timeout_zero_means_unlimited(self, monkeypatch):
        """BULLWHIP_CRON_TIMEOUT=0 yields None (unlimited)."""
        monkeypatch.setenv("BULLWHIP_CRON_TIMEOUT", "0")
        _cron_timeout = float(os.getenv("BULLWHIP_CRON_TIMEOUT", 600))
        _cron_inactivity_limit = _cron_timeout if _cron_timeout > 0 else None
...
class TestSysPathOrdering:
    ...
    def test_bullwhip_time_importable(self):
        """bullwhip_time should be importable when cron.scheduler loads."""
        # This import would fail if sys.path.insert comes after the import
        from cron.scheduler import _bullwhip_now
        assert callable(_bullwhip_now)

    def test_bullwhip_constants_importable(self):
        """bullwhip_constants should be importable from cron context."""
        from bullwhip_constants import get_bullwhip_home
        assert callable(get_bullwhip_home)
