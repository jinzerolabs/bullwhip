...
def sample_repo(tmp_path: Path) -> Path:
    ...
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.name", "BullWhip Tests")
...
@pytest.mark.asyncio
async def test_blocks_sensitive_home_andbullwhip_paths(tmp_path: Path, monkeypatch):
    from agent.context_references import preprocess_context_references_async

    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("BULLWHIP_HOME", str(tmp_path / ".bullwhip"))

    hermes_env = tmp_path / ".bullwhip" / ".env"
    ...
    result = await preprocess_context_references_async(
        "read @file:.bullwhip/.env and @file:.ssh/id_rsa",
        cwd=tmp_path,
