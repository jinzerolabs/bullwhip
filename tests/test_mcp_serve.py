"""
Tests for mcp_serve — BullWhip MCP server.
...
def _isolate_bullwhip_home(tmp_path, monkeypatch):
    """Redirect BULLWHIP_HOME to a temp directory."""
    monkeypatch.setenv("BULLWHIP_HOME", str(tmp_path))
    try:
        import bullwhip_constants
        monkeypatch.setattr(bullwhip_constants, "get_bullwhip_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking bullwhip_state schema."""
