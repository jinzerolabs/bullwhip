...
class TestReasoningCommand:
    ...
    async def test_reasoning_command_reloads_current_state_from_config(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_run, "_bullwhip_home", bullwhip_home)
    ...
    def test_run_agent_reloads_reasoning_config_per_message(self, tmp_path, monkeypatch):
        bullwhip_home = tmp_path / "bullwhip"
        bullwhip_home.mkdir()
        (bullwhip_home / "config.yaml").write_text("agent:\n  reasoning_effort: low\n", encoding="utf-8")

        monkeypatch.setattr(gateway_run, "_bullwhip_home", bullwhip_home)
        monkeypatch.setattr(gateway_run, "_env_path", bullwhip_home / ".env")
    ...
    def test_run_agent_homeassistant_uses_default_platform_toolset(self, tmp_path, monkeypatch):
        bullwhip_home = tmp_path / "bullwhip"
        bullwhip_home.mkdir()
        (bullwhip_home / "config.yaml").write_text("", encoding="utf-8")
