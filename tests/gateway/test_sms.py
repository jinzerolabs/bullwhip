...
class TestSmsToolset:
    def testbullwhip_sms_toolset_exists(self):
        from toolsets import get_toolset

        ts = get_toolset("bullwhip-sms")
        assert ts is not None
        assert "tools" in ts

    def testbullwhip_sms_in_gateway_includes(self):
        from toolsets import get_toolset

        gw = get_toolset("bullwhip-gateway")
        assert gw is not None
        assert "bullwhip-sms" in gw["includes"]
