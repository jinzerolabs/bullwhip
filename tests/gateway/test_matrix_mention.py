...
class TestIsBotMentioned:
    ...
    def test_full_user_id_in_body(self):
        assert self.adapter._is_bot_mentioned("hey @bullwhip:example.org help")

    def test_localpart_in_body(self):
        assert self.adapter._is_bot_mentioned("bullwhip can you help?")

    def test_localpart_case_insensitive(self):
        assert self.adapter._is_bot_mentioned("BULLWHIP can you help?")

    def test_matrix_pill_in_formatted_body(self):
        html = '<a href="https://matrix.to/#/@bullwhip:example.org">BullWhip</a> help'
        assert self.adapter._is_bot_mentioned("BullWhip help", html)
    ...
    def test_partial_localpart_no_match(self):
        # "hermesbot" should not match word-boundary check for "bullwhip"
    ...
    def test_m_mentions_user_ids_authoritative(self):
        ...
        assert self.adapter._is_bot_mentioned(
            "please reply",  # no @bullwhip anywhere in body
            mention_user_ids=["@bullwhip:example.org"],
        )

    def test_m_mentions_user_ids_with_body_mention(self):
        """Both m.mentions and body mention — should still be True."""
        assert self.adapter._is_bot_mentioned(
            "hey @bullwhip:example.org help",
            mention_user_ids=["@bullwhip:example.org"],
        )
...
class TestStripMention:
    ...
    def test_strip_full_user_id(self):
        result = self.adapter._strip_mention("@bullwhip:example.org help me")
        assert result == "help me"

    def test_strip_localpart(self):
        result = self.adapter._strip_mention("bullwhip help me")
        assert result == "help me"

    def test_strip_returns_empty_for_mention_only(self):
        result = self.adapter._strip_mention("@bullwhip:example.org")
        assert result == ""
...
async def test_require_mention_default_processes_mentioned(monkeypatch):
    ...
    adapter = _make_adapter()
    event = _make_event("@bullwhip:example.org help me")
...
async def test_require_mention_html_pill(monkeypatch):
    ...
    adapter = _make_adapter()
    formatted = '<a href="https://matrix.to/#/@bullwhip:example.org">BullWhip</a> help'
    event = _make_event("BullWhip help", formatted_body=formatted)
...
async def test_bare_mention_passes_empty_string(monkeypatch):
    ...
    adapter = _make_adapter()
    event = _make_event("@bullwhip:example.org")
...
async def test_dm_mention_thread_disabled_by_default(monkeypatch):
    ...
    adapter = _make_adapter()
    _set_dm(adapter)
    event = _make_event("@bullwhip:example.org help me", event_id="$dm1")
...
async def test_dm_mention_thread_preserves_existing_thread(monkeypatch):
    ...
    event = _make_event("@bullwhip:example.org help me", thread_id="$existing_thread")
...
async def test_dm_mention_thread_tracks_participation(monkeypatch):
    ...
    adapter = _make_adapter()
    _set_dm(adapter)
    event = _make_event("@bullwhip:example.org help", event_id="$dm1")
