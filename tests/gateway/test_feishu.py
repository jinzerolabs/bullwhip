...
class TestGatewayIntegration(unittest.TestCase):
    ...
    def test_feishu_toolset_exists(self):
        ...
        self.assertIn("bullwhip-feishu", TOOLSETS)
        self.assertIn("bullwhip-feishu", TOOLSETS["bullwhip-gateway"]["includes"])
...
class TestFeishuAdapterMessaging(unittest.TestCase):
    ...
    def test_get_chat_info_uses_real_feishu_chat_api(self):
        ...
        class _ChatAPI:
            def get(self, request):
                ...
                return SimpleNamespace(
                    success=lambda: True,
                    data=SimpleNamespace(name="BullWhip Group", chat_type="group"),
                )
        ...
        self.assertEqual(info["name"], "BullWhip Group")
...
class TestAdapterBehavior(unittest.TestCase):
    ...
    def test_group_message_matches_bot_name_when_only_name_available(self):
        ...
        adapter = FeishuAdapter(PlatformConfig())
        adapter._bot_name = "BullWhip Bot"
        sender_id = SimpleNamespace(open_id="ou_any", user_id=None)
    ...
    def test_group_post_message_uses_parsed_mentions_when_sdk_mentions_missing(self):
        ...
        message = SimpleNamespace(
            message_type="post",
            mentions=[],
            content='{"en_us":{"content":[[{"tag":"at","user_name":"BullWhip","open_id":"ou_bot"}]]}}',
        )
    ...
    @patch.dict(
        os.environ,
        {
            "BULLWHIP_FEISHU_TEXT_BATCH_MAX_MESSAGES": "2",
        },
        clear=True,
    )
    ...
    def test_dedup_state_persists_across_adapter_restart(self):
        ...
        with tempfile.TemporaryDirectory() as temp_home:
            with patch.dict(os.environ, {"BULLWHIP_HOME": temp_home}, clear=False):
