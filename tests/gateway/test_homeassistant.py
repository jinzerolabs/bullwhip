...
class TestSendViaRestApi:
    ...
    async def test_send_success(self):
        ...
        assert call_args[1]["json"]["title"] == "Bull Whip Agent"
...
class TestToolsetIntegration:
    ...
    def test_gateway_toolset_includes_ha_tools(self):
        ...
        gateway_tools = resolve_toolset("bullwhip-gateway")
        for tool in ("ha_list_entities", "ha_get_state", "ha_call_service", "ha_list_services"):
            assert tool in gateway_tools

    def testbullwhip_core_tools_includes_ha(self):
        from toolsets import _BULLWHIP_CORE_TOOLS

        for tool in ("ha_list_entities", "ha_get_state", "ha_call_service", "ha_list_services"):
            assert tool in _BULLWHIP_CORE_TOOLS
