...
import bullwhip_cli.gateway as gateway_cli
...
class TestSystemdServiceRefresh:
    def test_systemd_install_repairs_outdated_unit_without_force(self, tmp_path, monkeypatch):
        unit_path = tmp_path / "bullwhip-gateway.service"
...
class TestLaunchdServiceRecovery:
    def test_get_restart_drain_timeout_prefers_env_then_config_then_default(self, monkeypatch):
        monkeypatch.delenv("BULLWHIP_RESTART_DRAIN_TIMEOUT", raising=False)
        ...
        monkeypatch.setenv("BULLWHIP_RESTART_DRAIN_TIMEOUT", "9")
        assert gateway_cli._get_restart_drain_timeout() == 9.0

        monkeypatch.setenv("BULLWHIP_RESTART_DRAIN_TIMEOUT", "invalid")
        assert (
            gateway_cli._get_restart_drain_timeout()
            == DEFAULT_GATEWAY_RESTART_DRAIN_TIMEOUT
        )
    ...
    def test_launchd_install_repairs_outdated_plist_without_force(self, tmp_path, monkeypatch):
        plist_path = tmp_path / "ai.bullwhip.gateway.plist"
...
class TestGatewaySystemServiceRouting:
    ...
    def test_gateway_install_reports_termux_manual_mode(self, monkeypatch, capsys):
        ...
        assert "Run manually: bullwhip gateway" in out
    ...
    def test_gateway_status_on_termux_shows_manual_guidance(self, monkeypatch, capsys):
        ...
        assert "nohup bullwhip gateway" in out
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "kickstart", "-k", "gui/501/ai.bullwhip.gateway"])
            ),
        )
...
class TestSystemUnitBullWhipHome:
    """BULLWHIP_HOME in system units must reference the target user, not root."""
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'BULLWHIP_HOME=/home/alice/.bullwhip' in unit
        assert '/root/.bullwhip' not in unit

    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        # Simulate sudo with a profile: BULLWHIP_HOME was resolved under root
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("BULLWHIP_HOME", "/root/.bullwhip/profiles/coder")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'BULLWHIP_HOME=/home/alice/.bullwhip/profiles/coder' in unit
        assert '/root/' not in unit

    def test_system_unit_preserves_custom_bullwhip_home(self, monkeypatch):
        # Custom BULLWHIP_HOME not under any user's home — keep as-is
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("BULLWHIP_HOME", "/opt/bullwhip-shared")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'BULLWHIP_HOME=/opt/bullwhip-shared' in unit

    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's BULLWHIP_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        bullwhip_home = str(gateway_cli.get_bullwhip_home().resolve())
        assert f'BULLWHIP_HOME={bullwhip_home}' in unit


class TestBullWhipHomeForTargetUser:
    """Unit tests for _bullwhip_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("BULLWHIP_HOME", raising=False)

        result = gateway_cli._bullwhip_home_for_target_user("/home/alice")
        assert result == "/home/alice/.bullwhip"
    ...
    def test_remaps_profile_path(self, monkeypatch):
        ...
        result = gateway_cli._bullwhip_home_for_target_user("/home/alice")
        assert result == "/home/alice/.bullwhip/profiles/coder"

    def test_keeps_custom_path(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("BULLWHIP_HOME", "/opt/bullwhip")

        result = gateway_cli._bullwhip_home_for_target_user("/home/alice")
        assert result == "/opt/bullwhip"
...
class TestProfileArg:
    ...
    def test_default_bullwhip_home_returns_empty(self, tmp_path, monkeypatch):
        """Default ~/.bullwhip should not produce a --profile flag."""
        bullwhip_home = tmp_path / ".bullwhip"
        bullwhip_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("BULLWHIP_HOME", str(bullwhip_home))
        result = gateway_cli._profile_arg(str(bullwhip_home))
        assert result == ""

    def test_named_profile_returns_flag(self, tmp_path, monkeypatch):
        """~/.bullwhip/profiles/mybot should return '--profile mybot'."""
        profile_dir = tmp_path / ".bullwhip" / "profiles" / "mybot"
        profile_dir.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("BULLWHIP_HOME", str(tmp_path / ".bullwhip"))
    ...
    def test_hash_path_returns_empty(self, tmp_path, monkeypatch):
        """Arbitrary non-profile BULLWHIP_HOME should return empty string."""
        custom_home = tmp_path / "custom" / "bullwhip"
    ...
    def test_nested_profile_path_returns_empty(self, tmp_path, monkeypatch):
        """~/.bullwhip/profiles/mybot/subdir should NOT match — too deep."""
        nested = tmp_path / ".bullwhip" / "profiles" / "mybot" / "subdir"
    ...
    def test_invalid_profile_name_returns_empty(self, tmp_path, monkeypatch):
        ...
        bad_profile = tmp_path / ".bullwhip" / "profiles" / "My Bot!"
    ...
    def test_systemd_unit_includes_profile(self, tmp_path, monkeypatch):
        ...
        profile_dir.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("BULLWHIP_HOME", str(profile_dir))
        monkeypatch.setattr(gateway_cli, "get_bullwhip_home", lambda: profile_dir)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".bullwhip" / "profiles" / "orcha"
        profile_dir.mkdir(parents=True)
        ...
        assert plist_path == machine_home / "Library" / "LaunchAgents" / "ai.bullwhip.gateway-orcha.plist"
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".bullwhip" / "bullwhip-agent"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".bullwhip" / "bullwhip-agent")

    def test_keeps_system_path_unchanged(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "root")
        (tmp_path / "root").mkdir()
        result = gateway_cli._remap_path_for_user("/opt/bullwhip", str(tmp_path / "alice"))
        assert result == "/opt/bullwhip"

    def test_noop_when_same_user(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "alice")
        (tmp_path / "alice").mkdir()
        original = str(tmp_path / "alice" / ".bullwhip" / "bullwhip-agent")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        ...
        root_home.mkdir()
        project = root_home / ".bullwhip" / "bullwhip-agent"
        ...
        monkeypatch.setenv("BULLWHIP_HOME", str(root_home / ".bullwhip"))
        monkeypatch.setattr(gateway_cli, "get_bullwhip_home", lambda: root_home / ".bullwhip")
        ...
        assert "WorkingDirectory=/home/alice/.bullwhip/bullwhip-agent" in unit
...
class TestDockerAwareGateway:
    ...
    def test_run_systemctl_raises_runtimeerror_when_missing(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="systemctl is not available"):
            gateway_cli._run_systemctl(["start", "bullwhip-gateway"])
    ...
    def test_run_systemctl_passes_through_on_success(self, monkeypatch):
        ...
        result = gateway_cli._run_systemctl(["status", "bullwhip-gateway"])
        assert result.returncode == 0
        assert len(calls) == 1
        assert "status" in calls[0]

    def test_install_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'bullwhip gateway install' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_uninstall_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'bullwhip gateway uninstall' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_start_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'bullwhip gateway start' inside Docker exits 0 with container guidance."""
        import pytest
        ...
        out = capsys.readouterr().out
        assert "docker" in out.lower()
        assert "bullwhip gateway run" in out
