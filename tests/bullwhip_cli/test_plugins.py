"""Tests for the Bull Whip plugin system (bullwhip_cli.plugins)."""
...
from bullwhip_cli.plugins import (
    ENTRY_POINTS_GROUP,
    VALID_HOOKS,
    LoadedPlugin,
    PluginContext,
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.bullwhip/plugins/ are discovered."""
        plugins_dir = tmp_path / "hermes_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("BULLWHIP_HOME", str(tmp_path / "hermes_test"))
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("BULLWHIP_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".bullwhip" / "plugins"
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="bullwhip_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPreToolCallBlocking:
    ...
    def test_block_message_returned_for_valid_directive(self, monkeypatch):
        monkeypatch.setattr(
            "bullwhip_cli.plugins.invoke_hook",
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        import bullwhip_cli.plugins as plugins_mod
...
# in PluginContext (bullwhip_cli/plugins.py).  The tests referenced _plugin_commands,
