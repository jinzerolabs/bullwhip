"""Tests for bullwhip_cli.tools_config platform tool persistence."""
...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    Regression test for https://github.com/NousResearch/bullwhip-agent/issues/1247
    """
    config = {
        "platform_toolsets": {
    ...
    with patch("bullwhip_cli.tools_config.save_config"):
...
def test_save_platform_tools_does_not_preserve_platform_default_toolsets():
    """Platform default toolsets (bullwhip-cli, bullwhip-telegram, etc.) must NOT
    ...
    ``bullwhip tools``, but bullwhip-cli stays in the config and re-enables
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                ...
                "delegation", "file", "bullwhip-cli",  # <-- the culprit
    ...
    # bullwhip-cli must NOT survive — it's a platform default, not an MCP server
    assert "bullwhip-cli" not in saved
...
def test_save_platform_tools_does_not_preservebullwhip_telegram():
    """Same bug for Telegram — bullwhip-telegram must not be preserved."""
    config = {
        "platform_toolsets": {
            "telegram": [
                "browser", "file", "bullwhip-telegram", "terminal", "web",
            ]
        }
    }
    ...
    assert "bullwhip-telegram" not in saved
    assert "web" in saved
...
def test_save_platform_tools_still_preserves_mcp_with_platform_default_present():
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                "web", "terminal", "bullwhip-cli", "my-mcp-server", "github-tools",
            ]
        }
    }
...
def test_visible_providers_include_nous_subscription_when_logged_in(monkeypatch):
    ...
    monkeypatch.setattr(
        "bullwhip_cli.nous_subscription.get_nous_auth_status",
...
def test_local_browser_provider_is_saved_explicitly(monkeypatch):
    ...
    monkeypatch.setattr("bullwhip_cli.tools_config._run_post_setup", lambda key: None)
...
def test_first_install_nous_auto_configures_managed_defaults(monkeypatch):
    ...
    monkeypatch.setattr(
        "bullwhip_cli.tools_config._prompt_toolset_checklist",
        lambda *args, **kwargs: {"web", "image_gen", "tts", "browser"},
    )
    monkeypatch.setattr("bullwhip_cli.tools_config.save_config", lambda config: None)
    ...
    monkeypatch.setattr(
        "bullwhip_cli.tools_config._get_enabled_platforms",
        lambda: ["cli"],
    )
    monkeypatch.setattr(
        "bullwhip_cli.nous_subscription.get_nous_auth_status",
    ...
    configured = []
    monkeypatch.setattr(
        "bullwhip_cli.tools_config._configure_toolset",
...
class TestPlatformToolsetConsistency:
    ...
    def test_all_platforms_have_toolset_definitions(self):
        ...
        from bullwhip_cli.tools_config import PLATFORMS
    ...
    def test_gateway_toolset_includes_all_messaging_platforms(self):
        """bullwhip-gateway includes list should cover all messaging platforms."""
        from bullwhip_cli.tools_config import PLATFORMS
        from toolsets import TOOLSETS

        gateway_includes = set(TOOLSETS["bullwhip-gateway"]["includes"])
    ...
    def test_skills_config_covers_tools_config_platforms(self):
        ...
        from bullwhip_cli.tools_config import PLATFORMS as TOOLS_PLATFORMS
        from bullwhip_cli.skills_config import PLATFORMS as SKILLS_PLATFORMS
...
def test_numeric_mcp_server_name_does_not_crash_sorted():
    ...
    Regression test for https://github.com/NousResearch/bullwhip-agent/issues/6901
    """
    config = {
