"""Tests for the top-level `./bullwhip` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./bullwhip` should use `bullwhip_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "bullwhip"
    called = []

    fake_main_module = types.ModuleType("bullwhip_cli.main")

    def fake_main():
        called.append("bullwhip_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "bullwhip_cli.main", fake_main_module)
    ...
    assert called == ["bullwhip_cli.main"]
