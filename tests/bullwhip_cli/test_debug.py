"""Tests for ``bullwhip debug`` CLI command and debug utilities."""
...
def bullwhip_home(tmp_path, monkeypatch):
    """Set up an isolated BULLWHIP_HOME with minimal logs."""
...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from bullwhip_cli.debug import _upload_paste_rs
        ...
        with patch("bullwhip_cli.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "bullwhip_cli.debug.urllib.request.urlopen",
...
class TestUploadDpasteCom:
    ...
    def test_upload_dpaste_com_success(self):
        from bullwhip_cli.debug import _upload_dpaste_com
...
class TestUploadToPastebin:
    ...
    def test_tries_paste_rs_first(self):
        from bullwhip_cli.debug import upload_to_pastebin

        with patch("bullwhip_cli.debug._upload_paste_rs",
    ...
    def test_falls_back_to_dpaste_com(self):
        ...
        with patch("bullwhip_cli.debug._upload_paste_rs",
             ...
             patch("bullwhip_cli.debug._upload_dpaste_com",
...
class TestReadFullLog:
    ...
    def test_reads_small_file(self, bullwhip_home):
        from bullwhip_cli.debug import _read_full_log
    ...
    def test_returns_none_for_empty(self, bullwhip_home):
        # Truncate agent.log to empty
        (bullwhip_home / "logs" / "agent.log").write_text("")

        from bullwhip_cli.debug import _read_full_log
        assert _read_full_log("agent") is None

    def test_truncates_large_file(self, bullwhip_home):
        """Files larger than max_bytes get tail-truncated."""
        from bullwhip_cli.debug import _read_full_log

        # Write a file larger than 1KB
        big_content = "x" * 100 + "\n"
        (bullwhip_home / "logs" / "agent.log").write_text(big_content * 200)

        content = _read_full_log("agent", max_bytes=1024)
        assert content is not None
        assert "truncated" in content

    def test_unknown_log_returns_none(self, bullwhip_home):
        from bullwhip_cli.debug import _read_full_log
        assert _read_full_log("nonexistent") is None

    def test_falls_back_to_rotated_file(self, bullwhip_home):
        """When gateway.log doesn't exist, falls back to gateway.log.1."""
        from bullwhip_cli.debug import _read_full_log

        logs_dir = bullwhip_home / "logs"
    ...
    def test_prefers_primary_over_rotated(self, bullwhip_home):
    ...
    def test_falls_back_when_primary_empty(self, bullwhip_home):
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, bullwhip_home):
        from bullwhip_cli.debug import collect_debug_report

        with patch("bullwhip_cli.dump.run_dump") as mock_dump:
            mock_dump.side_effect = lambda args: print(
                "--- bullwhip dump ---\nversion: 0.8.0\n--- end dump ---"
            )
            report = collect_debug_report(log_lines=50)

        assert "--- bullwhip dump ---" in report
        assert "version: 0.8.0" in report

    def test_report_includes_agent_log(self, bullwhip_home):
        from bullwhip_cli.debug import collect_debug_report

        with patch("bullwhip_cli.dump.run_dump"):
            report = collect_debug_report(log_lines=50)

        assert "--- agent.log" in report
        assert "session started" in report

    def test_report_includes_errors_log(self, bullwhip_home):
        from bullwhip_cli.debug import collect_debug_report
    ...
    def test_report_includes_gateway_log(self, bullwhip_home):
        from bullwhip_cli.debug import collect_debug_report
...
class TestRunDebugShare:
    ...
    def test_local_flag_prints_full_logs(self, bullwhip_home, capsys):
    ...
    def test_share_uploads_three_pastes(self, bullwhip_home, capsys):
        ...
        with patch("bullwhip_cli.dump.run_dump") as mock_dump, \
             patch("bullwhip_cli.debug.upload_to_pastebin",
                    side_effect=_mock_upload):
            mock_dump.side_effect = lambda a: print("--- bullwhip dump ---\nversion: test\n--- end dump ---")
            run_debug_share(args)
        ...
        assert "--- bullwhip dump ---" in agent_paste
        assert "--- full agent.log ---" in agent_paste
        gateway_paste = uploaded_content[2]
        assert "--- bullwhip dump ---" in gateway_paste
    ...
    def test_share_continues_on_log_upload_failure(self, bullwhip_home, capsys):
    ...
    def test_share_exits_on_report_upload_failure(self, bullwhip_home, capsys):
...
class TestRunDebug:
    def test_no_subcommand_shows_usage(self, capsys):
        ...
        out = capsys.readouterr().out
        assert "bullwhip debug share" in out

    def test_share_subcommand_routes(self, bullwhip_home):
        from bullwhip_cli.debug import run_debug
...
class TestArgparseIntegration:
    def test_module_imports_clean(self):
        from bullwhip_cli.debug import run_debug, run_debug_share
        assert callable(run_debug)
        assert callable(run_debug_share)

    def test_cmd_debug_dispatches(self):
        from bullwhip_cli.main import cmd_debug
