...
from bullwhip_cli import config as hermes_config
...
def test_stash_local_changes_if_needed_returns_none_when_tree_clean(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(bullwhip_main.subprocess, "run", fake_run)
...
def test_stash_local_changes_if_needed_returns_specific_stash_commit(monkeypatch, tmp_path):
    ...
    def fake_run(cmd, **kwargs):
        calls.append((cmd, kwargs))
        if cmd[-2:] == ["status", "--porcelain"]:
            return SimpleNamespace(stdout=" M bullwhip_cli/main.py\n?? notes.txt\n", returncode=0)
...
def test_restore_stashed_changes_always_resets_on_conflict(monkeypatch, tmp_path, capsys):
    ...
    Leaving conflict markers in source files makes bullwhip unrunnable (SyntaxError).
    ...
    def fake_run(cmd, **kwargs):
        ...
        if cmd[1:3] == ["diff", "--name-only"]:
            return SimpleNamespace(stdout="bullwhip_cli/main.py\n", stderr="", returncode=0)
    ...
    assert "bullwhip_cli/main.py" in out
...
def _setup_update_mocks(monkeypatch, tmp_path):
    ...
    (tmp_path / ".git").mkdir()
    monkeypatch.setattr(bullwhip_main, "PROJECT_ROOT", tmp_path)
    monkeypatch.setattr(bullwhip_main, "_stash_local_changes_if_needed", lambda *a, **kw: None)
    monkeypatch.setattr(bullwhip_main, "_restore_stashed_changes", lambda *a, **kw: True)
    monkeypatch.setattr(bullwhip_config, "get_missing_env_vars", lambda required_only=True: [])
    monkeypatch.setattr(bullwhip_config, "get_missing_config_fields", lambda: [])
    monkeypatch.setattr(bullwhip_config, "check_config_version", lambda: (5, 5))
    monkeypatch.setattr(bullwhip_config, "migrate_config", lambda **kw: {"env_added": [], "config_added": []})
...
def test_cmd_update_retries_optional_extras_individually_when_all_fails(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(bullwhip_main, "_load_installable_optional_extras", lambda: ["matrix", "mcp"])
...
def test_cmd_update_falls_back_to_reset_when_ff_only_fails(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(bullwhip_main.subprocess, "run", side_effect)
