...
from bullwhip_cli.auth import AuthError, get_provider_auth_state, resolve_nous_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        from bullwhip_cli.auth import _resolve_verify
    ...
    def test_missing_ssl_cert_file_env_falls_back(self, monkeypatch):
        ...
        monkeypatch.delenv("BULLWHIP_CA_BUNDLE", raising=False)
        result = _resolve_verify(auth_state={"tls": {}})
        assert result is True

    def test_missingbullwhip_ca_bundle_env_falls_back(self, monkeypatch):
        from bullwhip_cli.auth import _resolve_verify

        monkeypatch.setenv("BULLWHIP_CA_BUNDLE", "/nonexistent/bullwhip-ca.pem")
...
def _setup_nous_auth(
    ...
    *,
    ...
    (bullwhip_home / "auth.json").write_text(json.dumps(auth_store, indent=2))
...
def test_get_nous_auth_status_checks_credential_pool(tmp_path, monkeypatch):
    ...
    """
    from bullwhip_cli.auth import get_nous_auth_status
...
def test_get_nous_auth_status_auth_store_fallback(tmp_path, monkeypatch):
    ...
    bullwhip_home = tmp_path / "bullwhip"
    _setup_nous_auth(bullwhip_home, access_token="at-123")
    monkeypatch.setenv("BULLWHIP_HOME", str(bullwhip_home))
...
def test_refresh_token_persisted_when_mint_returns_insufficient_credits(tmp_path, monkeypatch):
    bullwhip_home = tmp_path / "bullwhip"
    _setup_nous_auth(bullwhip_home, refresh_token="refresh-old")
    monkeypatch.setenv("BULLWHIP_HOME", str(bullwhip_home))
    ...
    monkeypatch.setattr("bullwhip_cli.auth._refresh_access_token", _fake_refresh_access_token)
    monkeypatch.setattr("bullwhip_cli.auth._mint_agent_key", _fake_mint_agent_key)
