"""Tests for Codex auth — tokens stored in BullWhip auth store (~/.bullwhip/auth.json)."""
...
def _setupbullwhip_auth(bullwhip_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Bull Whip auth store."""
    ...
    auth_file = bullwhip_home / "auth.json"
...
def test_read_codex_tokens_success(tmp_path, monkeypatch):
    ...
    _setupbullwhip_auth(bullwhip_home)
...
def test_read_codex_tokens_missing(tmp_path, monkeypatch):
    ...
    bullwhip_home.mkdir(parents=True, exist_ok=True)
    # Empty auth store
    (bullwhip_home / "auth.json").write_text(json.dumps({"version": 1, "providers": {}}))
    monkeypatch.setenv("BULLWHIP_HOME", str(bullwhip_home))
...
def test_resolve_codex_runtime_credentials_missing_access_token(tmp_path, monkeypatch):
    bullwhip_home = tmp_path / "bullwhip"
    _setupbullwhip_auth(bullwhip_home, access_token="")
    monkeypatch.setenv("BULLWHIP_HOME", str(bullwhip_home))
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setupbullwhip_auth(bullwhip_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("BULLWHIP_HOME", str(bullwhip_home))
    ...
    monkeypatch.setattr("bullwhip_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    bullwhip_home = tmp_path / "bullwhip"
    _setupbullwhip_auth(bullwhip_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("BULLWHIP_HOME", str(bullwhip_home))
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Bull Whip auth store, not ~/.codex/."""
    bullwhip_home = tmp_path / "bullwhip"
    codex_home = tmp_path / "codex-cli"
    bullwhip_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "bullwhip-at", "refresh_token": "bullwhip-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches BullWhip store
    assert not (codex_home / "auth.json").exists()

    # Bull Whip auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "bullwhip-at"
...
def test_refresh_codex_auth_tokens_writes_back_to_cli(tmp_path, monkeypatch):
    ...
    from bullwhip_cli.auth import _refresh_codex_auth_tokens
    ...
    monkeypatch.setattr("bullwhip_cli.auth.refresh_codex_oauth_pure", lambda *a, **kw: {
...
def test_resolve_returnsbullwhip_auth_store_source(tmp_path, monkeypatch):
    bullwhip_home = tmp_path / "bullwhip"
    _setupbullwhip_auth(bullwhip_home)
    monkeypatch.setenv("BULLWHIP_HOME", str(bullwhip_home))

    creds = resolve_codex_runtime_credentials()
    assert creds["source"] == "bullwhip-auth-store"
