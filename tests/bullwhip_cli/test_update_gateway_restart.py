...
Ensures ``bullwhip update`` correctly detects running gateways managed by
...
def _make_run_side_effect(
    ...
    def side_effect(cmd, **kwargs):
        ...
        # systemctl list-units bullwhip-gateway* — discover all gateway services
        if "systemctl" in joined and "list-units" in joined:
            if "--user" in joined and systemd_active:
                return subprocess.CompletedProcess(
                    cmd, 0,
                    stdout="bullwhip-gateway.service loaded active running BullWhip Gateway\n",
                    stderr="",
                )
...
class TestLaunchdPlistPath:
    def test_plist_contains_environment_variables(self):
        ...
        assert "<key>BULLWHIP_HOME</key>" in plist
...
class TestCmdUpdateLaunchdRestart:
    ...
    def test_update_detects_launchd_and_skips_manual_restart_message(
        ...
        'auto-restart via launchd' instead of 'Restart it with: bullwhip gateway run'."""
        ...
        assert "Restart manually: bullwhip gateway run" not in captured
    ...
    def test_update_without_launchd_shows_manual_restart(
        ...
        captured = capsys.readouterr().out
        assert "Restart manually: bullwhip gateway run" in captured
    ...
    def test_update_with_systemd_still_restarts_via_systemd(
        ...
        captured = capsys.readouterr().out
        assert "Restarted bullwhip-gateway" in captured
...
class TestServicePidExclusion:
    ...
    where ``bullwhip update`` would restart the gateway and immediately kill it.
    """
...
class TestGetServicePids:
    ...
    def test_returns_launchd_pid(self, monkeypatch):
        monkeypatch.setattr(gateway_cli, "is_linux", lambda: False)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: True)
        monkeypatch.setattr(gateway_cli, "get_launchd_label", lambda: "ai.bullwhip.gateway")
    ...
    def test_excludes_zero_pid(self, monkeypatch):
        ...
        def fake_run(cmd, **kwargs):
            ...
            if "list-units" in joined:
                return subprocess.CompletedProcess(
                    cmd, 0,
                    stdout="bullwhip-gateway.service loaded inactive dead BullWhip Gateway\n",
                    stderr="",
                )
            if "show" in joined and "MainPID" in joined:
    ...
    def test_filters_to_current_profile(self, monkeypatch, tmp_path):
        ...
        def fake_run(cmd, **kwargs):
            return subprocess.CompletedProcess(
                cmd, 0,
                stdout=(
                    "100 /Users/dgrieco/.bullwhip/bullwhip-agent/venv/bin/python -m bullwhip_cli.main --profile orcha gateway run --replace\n"
                    "200 /Users/dgrieco/.bullwhip/bullwhip-agent/venv/bin/python -m bullwhip_cli.main --profile other gateway run --replace\n"
                ),
                stderr="",
            )
        ...
        monkeypatch.setattr(gateway_cli, "_profile_arg", lambda bullwhip_home=None: "--profile orcha")
...
class TestGatewayModeWritesExitCodeEarly:
    """When running as ``bullwhip update --gateway``, the exit code marker must be
    ...
    def test_exit_code_written_in_gateway_mode(
        ...
        # Point BULLWHIP_HOME at a temp dir so the marker file lands there
        bullwhip_home = tmp_path / ".bullwhip"
        bullwhip_home.mkdir()
        monkeypatch.setenv("BULLWHIP_HOME", str(bullwhip_home))
        import bullwhip_cli.config as _cfg
        monkeypatch.setattr(_cfg, "get_bullwhip_home", lambda: bullwhip_home)
        # Also patch the module-level ref used by cmd_update
        import bullwhip_cli.main as _main_mod
        monkeypatch.setattr(_main_mod, "get_bullwhip_home", lambda: bullwhip_home)
