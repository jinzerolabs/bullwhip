...
and haven't been migrated to the Bull Whip auth store yet.

Root cause: list_authenticated_providers() checked the raw BullWhip auth
...
BullWhip auth store has no openai-codex tokens, and
...
def codex_cli_only_env(tmp_path, monkeypatch):
    ...
    NOT in the Bull Whip auth store."""
    ...
    # Empty BullWhip auth store
    (bullwhip_home / "auth.json").write_text(
...
def test_codex_cli_tokens_detected_by_model_picker(codex_cli_only_env):
    ...
    from bullwhip_cli.model_switch import list_authenticated_providers
...
def test_codex_cli_tokens_migrated_after_detection(codex_cli_only_env):
    ...
    migrated into the Bull Whip auth store for subsequent fast lookups."""
    from bullwhip_cli.model_switch import list_authenticated_providers

    # First call triggers migration
    list_authenticated_providers(current_provider="openai-codex")

    # Verify tokens are now in BullWhip auth store
    auth_path = codex_cli_only_env / "auth.json"
    store = json.loads(auth_path.read_text())
    providers = store.get("providers", {})
    assert "openai-codex" in providers, (
        f"openai-codex not migrated to Bull Whip auth store: {list(providers.keys())}"
    )
...
def hermes_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in BullWhip auth store (no Codex CLI needed)."""
    bullwhip_home = tmp_path / ".bullwhip"
    bullwhip_home.mkdir()
...
def test_normal_path_still_works(bullwhip_auth_only_env):
    """openai-codex appears when tokens are already in BullWhip auth store."""
    from bullwhip_cli.model_switch import list_authenticated_providers
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or BullWhip
    auth store."""
    bullwhip_home = tmp_path / ".bullwhip"
    bullwhip_home.mkdir()
