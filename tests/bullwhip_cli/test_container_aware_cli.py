...
def container_env(tmp_path, monkeypatch):
    """Set up a fake BULLWHIP_HOME with .container-mode file."""
    bullwhip_home = tmp_path / ".bullwhip"
    bullwhip_home.mkdir()
    monkeypatch.setenv("BULLWHIP_HOME", str(bullwhip_home))
    monkeypatch.delenv("BULLWHIP_DEV", raising=False)

    container_mode = bullwhip_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=bullwhip-agent\n"
        "exec_user=bullwhip\n"
        "bullwhip_bin=/data/current-package/bin/bullwhip\n"
    )
    return bullwhip_home


def test_get_container_exec_info_returns_metadata(container_env):
    """Reads .container-mode and returns all fields including exec_user."""
    with patch("bullwhip_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "bullwhip-agent"
    assert info["exec_user"] == "bullwhip"
    assert info["bullwhip_bin"] == "/data/current-package/bin/bullwhip"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("bullwhip_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_whenbullwhip_dev(container_env, monkeypatch):
    """Returns None when BULLWHIP_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("BULLWHIP_DEV", "1")
...
def test_get_container_exec_info_not_skipped_whenbullwhip_dev_zero(container_env, monkeypatch):
    """BULLWHIP_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("BULLWHIP_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        bullwhip_home = Path(tmpdir) / ".bullwhip"
        bullwhip_home.mkdir()
        (bullwhip_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("bullwhip_constants.is_container", return_value=False), \
             patch("bullwhip_cli.config.get_bullwhip_home", return_value=bullwhip_home), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("BULLWHIP_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=bullwhip-custom\n"
        "exec_user=myuser\n"
        "bullwhip_bin=/opt/bullwhip/bin/bullwhip\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "bullwhip-custom"
    assert info["exec_user"] == "myuser"
    assert info["bullwhip_bin"] == "/opt/bullwhip/bin/bullwhip"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from bullwhip_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "bullwhip"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "bullwhip-agent" in cmd
    assert "/data/current-package/bin/bullwhip" in cmd
    assert "chat" in cmd
