...
BULLWHIP_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot") while
_PROVIDER_MODELS and config.yaml use BullWhip IDs ("copilot").  The slug
...
def test_copilot_usesbullwhip_slug():
...
def test_kimi_for_coding_alias():
    ...
    from bullwhip_cli.auth import resolve_provider
...
def test_kimi_for_coding_overlay_usesbullwhip_slug():
...
def test_kilo_overlay_usesbullwhip_slug():
