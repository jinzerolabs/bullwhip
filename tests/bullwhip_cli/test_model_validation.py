"""Tests for provider-aware `/model` validation in bullwhip_cli.models."""
...
def _validate(model, provider="openrouter", api_models=FAKE_API_MODELS, **kw):
    ...
    with patch("bullwhip_cli.models.fetch_api_models", return_value=api_models), \
         patch("bullwhip_cli.models.probe_api_models", return_value=probe_payload):
...
class TestParseModelInput:
    ...
    def test_nous_provider_switch(self):
        provider, model = parse_model_input("nous:bullwhip-3", "openrouter")
        assert provider == "nous"
        assert model == "bullwhip-3"
...
class TestCuratedModelsForProvider:
    def test_openrouter_returns_curated_list(self):
        with patch(
            "bullwhip_cli.models.fetch_openrouter_models",
            return_value=[
...
class TestProviderModelIds:
    ...
    def test_copilot_prefers_live_catalog(self):
        with patch("bullwhip_cli.auth.resolve_api_key_provider_credentials", return_value={"api_key": "gh-token"}), \
             patch("bullwhip_cli.models._fetch_github_models", return_value=["gpt-5.4", "claude-sonnet-4.6"]):
    ...
    def test_copilot_acp_falls_back_to_copilot_defaults(self):
        with patch("bullwhip_cli.auth.resolve_api_key_provider_credentials", side_effect=Exception("no token")), \
             patch("bullwhip_cli.models._fetch_github_models", return_value=None):
...
class TestFetchApiModels:
    ...
    def test_returns_none_on_network_error(self):
        with patch("bullwhip_cli.models.urllib.request.urlopen", side_effect=Exception("timeout")):
    ...
    def test_probe_api_models_tries_v1_fallback(self):
        ...
        with patch("bullwhip_cli.models.urllib.request.urlopen", side_effect=_fake_urlopen):
    ...
    def test_probe_api_models_uses_copilot_catalog(self):
        ...
        with patch("bullwhip_cli.models.urllib.request.urlopen", return_value=_Resp()) as mock_urlopen:
    ...
    def test_fetch_github_model_catalog_filters_non_chat_models(self):
        ...
        with patch("bullwhip_cli.models.urllib.request.urlopen", return_value=_Resp()):
...
class TestValidateCodexAutoCorrection:
    ...
    def test_missing_dash_auto_corrects(self):
        ...
        with patch("bullwhip_cli.models.provider_model_ids", return_value=codex_models):
