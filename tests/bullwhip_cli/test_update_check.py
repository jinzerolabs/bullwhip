"""Tests for the update check mechanism in bullwhip_cli.banner."""
...
def test_version_string_no_v_prefix():
    ...
    from bullwhip_cli import __version__
...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    ...
    repo_dir = tmp_path / "bullwhip-agent"
    repo_dir.mkdir()
    (repo_dir / ".git").mkdir()
    ...
    with patch("bullwhip_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("BULLWHIP_HOME", str(tmp_path))
    with patch("bullwhip_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
        result = check_for_updates()
...
def test_check_for_updates_no_git_dir(tmp_path, monkeypatch):
    ...
    import bullwhip_cli.banner as banner

    # Create a fake banner.py so the fallback path also has no .git
    fake_banner = tmp_path / "bullwhip_cli" / "banner.py"
...
def test_check_for_updates_fallback_to_project_root(tmp_path, monkeypatch):
    """Dev install: falls back to Path(__file__).parent.parent when BULLWHIP_HOME has no git repo."""
    import bullwhip_cli.banner as banner
    ...
    # Point BULLWHIP_HOME at a temp dir with no bullwhip-agent/.git
    monkeypatch.setenv("BULLWHIP_HOME", str(tmp_path))
    with patch("bullwhip_cli.banner.subprocess.run") as mock_run:
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from bullwhip_cli.main import _invalidate_update_cache

    # Build a fake ~/.bullwhip with default + two named profiles
    ...
    with patch.object(Path, "home", return_value=tmp_path), \
         patch.dict(os.environ, {"BULLWHIP_HOME": str(default_home)}):
