"""Tests for bullwhip_cli.gateway."""
...
def test_systemd_install_checks_linger_status(monkeypatch, tmp_path, capsys):
    unit_path = tmp_path / "systemd" / "user" / "bullwhip-gateway.service"
...
def test_systemd_install_system_scope_skips_linger_and_uses_systemctl(monkeypatch, tmp_path, capsys):
    unit_path = tmp_path / "etc" / "systemd" / "system" / "bullwhip-gateway.service"
...
def test_conflicting_systemd_units_warning(monkeypatch, tmp_path, capsys):
    user_unit = tmp_path / "user" / "bullwhip-gateway.service"
    system_unit = tmp_path / "system" / "bullwhip-gateway.service"
    ...
    assert "bullwhip gateway uninstall" in out
...
def test_install_linux_gateway_from_setup_system_choice_without_root_prints_followup(monkeypatch, capsys):
    ...
    assert "sudo bullwhip gateway install --system --run-as-user alice" in out
    assert "sudo bullwhip gateway start --system" in out
