"""Tests for bullwhip backup and import commands."""
...
def _makebullwhip_tree(root: Path) -> None:
    """Create a realistic ~/.bullwhip directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    (root / "memory_store.db").write_bytes(b"fake-sqlite")
    (root / "bullwhip_state.db").write_bytes(b"fake-state")
    ...
    # bullwhip-agent repo (should be EXCLUDED)
    (root / "bullwhip-agent").mkdir(exist_ok=True)
    (root / "bullwhip-agent" / "run_agent.py").write_text("# big file\n")
    (root / "bullwhip-agent" / ".git").mkdir()
    (root / "bullwhip-agent" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_bullwhip_agent(self):
        from bullwhip_cli.backup import _should_exclude
        assert _should_exclude(Path("bullwhip-agent/run_agent.py"))
        assert _should_exclude(Path("bullwhip-agent/.git/HEAD"))
...
class TestBackup:
    def test_creates_zip(self, tmp_path, monkeypatch):
        ...
        bullwhip_home.mkdir()
        _makebullwhip_tree(bullwhip_home)

        monkeypatch.setenv("BULLWHIP_HOME", str(bullwhip_home))
        # get_defaultbullwhip_root needs this
    ...
    def test_excludes_bullwhip_agent(self, tmp_path, monkeypatch):
        """Backup does NOT include bullwhip-agent/ directory."""
        bullwhip_home = tmp_path / ".bullwhip"
        bullwhip_home.mkdir()
        _makebullwhip_tree(bullwhip_home)
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            agent_files = [n for n in names if "bullwhip-agent" in n]
            assert agent_files == [], f"bullwhip-agent files leaked into backup: {agent_files}"
    ...
    def test_default_output_path(self, tmp_path, monkeypatch):
        """When no output path given, zip goes to ~/bullwhip-backup-*.zip."""
        bullwhip_home = tmp_path / ".bullwhip"
        bullwhip_home.mkdir()
        (bullwhip_home / "config.yaml").write_text("model: test\n")
        ...
        zips = list(tmp_path.glob("bullwhip-backup-*.zip"))
        assert len(zips) == 1
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid BullWhip backup."""
        from bullwhip_cli.backup import _validate_backup_zip
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only bullwhip_state.db (old wrong name) is rejected."""
        from bullwhip_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["bullwhip_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestImport:
    ...
    def test_restores_files(self, tmp_path, monkeypatch):
        """Import extracts files into bullwhip home."""
        bullwhip_home = tmp_path / ".bullwhip"
        bullwhip_home.mkdir()
        monkeypatch.setenv("BULLWHIP_HOME", str(bullwhip_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (bullwhip_home / "config.yaml").read_text() == "model:\n  provider: openrouter\n"
        assert (bullwhip_home / ".env").read_text() == "OPENROUTER_API_KEY=sk-test\n"
        assert (bullwhip_home / "skills" / "my-skill" / "SKILL.md").read_text() == "# My Skill\n"
        assert (bullwhip_home / "profiles" / "coder" / "config.yaml").exists()

    def test_stripsbullwhip_prefix(self, tmp_path, monkeypatch):
        """Import strips .bullwhip/ prefix if all entries share it."""
        bullwhip_home = tmp_path / ".bullwhip"
        bullwhip_home.mkdir()
        monkeypatch.setenv("BULLWHIP_HOME", str(bullwhip_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        zip_path = tmp_path / "backup.zip"
        self._make_backup_zip(zip_path, {
            ".bullwhip/config.yaml": "model: test\n",
            ".bullwhip/skills/a/SKILL.md": "# A\n",
        })
        ...
        assert (bullwhip_home / "config.yaml").read_text() == "model: test\n"
        assert (bullwhip_home / "skills" / "a" / "SKILL.md").read_text() == "# A\n"
    ...
    def test_rejects_nonbullwhip_zip(self, tmp_path, monkeypatch):
        """Import rejects a zip that doesn't look like a bullwhip backup."""
        bullwhip_home = tmp_path / ".bullwhip"
        bullwhip_home.mkdir()
        monkeypatch.setenv("BULLWHIP_HOME", str(bullwhip_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_blocks_path_traversal(self, tmp_path, monkeypatch):
        ...
        assert (bullwhip_home / "config.yaml").exists()
        # traversal file should NOT exist outside bullwhip home
    ...
    def test_confirmation_prompt_abort(self, tmp_path, monkeypatch):
        ...
        (bullwhip_home / "config.yaml").write_text("existing: true\n")
        monkeypatch.setenv("BULLWHIP_HOME", str(bullwhip_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (bullwhip_home / "config.yaml").read_text() == "existing: true\n"
    ...
    def test_force_skips_confirmation(self, tmp_path, monkeypatch):
        ...
        assert (bullwhip_home / "config.yaml").read_text() == "model: restored\n"
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".bullwhip"
        src_home.mkdir(parents=True)
        _makebullwhip_tree(src_home)

        monkeypatch.setenv("BULLWHIP_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from bullwhip_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".bullwhip"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("BULLWHIP_HOME", str(dst_home))
        ...
        # bullwhip-agent should NOT be present
        assert not (dst_home / "bullwhip-agent").exists()
...
class TestFormatSize:
    def test_bytes(self):
        from bullwhip_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_validate_rejects_random(self):
        """Zip without bullwhip markers fails validation."""
        import io
        from bullwhip_cli.backup import _validate_backup_zip
    ...
    def test_detect_prefix_hermes(self):
        """Detects .bullwhip/ prefix wrapping all entries."""
        import io
        from bullwhip_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            zf.writestr(".bullwhip/config.yaml", "test")
            zf.writestr(".bullwhip/skills/a/SKILL.md", "skill")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ".bullwhip/"
    ...
    def test_detect_prefix_only_dirs(self):
        ...
        with zipfile.ZipFile(buf, "w") as zf:
            ...
            zf.writestr(".bullwhip/", "")
            zf.writestr(".bullwhip/skills/", "")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ""
...
class TestBackupEdgeCases:
    def test_nonexistent_bullwhip_home(self, tmp_path, monkeypatch):
        """Backup exits when bullwhip home doesn't exist."""
        fake_home = tmp_path / "nonexistent" / ".bullwhip"
        monkeypatch.setenv("BULLWHIP_HOME", str(fake_home))
    ...
    def test_output_is_directory(self, tmp_path, monkeypatch):
        ...
        zips = list(out_dir.glob("bullwhip-backup-*.zip"))
        assert len(zips) == 1
    ...
    def test_empty_bullwhip_home(self, tmp_path, monkeypatch):
        """Backup handles empty bullwhip home (no files to back up)."""
        bullwhip_home = tmp_path / ".bullwhip"
        bullwhip_home.mkdir()
        # Only excluded dirs, no actual files
        (bullwhip_home / "__pycache__").mkdir()
        (bullwhip_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_permission_error_during_backup(self, tmp_path, monkeypatch):
        ...
        bad_file = bullwhip_home / "secret.db"
    ...
    def test_skips_output_zip_inside_hermes(self, tmp_path, monkeypatch):
        """Backup skips its own output zip if it's inside bullwhip root."""
        bullwhip_home = tmp_path / ".bullwhip"
        bullwhip_home.mkdir()
        (bullwhip_home / "config.yaml").write_text("model: test\n")
        ...
        # Output inside bullwhip home
        out_zip = bullwhip_home / "backup.zip"
        args = Namespace(output=str(out_zip))
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        bullwhip_home = tmp_path / ".bullwhip"
        bullwhip_home.mkdir()
        (bullwhip_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("BULLWHIP_HOME", str(bullwhip_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_keyboard_interrupt_during_confirmation(self, tmp_path, monkeypatch):
        ...
        bullwhip_home = tmp_path / ".bullwhip"
        bullwhip_home.mkdir()
        (bullwhip_home / ".env").write_text("KEY=val\n")
        monkeypatch.setenv("BULLWHIP_HOME", str(bullwhip_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_permission_error_during_import(self, tmp_path, monkeypatch):
        ...
        locked_dir = bullwhip_home / "locked"
        locked_dir.mkdir()
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (bullwhip_home / "config.yaml").exists()
        assert (bullwhip_home / "sessions" / "s0599.json").exists()
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert (bullwhip_home / "profiles" / "coder" / "config.yaml").exists()
        assert (bullwhip_home / "profiles" / "researcher" / "config.yaml").exists()
        ...
        assert "bullwhip -p coder" in coder_wrapper
    ...
    def test_import_without_profiles_module(self, tmp_path, monkeypatch):
        ...
        import bullwhip_cli.backup as backup_mod
        original_import = __builtins__.__import__ if hasattr(__builtins__, '__import__') else __import__

        def fake_import(name, *a, **kw):
            if name == "bullwhip_cli.profiles":
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from bullwhip_cli.backup import _safe_copy_db
...
class TestQuickSnapshot:
    @pytest.fixture
    def bullwhip_home(self, tmp_path):
        """Create a fake BULLWHIP_HOME with critical state files."""
        home = tmp_path / ".bullwhip"
        home.mkdir()
    ...
    def test_creates_snapshot(self, bullwhip_home):
        from bullwhip_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(bullwhip_home=bullwhip_home)
        assert snap_id is not None
        snap_dir = bullwhip_home / "state-snapshots" / snap_id
        assert snap_dir.is_dir()
        assert (snap_dir / "manifest.json").exists()

    def test_label_in_id(self, bullwhip_home):
        from bullwhip_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(label="before-upgrade", bullwhip_home=bullwhip_home)
        assert "before-upgrade" in snap_id

    def test_state_db_safely_copied(self, bullwhip_home):
        from bullwhip_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(bullwhip_home=bullwhip_home)
        db_copy = bullwhip_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_copies_nested_files(self, bullwhip_home):
        from bullwhip_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(bullwhip_home=bullwhip_home)
        assert (bullwhip_home / "state-snapshots" / snap_id / "cron" / "jobs.json").exists()

    def test_missing_files_skipped(self, bullwhip_home):
        from bullwhip_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(bullwhip_home=bullwhip_home)
        with open(bullwhip_home / "state-snapshots" / snap_id / "manifest.json") as f:
            meta = json.load(f)
    ...
    def test_empty_home_returns_none(self, tmp_path):
        ...
        empty.mkdir()
        assert create_quick_snapshot(bullwhip_home=empty) is None

    def test_list_snapshots(self, bullwhip_home):
        from bullwhip_cli.backup import create_quick_snapshot, list_quick_snapshots
        id1 = create_quick_snapshot(label="first", bullwhip_home=bullwhip_home)
        id2 = create_quick_snapshot(label="second", bullwhip_home=bullwhip_home)

        snaps = list_quick_snapshots(bullwhip_home=bullwhip_home)
        assert len(snaps) == 2
        assert snaps[0]["id"] == id2  # most recent first
        assert snaps[1]["id"] == id1

    def test_list_limit(self, bullwhip_home):
        from bullwhip_cli.backup import create_quick_snapshot, list_quick_snapshots
        for i in range(5):
            create_quick_snapshot(label=f"s{i}", bullwhip_home=bullwhip_home)
        snaps = list_quick_snapshots(limit=3, bullwhip_home=bullwhip_home)
        assert len(snaps) == 3

    def test_restore_config(self, bullwhip_home):
        from bullwhip_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(bullwhip_home=bullwhip_home)

        (bullwhip_home / "config.yaml").write_text("model:\n  provider: anthropic\n")
        assert "anthropic" in (bullwhip_home / "config.yaml").read_text()

        result = restore_quick_snapshot(snap_id, bullwhip_home=bullwhip_home)
        assert result is True
        assert "openrouter" in (bullwhip_home / "config.yaml").read_text()

    def test_restore_state_db(self, bullwhip_home):
        from bullwhip_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(bullwhip_home=bullwhip_home)

        conn = sqlite3.connect(str(bullwhip_home / "state.db"))
        conn.execute("INSERT INTO sessions VALUES ('s2', 'new')")
        conn.commit()
        conn.close()

        restore_quick_snapshot(snap_id, bullwhip_home=bullwhip_home)
    ...
    def test_restore_nonexistent(self, bullwhip_home):
        from bullwhip_cli.backup import restore_quick_snapshot
        assert restore_quick_snapshot("nonexistent", bullwhip_home=bullwhip_home) is False

    def test_auto_prune(self, bullwhip_home):
        from bullwhip_cli.backup import create_quick_snapshot, list_quick_snapshots, _QUICK_DEFAULT_KEEP
        for i in range(_QUICK_DEFAULT_KEEP + 5):
            create_quick_snapshot(label=f"snap-{i:03d}", bullwhip_home=bullwhip_home)
        snaps = list_quick_snapshots(limit=100, bullwhip_home=bullwhip_home)
        assert len(snaps) <= _QUICK_DEFAULT_KEEP

    def test_manual_prune(self, bullwhip_home):
        from bullwhip_cli.backup import create_quick_snapshot, prune_quick_snapshots, list_quick_snapshots
        for i in range(10):
            create_quick_snapshot(label=f"s{i}", bullwhip_home=bullwhip_home)
        deleted = prune_quick_snapshots(keep=3, bullwhip_home=bullwhip_home)
        assert deleted == 7
        assert len(list_quick_snapshots(bullwhip_home=bullwhip_home)) == 3
