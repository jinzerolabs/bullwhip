...
The terminal `bullwhip model` flow already exposes `custom_providers`, but the
shared slash-command pipeline (`/model` in CLI/gateway/Telegram) historically
only looked at `providers:`.
"""

import bullwhip_cli.providers as providers_mod
from bullwhip_cli.model_switch import list_authenticated_providers, switch_model
from bullwhip_cli.providers import resolve_provider_full
...
def test_list_authenticated_providers_includes_custom_providers(monkeypatch):
    ...
    monkeypatch.setattr(providers_mod, "BULLWHIP_OVERLAYS", {})
...
def test_switch_model_accepts_explicit_named_custom_provider(monkeypatch):
    ...
    monkeypatch.setattr("bullwhip_cli.models.validate_requested_model", lambda *a, **k: _MOCK_VALIDATION)
    monkeypatch.setattr("bullwhip_cli.model_switch.get_model_info", lambda *a, **k: None)
    monkeypatch.setattr("bullwhip_cli.model_switch.get_model_capabilities", lambda *a, **k: None)
