...
def _write_auth_store(tmp_path, payload: dict) -> None:
    ...
    (bullwhip_home / "auth.json").write_text(json.dumps(payload, indent=2))
...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    ...
    payload = json.loads((tmp_path / "bullwhip" / "auth.json").read_text())
...
def test_auth_add_anthropic_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.run_bullwhip_oauth_login_pure",
        lambda: {
            "access_token": token,
    ...
    entry = next(item for item in entries if item["source"] == "manual:bullwhip_pkce")
    assert entry["label"] == "claude@example.com"
    assert entry["source"] == "manual:bullwhip_pkce"
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "bullwhip_cli.auth._nous_device_code_login",
        lambda **kwargs: {
            "portal_base_url": "https://portal.example.com",
            "inference_base_url": "https://inference.example.com/v1",
            "client_id": "bullwhip-cli",
...
def test_auth_add_codex_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "bullwhip_cli.auth._codex_device_code_login",
        lambda: {
            "tokens": {
                "access_token": token,
                "refresh_token": "refresh-token",
...
def test_auth_remove_reindexes_priorities(tmp_path, monkeypatch):
    ...
    from bullwhip_cli.auth_commands import auth_remove_command
...
def test_auth_reset_clears_provider_statuses(tmp_path, monkeypatch, capsys):
    ...
    from bullwhip_cli.auth_commands import auth_reset_command
...
def test_auth_list_does_not_call_mutating_select(monkeypatch, capsys):
    from bullwhip_cli.auth_commands import auth_list_command
    ...
    monkeypatch.setattr(
        "bullwhip_cli.auth_commands.load_pool",
...
def test_auth_list_shows_exhausted_cooldown(monkeypatch, capsys):
    ...
    monkeypatch.setattr("bullwhip_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr("bullwhip_cli.auth_commands.time.time", lambda: 1030.0)
...
def test_auth_list_prefers_explicit_reset_time(monkeypatch, capsys):
    ...
    monkeypatch.setattr("bullwhip_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr(
        "bullwhip_cli.auth_commands.time.time",
...
def test_auth_remove_claude_code_suppresses_reseed(tmp_path, monkeypatch):
    ...
    (bullwhip_home / "auth.json").write_text(json.dumps(auth_store))

    from types import SimpleNamespace
    from bullwhip_cli.auth_commands import auth_remove_command
    auth_remove_command(SimpleNamespace(provider="anthropic", target="1"))

    updated = json.loads((bullwhip_home / "auth.json").read_text())
