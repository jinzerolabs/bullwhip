...
from bullwhip_cli.codex_models import DEFAULT_CODEX_MODELS, get_codex_model_ids
...
def test_setup_wizard_codex_import_resolves():
    ...
    # This mirrors the exact import used in bullwhip_cli/setup.py line 873.
    # A prior bug had 'get_codex_models' (wrong) instead of 'get_codex_model_ids'.
    from bullwhip_cli.codex_models import get_codex_model_ids as setup_import
...
def test_get_codex_model_ids_adds_forward_compat_models_from_templates(monkeypatch):
    monkeypatch.setattr(
        "bullwhip_cli.codex_models._fetch_models_from_api",
...
def test_model_command_uses_runtime_access_token_for_codex_list(monkeypatch):
    from bullwhip_cli.main import _model_flow_openai_codex

    captured = {}

    monkeypatch.setattr(
        "bullwhip_cli.auth.get_codex_auth_status",
        lambda: {"logged_in": True},
    )
    monkeypatch.setattr(
        "bullwhip_cli.auth.resolve_codex_runtime_credentials",
...
def _make_cli(model="anthropic/claude-opus-4.6", **kwargs):
    """Create a BullWhipCLI with minimal mocking."""
    ...
    clean_env = {"LLM_MODEL": "", "BULLWHIP_MAX_ITERATIONS": ""}
    with (
        patch("cli.get_tool_definitions", return_value=[]),
        patch.dict("os.environ", clean_env, clear=False),
        patch.dict(_cli_mod.__dict__, {"CLI_CONFIG": _clean_config}),
    ):
        cli = BullWhipCLI(model=model, **kwargs)
    return cli
...
class TestNormalizeModelForProvider:
    ...
    def test_default_model_replaced(self):
        ...
        with (
            patch("cli.get_tool_definitions", return_value=[]),
            patch.dict("os.environ", {"LLM_MODEL": "", "BULLWHIP_MAX_ITERATIONS": ""}, clear=False),
            patch.dict(_cli_mod.__dict__, {"CLI_CONFIG": _clean_config}),
        ):
            from cli import BullWhipCLI
            cli = BullWhipCLI()
