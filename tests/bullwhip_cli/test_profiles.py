"""Comprehensive tests for bullwhip_cli.profiles module.
...
from bullwhip_cli.profiles import (
    ...
    _get_profiles_root,
    _get_default_bullwhip_home,
)


# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and BULLWHIP_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.bullwhip/profiles)
    * BULLWHIP_HOME  -> tmp_path/.bullwhip  (so get_bullwhip_home() agrees)
    * Creates the bare-minimum ~/.bullwhip directory.
    """
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    default_home = tmp_path / ".bullwhip"
    default_home.mkdir(exist_ok=True)
    monkeypatch.setenv("BULLWHIP_HOME", str(default_home))
    return tmp_path
...
class TestGetProfileDir:
    ...
    def test_default_returns_bullwhip_home(self, profile_env):
        tmp_path = profile_env
        result = get_profile_dir("default")
        assert result == tmp_path / ".bullwhip"

    def test_named_profile_returns_profiles_subdir(self, profile_env):
        tmp_path = profile_env
        result = get_profile_dir("coder")
        assert result == tmp_path / ".bullwhip" / "profiles" / "coder"
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("bullwhip_cli.profiles._cleanup_gateway_service"):
...
class TestActiveProfile:
    ...
    def test_empty_file_returns_default(self, profile_env):
        tmp_path = profile_env
        active_path = tmp_path / ".bullwhip" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_default_bullwhip_home_returns_default(self, profile_env):
        # BULLWHIP_HOME points to tmp_path/.bullwhip which is the default
        assert get_active_profile_name() == "default"

    def test_profile_path_returns_profile_name(self, profile_env, monkeypatch):
        tmp_path = profile_env
        create_profile("coder", no_alias=True)
        profile_dir = tmp_path / ".bullwhip" / "profiles" / "coder"
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom BULLWHIP_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("BULLWHIP_HOME", str(custom))
        # With Docker-aware roots, a custom BULLWHIP_HOME is the default —
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("bullwhip")
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        old_dir = tmp_path / ".bullwhip" / "profiles" / "oldname"
        assert old_dir.is_dir()

        # Mock alias collision to avoid subprocess calls
        with patch("bullwhip_cli.profiles.check_alias_collision", return_value="skip"):
            new_dir = rename_profile("oldname", "newname")

        assert not old_dir.is_dir()
        assert new_dir.is_dir()
        assert new_dir == tmp_path / ".bullwhip" / "profiles" / "newname"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("bullwhip-agent", ".worktrees", "profiles", "bin",
        ...
        excluded_prefixes = [
            "default/bullwhip-agent", "default/.worktrees", "default/profiles",
...
class TestCompletion:
    ...
    def test_bash_completion_hasbullwhip_profiles_function(self):
        script = generate_bash_completion()
        assert "bullwhip_profiles" in script

    def test_zsh_completion_hasbullwhip_function(self):
        script = generate_zsh_completion()
        assert "_bullwhip" in script


# ===================================================================
# TestGetProfilesRoot / TestGetDefaultBullWhipHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_bullwhip_home()."""

    def test_profiles_root_under_home(self, profile_env):
        tmp_path = profile_env
        root = _get_profiles_root()
        assert root == tmp_path / ".bullwhip" / "profiles"

    def test_default_bullwhip_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_bullwhip_home()
        assert home == tmp_path / ".bullwhip"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (BULLWHIP_HOME outside ~/.bullwhip), profiles go under BULLWHIP_HOME."""
    ...
    def test_default_bullwhip_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_bullwhip_home() returns BULLWHIP_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("BULLWHIP_HOME", str(docker_home))
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (BULLWHIP_HOME under ~/.bullwhip), profiles root is still ~/.bullwhip/profiles."""
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        """In Docker, active_profile file lives under BULLWHIP_HOME."""
        from bullwhip_cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("BULLWHIP_HOME", str(docker_home))
    ...
    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under BULLWHIP_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("BULLWHIP_HOME", str(docker_home))
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".bullwhip" / "profiles" / "mybot"
        assert result == expected
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from bullwhip_cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".bullwhip"
