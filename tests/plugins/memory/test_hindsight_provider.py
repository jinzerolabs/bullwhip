...
def provider(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "plugins.memory.hindsight.get_bullwhip_home", lambda: tmp_path
    )

    p = HindsightMemoryProvider()
    p.initialize(session_id="test-session", bullwhip_home=str(tmp_path), platform="cli")
...
class TestConfig:
    def test_default_values(self, provider):
        ...
        assert provider._retain_context == "conversation between Bull Whip Agent and the User"
    ...
    def test_config_from_env_fallback(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "plugins.memory.hindsight.get_bullwhip_home",
        ...
        assert cfg["banks"]["bullwhip"]["bankId"] == "env-bank"
        assert cfg["banks"]["bullwhip"]["budget"] == "high"
...
class TestSyncTurn:
    ...
    def test_sync_turn_uses_aretain_batch(self, provider):
        ...
        assert call_kwargs["items"][0]["context"] == "conversation between Bull Whip Agent and the User"
