"""
Tests for timezone support (bullwhip_time module + integration points).
...
import bullwhip_time


def _reset_bullwhip_time_cache():
    """Reset the bullwhip_time module cache (replacement for removed reset_cache)."""
    bullwhip_time._cached_tz = None
    bullwhip_time._cached_tz_name = None
    bullwhip_time._cache_resolved = False


# =========================================================================
# bullwhip_time.now() — core helper
# =========================================================================

class TestBullWhipTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_bullwhip_time_cache()
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["BULLWHIP_TIMEZONE"] = "Asia/Kolkata"
        result = bullwhip_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["BULLWHIP_TIMEZONE"] = "UTC"
        result = bullwhip_time.now()
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["BULLWHIP_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="bullwhip_time"):
            result = bullwhip_time.now()
    ...
    def test_cache_invalidation(self):
        ...
        os.environ["BULLWHIP_TIMEZONE"] = "UTC"
        _reset_bullwhip_time_cache()
        r1 = bullwhip_time.now()
        assert r1.utcoffset() == timedelta(0)

        os.environ["BULLWHIP_TIMEZONE"] = "Asia/Kolkata"
        _reset_bullwhip_time_cache()
        r2 = bullwhip_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["BULLWHIP_TIMEZONE"] = "Europe/London"
        tz = bullwhip_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["BULLWHIP_TIMEZONE"] = "Not/A/Timezone"
        tz = bullwhip_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When BULLWHIP_TIMEZONE is set, child process sees TZ env var."""
        import json as _json
        os.environ["BULLWHIP_TIMEZONE"] = "Asia/Kolkata"
    ...
    def test_tz_not_injected_when_empty(self):
        """When BULLWHIP_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("BULLWHIP_TIMEZONE", None)
    ...
    def test_bullwhip_timezone_not_leaked_to_child(self):
        """BULLWHIP_TIMEZONE itself must NOT appear in child env (only TZ)."""
        import json as _json
        os.environ["BULLWHIP_TIMEZONE"] = "Asia/Kolkata"

        with patch("model_tools.handle_function_call", side_effect=self._mock_handle):
            result = _json.loads(self._execute_code(
                code='import os; print(os.environ.get("BULLWHIP_TIMEZONE", "NOT_SET"))',
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        absolute time when system-local tz != BullWhip tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_tobullwhip_tz(self):
        """Already-aware datetimes should be normalized to Bull Whip tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in BullWhip tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Bull Whip tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), BullWhip is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when BullWhip tz
        ...
        # Use a BullWhip timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _bullwhip_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["BULLWHIP_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_bullwhip_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Bull Whip timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["BULLWHIP_TIMEZONE"] = "US/Eastern"
        _reset_bullwhip_time_cache()
