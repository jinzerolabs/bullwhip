...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("bullwhip/bullwhip-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned bullwhip/* and pr-* branches."""

    def test_prunes_orphanedbullwhip_branch(self, git_repo):
        """bullwhip/bullwhip-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "bullwhip/bullwhip-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "bullwhip/bullwhip-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "bullwhip/bullwhip-deadbeef" in result.stdout
        ...
        assert "bullwhip/bullwhip-deadbeef" in orphaned
        ...
        assert "bullwhip/bullwhip-deadbeef" not in result.stdout
