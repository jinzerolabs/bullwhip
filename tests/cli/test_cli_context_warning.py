...
def _isolate(tmp_path, monkeypatch):
    """Isolate BULLWHIP_HOME so tests don't touch real config."""
...
def cli_obj(_isolate):
    """Create a minimal BullWhipCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = BullWhipCLI.__new__(BullWhipCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["bullwhip-core"]
        obj.compact = False
