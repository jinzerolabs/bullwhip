...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security.bullwhip_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_bullwhip_bin(self):
        """After _INSTALL_FAILED, manual install in BULLWHIP_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        bullwhip_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(bullwhip_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(bullwhip_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security.bullwhip_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == bullwhip_bin
            assert _tirith_mod._resolved_path == bullwhip_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# BULLWHIP_HOME isolation
# ---------------------------------------------------------------------------

class TestBullWhipHomeIsolation:
    def test_bullwhip_bin_dir_respects_bullwhip_home(self):
        """bullwhip_bin_dir must use BULLWHIP_HOME, not hardcoded ~/.bullwhip."""
        from tools.tirith_security import bullwhip_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"BULLWHIP_HOME": tmpdir}):
            result = bullwhip_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_bullwhip_home(self):
        """_failure_marker_path must use BULLWHIP_HOME, not hardcoded ~/.bullwhip."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"BULLWHIP_HOME": "/custom/bullwhip"}):
            result = _failure_marker_path()
        assert result == "/custom/bullwhip/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets BULLWHIP_HOME; verify it's active."""
        bullwhip_home = os.getenv("BULLWHIP_HOME")
        assert bullwhip_home is not None, "BULLWHIP_HOME should be set by conftest"
        assert "hermes_test" in bullwhip_home, "Should point to test temp dir"

    def test_get_bullwhip_home_fallback(self):
        """Without BULLWHIP_HOME set, falls back to ~/.bullwhip."""
        from tools.tirith_security import _get_bullwhip_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove BULLWHIP_HOME entirely
            os.environ.pop("BULLWHIP_HOME", None)
            result = _get_bullwhip_home()
        assert result == os.path.join(os.path.expanduser("~"), ".bullwhip")
