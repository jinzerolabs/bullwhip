...
def _clean_state():
    ...
    saved = {}
    for k in ("BULLWHIP_INTERACTIVE", "BULLWHIP_GATEWAY_SESSION", "BULLWHIP_EXEC_ASK", "BULLWHIP_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["BULLWHIP_INTERACTIVE"] = "1"
        session_key = os.getenv("BULLWHIP_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No BULLWHIP_INTERACTIVE or BULLWHIP_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
