...
class TestApprovalModeParsing:
    def test_unquoted_yaml_off_boolean_false_maps_to_off(self):
        with mock_patch("bullwhip_cli.config.load_config", return_value={"approvals": {"mode": False}}):
            assert _get_approval_mode() == "off"

    def test_string_off_still_maps_to_off(self):
        with mock_patch("bullwhip_cli.config.load_config", return_value={"approvals": {"mode": "off"}}):
            assert _get_approval_mode() == "off"
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"BULLWHIP_SESSION_KEY": "bob"}, clear=False):
...
class TestTeePattern:
    ...
    def test_teebullwhip_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.bullwhip/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_custom_bullwhip_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $BULLWHIP_HOME/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_quoted_custom_bullwhip_home_env(self):
        dangerous, key, desc = detect_dangerous_command('echo x | tee "$BULLWHIP_HOME/.env"')
        assert dangerous is True
        assert key is not None
...
class TestSensitiveRedirectPattern:
    ...
    def test_redirect_to_custom_bullwhip_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x > $BULLWHIP_HOME/.env")
        assert dangerous is True
        assert key is not None
...
class TestGatewayProtection:
    ...
    def test_gateway_run_with_disown_detected(self):
        cmd = "kill 1605 && cd ~/.bullwhip/bullwhip-agent && source venv/bin/activate && python -m bullwhip_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc

    def test_gateway_run_with_ampersand_detected(self):
        cmd = "python -m bullwhip_cli.main gateway run --replace &"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_nohup_detected(self):
        cmd = "nohup python -m bullwhip_cli.main gateway run --replace"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_setsid_detected(self):
        cmd = "bullwhip_cli.main gateway run --replace &disown"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart bullwhip-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "stop/restart" in desc

    def test_pkillbullwhip_detected(self):
        """pkill targeting bullwhip/gateway processes must be caught."""
        cmd = 'pkill -f "cli.py --gateway"'
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc

    def test_killallbullwhip_detected(self):
        cmd = "killall bullwhip"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep bullwhip) bypasses the pkill/killall name-matching
    ...
    def test_pkillbullwhip_still_detected(self):
        """Existing pkill pattern must not regress."""
        cmd = "pkill -9 bullwhip"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True
