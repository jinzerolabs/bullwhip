...
class TestFirecrawlClientConfig:
    ...
    def setup_method(self):
        ...
        for key in (
            "BULLWHIP_ENABLE_NOUS_MANAGED_TOOLS",
            "FIRECRAWL_API_KEY",
            "FIRECRAWL_API_URL",
        ...
        os.environ["BULLWHIP_ENABLE_NOUS_MANAGED_TOOLS"] = "1"
    ...
    def test_nous_auth_token_respects_bullwhip_home_override(self, tmp_path):
        """Auth lookup should read from BULLWHIP_HOME/auth.json, not ~/.bullwhip/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".bullwhip").mkdir(parents=True)
        ...
        with patch.dict(os.environ, {
            "HOME": str(real_home),
            "BULLWHIP_HOME": str(bullwhip_home),
        }, clear=False):
            import tools.web_tools
...
class TestBackendSelection:
    ...
    ``bullwhip tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
    ...
    def setup_method(self):
        ...
        for key in self._ENV_KEYS:
            if key != "BULLWHIP_ENABLE_NOUS_MANAGED_TOOLS":
                os.environ.pop(key, None)
