...
class TestWriteDenyExactPaths:
    ...
    def testbullwhip_env(self):
        path = os.path.join(str(Path.home()), ".bullwhip", ".env")
        assert _is_write_denied(path) is True
...
class TestWriteAllowed:
    ...
    def testbullwhip_config_not_env(self):
        path = os.path.join(str(Path.home()), ".bullwhip", "config.yaml")
