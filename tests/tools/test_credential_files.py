...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        bullwhip_home.mkdir()
        (bullwhip_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"BULLWHIP_HOME": str(bullwhip_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(bullwhip_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.bullwhip/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        bullwhip_home = tmp_path / ".bullwhip"
        bullwhip_home.mkdir()
        (bullwhip_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        bullwhip_home = tmp_path / ".bullwhip"
        bullwhip_home.mkdir()
        (bullwhip_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        bullwhip_home = tmp_path / ".bullwhip"
        bullwhip_home.mkdir()
        (bullwhip_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        bullwhip_home = tmp_path / ".bullwhip"
        skills_dir = bullwhip_home / "skills"
        ...
        assert mounts[0]["container_path"] == "/root/.bullwhip/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        bullwhip_home = tmp_path / ".bullwhip"
        (bullwhip_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"BULLWHIP_HOME": str(bullwhip_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.bullwhip")

        assert mounts[0]["container_path"] == "/home/user/.bullwhip/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.bullwhip/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.bullwhip/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape BULLWHIP_HOME."""
        monkeypatch.setenv("BULLWHIP_HOME", str(tmp_path / ".bullwhip"))
        (tmp_path / ".bullwhip").mkdir()

        # Create a sensitive file one level above bullwhip_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside bullwhip_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside BULLWHIP_HOME must still be registered."""
        bullwhip_home = tmp_path / ".bullwhip"
        bullwhip_home.mkdir()
        monkeypatch.setenv("BULLWHIP_HOME", str(bullwhip_home))
        (bullwhip_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_bullwhip_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of BULLWHIP_HOME must be allowed."""
        bullwhip_home = tmp_path / ".bullwhip"
        bullwhip_home.mkdir()
        subdir = bullwhip_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside BULLWHIP_HOME pointing outside must be rejected."""
        bullwhip_home = tmp_path / ".bullwhip"
        bullwhip_home.mkdir()
        monkeypatch.setenv("BULLWHIP_HOME", str(bullwhip_home))

        # Create a sensitive file outside bullwhip_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside bullwhip_home pointing outside
        symlink = bullwhip_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes BULLWHIP_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, bullwhip_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape BULLWHIP_HOME."""
        bullwhip_home = tmp_path / ".bullwhip"
        bullwhip_home.mkdir()
        monkeypatch.setenv("BULLWHIP_HOME", str(bullwhip_home))
        ...
        sensitive.write_text("{}")
        self._write_config(bullwhip_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(bullwhip_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside BULLWHIP_HOME via config must still mount."""
        bullwhip_home = tmp_path / ".bullwhip"
        bullwhip_home.mkdir()
        monkeypatch.setenv("BULLWHIP_HOME", str(bullwhip_home))

        (bullwhip_home / "oauth.json").write_text("{}")
        self._write_config(bullwhip_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        bullwhip_home = tmp_path / ".bullwhip"
        bullwhip_home.mkdir()
        (bullwhip_home / "cache" / "documents").mkdir(parents=True)
        (bullwhip_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("BULLWHIP_HOME", str(bullwhip_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.bullwhip/cache/documents" in paths
        assert "/root/.bullwhip/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.bullwhip/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        bullwhip_home = tmp_path / ".bullwhip"
        bullwhip_home.mkdir()
        # Use legacy dir name — getbullwhip_dir prefers old if it exists
        (bullwhip_home / "document_cache").mkdir()
        (bullwhip_home / "image_cache").mkdir()
        monkeypatch.setenv("BULLWHIP_HOME", str(bullwhip_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(bullwhip_home / "document_cache") in host_paths
        assert str(bullwhip_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.bullwhip/cache/documents" in container_paths
        assert "/root/.bullwhip/cache/images" in container_paths
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        bullwhip_home = tmp_path / ".bullwhip"
        doc_dir = bullwhip_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        bullwhip_home = tmp_path / ".bullwhip"
        ss_dir = bullwhip_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.bullwhip/cache/screenshots/session_abc/screen1.png"
