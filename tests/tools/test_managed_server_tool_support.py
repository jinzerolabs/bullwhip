...
Validates that:
1. The installed atroposlib API still matches BullWhip's expectations
2. BullWhip's parser registry remains compatible with ManagedServer parsing
3. BullWhipAgentBaseEnv wires the selected parser into ServerManager correctly

These tests verify the contract between bullwhip-agent's environments/ code
...
class TestManagedServerAPI:
    """Test that ManagedServer's API matches what bullwhip-agent expects."""
...
class TestParserCompatibility:
    """Test that bullwhip-agent's parsers match ManagedServer's expectations."""
...
class TestBaseEnvCompatibility:
    """Test that bullwhip_base_env.py's tool-parser wiring matches the current API."""

    def test_bullwhip_base_env_sets_server_manager_tool_parser(self):
        """BullWhip wires parser selection through ServerManager.tool_parser."""
        import ast

        base_env_path = Path(__file__).parent.parent.parent / "environments" / "bullwhip_base_env.py"
        ...
        assert found_assignment, (
            "bullwhip_base_env.py should set self.server.tool_parser from config.tool_call_parser"
        )

    def test_bullwhip_base_env_uses_config_tool_call_parser(self):
        """Verify bullwhip_base_env uses the config field rather than a local parser instance."""
        base_env_path = Path(__file__).parent.parent.parent / "environments" / "bullwhip_base_env.py"
        source = base_env_path.read_text()
