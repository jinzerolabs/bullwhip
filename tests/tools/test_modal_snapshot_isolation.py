...
def _restore_tool_modules():
    original_bullwhip_home = os.environ.get("BULLWHIP_HOME")
    original_modules = {
        name: module
        for name, module in sys.modules.items()
        if name == "tools"
        or name.startswith("tools.")
        or name == "bullwhip_cli"
        or name.startswith("bullwhip_cli.")
        or name == "modal"
    ...
    try:
        ...
        if original_bullwhip_home is None:
            os.environ.pop("BULLWHIP_HOME", None)
        else:
            os.environ["BULLWHIP_HOME"] = original_bullwhip_home
        _reset_modules(("tools", "bullwhip_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    bullwhip_cli = types.ModuleType("bullwhip_cli")
    bullwhip_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["bullwhip_cli"] = bullwhip_cli
    bullwhip_home = tmp_path / "bullwhip-home"
    os.environ["BULLWHIP_HOME"] = str(bullwhip_home)
    sys.modules["bullwhip_cli.config"] = types.SimpleNamespace(
        get_bullwhip_home=lambda: bullwhip_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="bullwhip-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": bullwhip_home / "modal_snapshots.json",
