...
"""
    ...
    assert "Tyler Williams > Active Projects: Bull Whip Agent" in entries
...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `bullwhip cron` to recreate scheduled tasks" in notes_text
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # bullwhip_config_mod  — print statements in the post-migration summary that
    #                      tell the user to *review* ~/.bullwhip/config.yaml;
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "BullWhip prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told BullWhip to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "BullWhip config is great"
    assert mod.rebrand_text("openclaw should always respond concisely") == "BullWhip should always respond concisely"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "BullWhip uses tools well"


def test_rebrand_text_replaces_legacy_bot_names():
    mod = load_module()
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "BullWhip remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "BullWhip prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "BullWhip was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "BullWhip uses Python"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "BullWhip said to ask BullWhip about BullWhip settings"
...
def test_migrate_soul_rebrands_content(tmp_path):
    ...
    assert "OpenClaw" not in result
    assert "You are BullWhip" in result
