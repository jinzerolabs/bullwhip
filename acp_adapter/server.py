"""ACP agent server — exposes Bull Whip Agent via the Agent Client Protocol."""
...
try:
    from bullwhip_cli import __version__ as BULLWHIP_VERSION
except Exception:
    BULLWHIP_VERSION = "0.0.0"
...
class BullWhipACPAgent(acp.Agent):
    """ACP Agent implementation wrapping BullWhip AIAgent."""
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show BullWhip version",
        },
    )
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = getattr(state.agent, "enabled_toolsets", None) or ["bullwhip-acp"]
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        if provider:
            auth_methods = [
                AuthMethodAgent(
                    ...
                    description=f"Authenticate BullWhip using the currently configured {provider} runtime credentials.",
                )
            ]
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="bullwhip-agent", version=BULLWHIP_VERSION),
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run BullWhip on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
    ...
    def _cmd_model(self, args: str, state: SessionState) -> str:
        ...
        try:
            from bullwhip_cli.models import parse_model_input, detect_provider_for_model
    ...
    def _cmd_tools(self, args: str, state: SessionState) -> str:
        try:
            from model_tools import get_tool_definitions
            toolsets = getattr(state.agent, "enabled_toolsets", None) or ["bullwhip-acp"]
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Bull Whip Agent v{BULLWHIP_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when BullWhip has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
