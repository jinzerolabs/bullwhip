"""ACP session manager — maps ACP sessions to Bull Whip AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.bullwhip/state.db``) so they
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Bull Whip agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Bull Whip AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current BullWhip runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.bullwhip/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``BULLWHIP_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_bullwhip_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=bullwhip_home / "state.db")
            return self._db_instance
        except Exception:
