...
The evaluate flow:
    ...
    2. evaluate()  -- Iterates over all runs sequentially through:
        a. rollout_and_score_eval()  -- Per-run agent loop
            ...
            - Runs BullWhipAgentLoop with terminal tool only
...
Requires: pip install bullwhip-agent[yc-bench]
"""
...
from environments.agent_loop import BullWhipAgentLoop
...
class YCBenchEvalConfig(BullWhipAgentEnvConfig):
    """
    Configuration for the YC-Bench evaluation environment.

    Extends BullWhipAgentEnvConfig with YC-Bench-specific settings for
...
class YCBenchEvalEnv(BullWhipAgentBaseEnv):
    """
    ...
    a competing built-in agent loop). The BullWhipAgentLoop then drives the
    ...
    async def rollout_and_score_eval(self, eval_item: Dict[str, Any]) -> Dict:
        ...
        3. Runs BullWhipAgentLoop with terminal tool
        ...
        try:
            ...
            # LiteLLM), which would compete with our BullWhipAgentLoop.
            ...
            # ----------------------------------------------------------
            # Step 2: Run the Bull WhipAgentLoop
            # ----------------------------------------------------------
