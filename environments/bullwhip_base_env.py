"""
BullWhipAgentBaseEnv -- Abstract Base Environment for Bull Whip-Agent + Atropos

Provides the Atropos integration plumbing that all bullwhip-agent environments share:
- Two-mode operation (OpenAI server for Phase 1, VLLM ManagedServer for Phase 2)
- Per-group toolset/distribution resolution
- Agent loop orchestration via BullWhipAgentLoop
...
# Ensure the bullwhip-agent repo root is on sys.path so that imports like
...
# Load API keys from bullwhip-agent/.env so all environments can access them
...
from environments.agent_loop import AgentResult, BullWhipAgentLoop
...
# Import bullwhip-agent toolset infrastructure
...
class BullWhipAgentEnvConfig(BaseEnvConfig):
    """
    Configuration for bullwhip-agent Atropos environments.
    ...
    enabled_toolsets: Optional[List[str]] = Field(
        default=None,
        description="Explicit list of bullwhip toolsets to enable (e.g., ['terminal', 'file', 'web']). "
    ...
    tool_call_parser: str = Field(
        ...
        "Options: bullwhip, mistral, llama3_json, qwen, deepseek_v3, etc.",
    )
    ...
    default_result_size_chars: int = Field(
        ...
        "to sandbox. Results exceeding this are written to /tmp/bullwhip-results/ "
...
class BullWhipAgentBaseEnv(BaseEnv):
    """
    Abstract base environment for bullwhip-agent Atropos integration.
    ...
    name: Optional[str] = "bullwhip-agent"
    env_config_cls = BullWhipAgentEnvConfig

    def __init__(
        self,
        config: BullWhipAgentEnvConfig,
        ...
        # Set terminal environment variables so bullwhip tools pick them up.
    ...
    async def compute_reward(
        ...
        - ctx: ToolContext -- call ANY bullwhip-agent tool (terminal, file, web,
