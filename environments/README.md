# Bull Whip-Agent Atropos Environments

This directory contains the integration layer between **bullwhip-agent's** tool-calling capabilities and the **Atropos** RL training framework. It provides everything needed to run agentic LLMs through multi-turn tool-calling loops, score their output with arbitrary reward functions, and feed results into Atropos for training or evaluation.
...
```
                    ...
                    └───────────┬───────────┘
                                │ inherits
                    ┌───────────┴───────────┐
                    │  HermesAgentBaseEnv    │  bullwhip_base_env.py
                    │  - Terminal backend    │
                    │  - Tool resolution     │
                    │  - Agent loop          │
                    │  - ToolContext          │
...
**HermesAgentBaseEnv** (`bullwhip_base_env.py`) extends BaseEnv with bullwhip-agent specifics:
- Sets `os.environ["TERMINAL_ENV"]` to configure the terminal backend (local, docker, modal, daytona, ssh, singularity)
- Resolves bullwhip-agent toolsets via `_resolve_tools_for_group()` (calls `get_tool_definitions()` which queries `tools/registry.py`)
...
`HermesAgentLoop` is the reusable multi-turn agent engine. It runs the same pattern as bullwhip-agent's `run_agent.py`:
...
`ToolContext` is a per-rollout handle that gives reward/verification functions direct access to **all** bullwhip-agent tools, scoped to the rollout's `task_id`. The same `task_id` means the terminal/browser session is the SAME one the model used during its rollout -- all state (files, processes, browser tabs) is preserved.
...
- **Generic**: `call_tool(name, args)` -- call any bullwhip-agent tool by name
- **Cleanup**: `cleanup()` -- release all resources (called automatically after `compute_reward`)

### Patches (`patches.py`)

**Problem**: Some bullwhip-agent tools use `asyncio.run()` internally (e.g., the Modal backend). This crashes when called from inside Atropos's event loop because `asyncio.run()` cannot be nested.
...
Available parsers:
- `bullwhip` -- Bull Whip/ChatML `<tool_call>` XML format
...
├── bullwhip_base_env.py            # Abstract base (HermesAgentBaseEnv)
...
│
├── bullwhip_swe_env/               # SWE-bench style training environment
│   └── bullwhip_swe_env.py
│
...
### Bull WhipSweEnv (`bullwhip_swe_env/`)
...
|-------|-------------|---------|
| `enabled_toolsets` | Which bullwhip toolsets to enable | `None` (all) |
...
| `tool_call_parser` | Parser name for Phase 2 | `bullwhip` |
