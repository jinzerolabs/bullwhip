...
Training tasks (3):
    1. Create ~/greeting.txt with "Hello from Bull Whip Agent"
...
TRAIN_TASKS = [
    {
        "prompt": "Create a file at ~/greeting.txt containing exactly the text: Hello from Bull Whip Agent",
        "verify_path": "~/greeting.txt",
        "expected_content": "Hello from Bull Whip Agent",
    },
    {
...
class TerminalTestEnv(BullWhipAgentBaseEnv):
    ...
    def config_init(cls) -> Tuple[TerminalTestEnvConfig, List[APIServerConfig]]:
        ...
        Claude for inference. API keys loaded from ~/bullwhip-agent/.env.
        """
