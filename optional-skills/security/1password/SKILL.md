...
version: 1.0.0
author: arceus77-7, enhanced by Bull Whip Agent
license: MIT
metadata:
  bullwhip:
...
- `tmux` available for stable authenticated sessions during Bull Whip terminal calls (desktop app flow only)
...
### Service Account (recommended for Bull Whip)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.bullwhip/.env` (the skill will prompt for this on first load).
...
## Bull Whip Execution Pattern (desktop app flow)

Bull Whip terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/bullwhip-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/bullwhip-op.sock"
