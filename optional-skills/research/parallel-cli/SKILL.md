...
This is an optional third-party workflow, not a Bull Whip core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with Bull Whip native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer Bull Whip native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer Bull Whip native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
Recommended Bull Whip workflow:
...
## Recommended Bull Whip usage patterns
...
- Do not silently choose Parallel when Bull Whip native tools are already sufficient.
