...
description: Migrate a user's OpenClaw customization footprint into Bull Whip Agent. Imports Bull Whip-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
version: 1.0.0
author: Bull Whip Agent (ZeroLabs Korea)
license: MIT
metadata:
  bullwhip:
    tags: [Migration, OpenClaw, Bull Whip, Memory, Persona, Import]
    related_skills: [bullwhip-agent]
---

# OpenClaw -> Bull Whip Migration

Use this skill when a user wants to move their OpenClaw setup into Bull Whip Agent with minimal manual cleanup.
...
```bash
bullwhip claw migrate              # Full interactive migration
...
bullwhip claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `bullwhip setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_bullwhip.py` to:

- import `SOUL.md` into the Bull Whip home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Bull Whip memory entries
- merge OpenClaw command approval patterns into Bull Whip `command_allowlist`
- migrate Bull Whip-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.bullwhip/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Bull Whip workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.bullwhip/tts/`
- archive non-secret docs that do not have a direct Bull Whip destination
...
- `scripts/openclaw_to_bullwhip.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.bullwhip/skills/migration/openclaw-migration/scripts/openclaw_to_bullwhip.py`

Do not guess a shorter path like `~/.bullwhip/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.bullwhip/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Bull Whip-compatible secrets, currently:
...
Bull Whip CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Bull Whip. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Bull Whip workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Bull Whip skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.bullwhip/skills/migration/openclaw-migration/scripts/openclaw_to_bullwhip.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.bullwhip/skills/migration/openclaw-migration/scripts/openclaw_to_bullwhip.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.bullwhip/skills/migration/openclaw-migration/scripts/openclaw_to_bullwhip.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.bullwhip/skills/migration/openclaw-migration/scripts/openclaw_to_bullwhip.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.bullwhip/skills/migration/openclaw-migration/scripts/openclaw_to_bullwhip.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.bullwhip/skills/migration/openclaw-migration/scripts/openclaw_to_bullwhip.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Bull Whip unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Bull Whip targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Bull Whip destination. Unsupported auth blobs must still be reported as skipped.
...
- Bull Whip persona state imported
- Bull Whip memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.bullwhip/skills/openclaw-imports/`
