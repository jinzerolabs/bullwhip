---
name: bullwhip-atropos-environments
description: Build, test, and debug Bull Whip Agent RL environments for Atropos training. Covers the Bull WhipAgentBaseEnv interface, reward functions, agent loop integration, evaluation with tools, wandb logging, and the three CLI modes (serve/process/evaluate). Use when creating, reviewing, or fixing RL environments in the bullwhip-agent repo.
version: 1.1.0
...
Guide for building RL environments in the bullwhip-agent repo that integrate with the Atropos training framework.

## Architecture Overview

```
Atropos BaseEnv (atroposlib/envs/base.py)
    └── HermesAgentBaseEnv (environments/bullwhip_base_env.py)
...
Bull Whip environments are special because they run a **multi-turn agent loop with tool calling** — not just single-turn completions. The base env handles the loop; you implement the task and scoring.

## File Locations

| File | Purpose |
|------|---------|
| `environments/bullwhip_base_env.py` | Base class with agent loop + tool resolution |
...
| `environments/tool_call_parsers.py` | Phase 2 tool call parsers (bullwhip, mistral, etc.) |
...
The whole point of bullwhip-agent environments is agentic evaluation:
...
2. **evaluate() must use HermesAgentLoop, not chat_completion** — Single-turn chat_completion has no tools. The whole point of bullwhip-agent benchmarks is agentic evaluation with tool use.
