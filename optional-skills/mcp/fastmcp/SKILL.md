...
- smoke-test a server with the FastMCP CLI before wiring it into Bull Whip or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Bull Whip. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.bullwhip/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.bullwhip/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Bull Whip integration, either:

- configure the server in `~/.bullwhip/config.yaml` using the `native-mcp` skill, or
...
### Bull Whip cannot see the deployed server

The server-building part may be correct while the Bull Whip config is not. Load the `native-mcp` skill and configure the server in `~/.bullwhip/config.yaml`, then restart Bull Whip.
