...
# All skills live in ~/.bullwhip/skills/ (seeded from bundled skills/ on install).
...
def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from BULLWHIP_HOME/.env."""
    env_path = get_bullwhip_home() / ".env"
...
def _is_gateway_surface() -> bool:
    if os.getenv("BULLWHIP_GATEWAY_SESSION"):
        return True
    from gateway.session_context import get_session_env
    return bool(get_session_env("BULLWHIP_SESSION_PLATFORM"))
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return "Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to ~/.bullwhip/.env manually."
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.bullwhip/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    try:
        ...
        resolved_platform = platform or os.getenv("BULLWHIP_PLATFORM")
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.bullwhip/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``bullwhip skills`` config UI). Default False
            ...
            return json.dumps(
                {
                    ...
                    "skills": [],
                    "categories": [],
                    "message": "No skills found. Skills directory created at ~/.bullwhip/skills/",
                },
                ensure_ascii=False,
            )
...
def _serve_plugin_skill(
    ...
    from bullwhip_cli.plugins import _get_disabled_plugins, get_plugin_manager
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable with: bullwhip plugins enable {namespace}"
                ),
            },
            ensure_ascii=False,
        )
...
def skill_view(name: str, file_path: str = None, task_id: str = None) -> str:
    ...
    try:
        ...
        if ":" in name:
            ...
            from bullwhip_cli.plugins import discover_plugins, get_plugin_manager
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.bullwhip/skills/): {skill_md}")
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `bullwhip skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
        ...
        # Check metadata.bullwhip.* first (agentskills.io convention), fall back to top-level
        hermes_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            hermes_meta = metadata.get("bullwhip", {}) or {}

        tags = _parse_tags(bullwhip_meta.get("tags") or frontmatter.get("tags", ""))
