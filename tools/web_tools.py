...
Backend is selected during ``bullwhip tools`` setup (web.backend in config.yaml).
When available, BullWhip can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.bullwhip/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``bullwhip tools``).
...
def _raise_web_backend_configuration_error() -> None:
    ...
    if managed_nous_tools_enabled():
        message += (
            ...
            "(`bullwhip model`) and provide FIRECRAWL_GATEWAY_URL or TOOL_GATEWAY_DOMAIN."
        )
...
def _get_firecrawl_client():
    ...
    BullWhip falls back to the Firecrawl tool-gateway for logged-in Nous Subscribers.
    """
...
def _resolve_web_extract_auxiliary(model: Optional[str] = None) -> tuple[Optional[Any], Optional[str], Dict[str, Any]]:
    ...
    if client is not None and _is_nous_auxiliary_client(client):
        ...
        extra_body = get_auxiliary_extra_body() or {"tags": ["product=bullwhip-agent"]}
...
def _get_exa_client():
    ...
    if _exa_client is None:
        ...
        _exa_client.headers["x-exa-integration"] = "bullwhip-agent"
    return _exa_client
