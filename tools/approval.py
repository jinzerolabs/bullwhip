...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("BULLWHIP_SESSION_KEY", default)

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $BULLWHIP_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_BULLWHIP_ENV_PATH = (
    r'(?:~\/\.bullwhip/|'
    r'(?:\$home|\$\{home\})/\.bullwhip/|'
    r'(?:\$bullwhip_home|\$\{bullwhip_home\})/)'
    r'\.env\b'
)
_SENSITIVE_WRITE_TARGET = (
    r'(?:/etc/|/dev/sd|'
    rf'{_SSH_SENSITIVE_PATH}|'
    rf'{_BULLWHIP_ENV_PATH})'
)
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bhermes\s+gateway\s+(stop|restart)\b', "stop/restart bullwhip gateway (kills running agents)"),
    (r'\bhermes\s+update\b', "bullwhip update (restarts gateway, kills running agents)"),
    # Gateway protection: never start gateway outside systemd management
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart bullwhip-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart bullwhip-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(bullwhip|gateway|cli\.py)\b', "kill bullwhip/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill bullwhip` but not
    # `kill -9 $(pgrep -f bullwhip)` because the substitution is opaque
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    os.environ["BULLWHIP_SPINNER_PAUSE"] = "1"
    try:
        while True:
            print()
        ...
        if "BULLWHIP_SPINNER_PAUSE" in os.environ:
            del os.environ["BULLWHIP_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if os.getenv("BULLWHIP_YOLO_MODE") or is_current_session_yolo_enabled():
        return {"approved": True, "message": None}
    ...
    is_cli = os.getenv("BULLWHIP_INTERACTIVE")
    is_gateway = os.getenv("BULLWHIP_GATEWAY_SESSION")

    if not is_cli and not is_gateway:
        return {"approved": True, "message": None}

    if is_gateway or os.getenv("BULLWHIP_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if os.getenv("BULLWHIP_YOLO_MODE") or is_current_session_yolo_enabled() or approval_mode == "off":
        return {"approved": True, "message": None}

    is_cli = os.getenv("BULLWHIP_INTERACTIVE")
    is_gateway = os.getenv("BULLWHIP_GATEWAY_SESSION")
    is_ask = os.getenv("BULLWHIP_EXEC_ASK")
