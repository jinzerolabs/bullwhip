...
2. **Per-result persistence** (maybe_persist_tool_result): After a tool
   ...
   SANDBOX temp dir (for example /tmp/bullwhip-results/{tool_use_id}.txt on
   standard Linux, or $TMPDIR/bullwhip-results/{tool_use_id}.txt on Termux)
...
STORAGE_DIR = "/tmp/bullwhip-results"
HEREDOC_MARKER = "BULLWHIP_PERSIST_EOF"
        ...
        if callable(get_temp_dir):
            try:
                ...
                if temp_dir:
                    ...
                    return f"{temp_dir}/bullwhip-results"
    return STORAGE_DIR
...
def _heredoc_marker(content: str) -> str:
    ...
    return f"BULLWHIP_PERSIST_{uuid.uuid4().hex[:8]}"
