...
Architecture:
    ~/.bullwhip/checkpoints/{sha256(abs_dir)[:16]}/   — shadow git repo
        HEAD, refs/, objects/                        — standard git internals
        BULLWHIP_WORKDIR                               — original dir path
...
CHECKPOINT_BASE = get_bullwhip_home() / "checkpoints"
...
_GIT_TIMEOUT: int = max(10, min(60, int(os.getenv("BULLWHIP_CHECKPOINT_TIMEOUT", "30"))))
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    _run_git(["config", "user.email", "bullwhip@local"], shadow_repo, working_dir)
    _run_git(["config", "user.name", "BullWhip Checkpoint"], shadow_repo, working_dir)
    ...
    (shadow_repo / "BULLWHIP_WORKDIR").write_text(
