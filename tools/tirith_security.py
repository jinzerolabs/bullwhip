...
it is automatically downloaded from GitHub releases to $BULLWHIP_HOME/bin/tirith.
...
def _get_bullwhip_home() -> str:
    """Return the Bull Whip home directory, respecting BULLWHIP_HOME env var."""
    return str(get_bullwhip_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_bullwhip_home(), ".tirith-install-failed")
...
def bullwhip_bin_dir() -> str:
    """Return $BULLWHIP_HOME/bin, creating it if needed."""
    d = os.path.join(_get_bullwhip_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $BULLWHIP_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(bullwhip_bin_dir(), "tirith")
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $BULLWHIP_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $BULLWHIP_HOME/bin/tirith
    ...
    bullwhip_bin = os.path.join(bullwhip_bin_dir(), "tirith")
    if os.path.isfile(bullwhip_bin) and os.access(bullwhip_bin, os.X_OK):
        _resolved_path = bullwhip_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return bullwhip_bin
