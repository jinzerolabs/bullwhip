...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── BullWhip internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files.
        from bullwhip_constants import get_bullwhip_home as _get_hh
        _bullwhip_home = _get_hh().resolve()
        _blocked_dirs = [
            _bullwhip_home / "skills" / ".hub" / "index-cache",
            _bullwhip_home / "skills" / ".hub",
        ]
        for _blocked in _blocked_dirs:
            try:
                ...
                return json.dumps({
                    "error": (
                        f"Access denied: {path} is an internal BullWhip cache file "
