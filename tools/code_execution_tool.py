...
Lets the LLM write a Python script that calls BullWhip tools via RPC,
...
# ---------------------------------------------------------------------------
# bullwhip_tools.py code generator
# ---------------------------------------------------------------------------
...
def generatebullwhip_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the bullwhip_tools.py stub module.
...
"""Auto-generated BullWhip tools RPC stubs."""
...
def _connect():
    ...
    if _sock is None:
        ...
        _sock.connect(os.environ["BULLWHIP_RPC_SOCKET"])
...
"""Auto-generated BullWhip tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, time

_RPC_DIR = os.environ.get("BULLWHIP_RPC_DIR") or os.path.join(tempfile.gettempdir(), "hermes_rpc")
_seq = 0
''' + _COMMON_HELPERS + '''\
...
def _execute_remote(
    ...
    try:
        ...
        tools_src = generatebullwhip_tools_module(
        ...
        env_prefix = (
            f"BULLWHIP_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("BULLWHIP_TIMEZONE", "").strip()
        if tz:
            env_prefix += f" TZ={tz}"
...
def execute_code(
    ...
    to a subset of Bull Whip tools.
    ...
    sock_path = os.path.join(_sock_tmpdir, f"bullwhip_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        ...
        tools_src = generatebullwhip_tools_module(list(sandbox_tools))
        ...
        for k, v in os.environ.items():
            ...
            if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
                child_env[k] = v
        child_env["BULLWHIP_RPC_SOCKET"] = sock_path
        child_env["PYTHONDONTWRITEBYTECODE"] = "1"
        # Ensure the bullwhip-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.
        bullwhip_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        child_env["PYTHONPATH"] = bullwhip_root + (os.pathsep + _existing_pp if _existing_pp else "")
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.
        _tz_name = os.getenv("BULLWHIP_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name

        # Per-profile HOME isolation: redirect system tool configs into
        # {BULLWHIP_HOME}/home/ when that directory exists.
        from bullwhip_constants import get_subprocess_home
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.bullwhip/.env')).
...
def build_execute_code_schema(enabled_sandbox_tools: set = None) -> dict:
    ...
    When tools are disabled via ``bullwhip tools`` (e.g. web is turned off),
    ...
    description = (
        "Run a Python script that can call BullWhip tools programmatically. "
        ...
        "Also available (no import needed — built into bullwhip_tools):\n"
