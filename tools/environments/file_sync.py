...
_FORCE_SYNC_ENV = "BULLWHIP_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.bullwhip") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.bullwhip to *container_base* because the remote
    ...
    for entry in get_credential_file_mounts():
        remote = entry["container_path"].replace(
            "/root/.bullwhip", container_base, 1
        )
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``BULLWHIP_FORCE_FILE_SYNC=1`` is set.
