...
from tools.environments.local import _BULLWHIP_PROVIDER_ENV_BLOCKLIST
...
def _loadbullwhip_env_vars() -> dict[str, str]:
    """Load ~/.bullwhip/.env values without failing Docker command execution."""
    try:
        from bullwhip_cli.config import load_env
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.bullwhip/sandboxes/). Non-persistent
        ...
        container_name = f"bullwhip-{uuid.uuid4().hex[:8]}"
        run_cmd = [
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic BullWhip secret blocklist. Only implicit passthrough
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _BULLWHIP_PROVIDER_ENV_BLOCKLIST)
        hermes_env = _loadbullwhip_env_vars() if forward_keys else {}
