"""Base class for all BullWhip execution environment backends.
...
def get_sandbox_dir() -> Path:
    ...
    Configurable via TERMINAL_SANDBOX_DIR. Defaults to {BULLWHIP_HOME}/sandboxes/.
    """
    custom = os.getenv("TERMINAL_SANDBOX_DIR")
    if custom:
        p = Path(custom)
    else:
        p = get_bullwhip_home() / "sandboxes"
...
def _cwd_marker(session_id: str) -> str:
    return f"__BULLWHIP_CWD_{session_id}__"
...
class BaseEnvironment(ABC):
    """Common interface and unified execution flow for all BullWhip backends.
    ...
    def __init__(self, cwd: str, timeout: int, env: dict = None):
        ...
        self._snapshot_path = f"{temp_dir}/bullwhip-snap-{self._session_id}.sh"
        self._cwd_file = f"{temp_dir}/bullwhip-cwd-{self._session_id}.txt"
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        parts.append("exit $_bullwhip_ec")
    ...
    def _embed_stdin_heredoc(command: str, stdin_data: str) -> str:
        ...
        delimiter = f"BULLWHIP_STDIN_{uuid.uuid4().hex[:12]}"
    ...
    def _extract_cwd_from_output(self, result: dict):
        """Parse the __BULLWHIP_CWD_{session}__ marker from stdout output.
