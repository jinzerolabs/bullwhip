"""
Cron job management tools for Bull Whip Agent.
...
def _origin_from_env() -> Optional[Dict[str, str]]:
    ...
    origin_platform = get_session_env("BULLWHIP_SESSION_PLATFORM")
    origin_chat_id = get_session_env("BULLWHIP_SESSION_CHAT_ID")
    if origin_platform and origin_chat_id:
        thread_id = get_session_env("BULLWHIP_SESSION_THREAD_ID") or None
        if thread_id:
            logger.debug(
        ...
        return {
            ...
            "chat_name": get_session_env("BULLWHIP_SESSION_CHAT_NAME") or None,
            "thread_id": thread_id,
        }
    return None
...
def _resolve_model_override(model_obj: Optional[Dict[str, Any]]) -> tuple:
    ...
    job doesn't drift when the user later changes their default via bullwhip model.
...
def _validate_cron_script_path(script: Optional[str]) -> Optional[str]:
    ...
    Scripts must be relative paths that resolve within BULLWHIP_HOME/scripts/.
    ...
    # Only relative paths within ~/.bullwhip/scripts/ are allowed.
    if raw.startswith(("/", "~")) or (len(raw) >= 2 and raw[1] == ":"):
        return (
            f"Script path must be relative to ~/.bullwhip/scripts/. "
            f"Got absolute or home-relative path: {raw!r}. "
            f"Place scripts in ~/.bullwhip/scripts/ and use just the filename."
        )
...
Important safety rule: cron-run sessions should not recursively schedule more cron jobs.""",
    "parameters": {
        ...
        "properties": {
            ...
            "script": {
                "type": "string",
                "description": "Optional path to a Python script that runs before each cron job execution. Its stdout is injected into the prompt as context. Use for data collection and change detection. Relative paths resolve under ~/.bullwhip/scripts/. On update, pass empty string to clear."
            },
        },
        "required": ["action"]
    }
...
def check_cronjob_requirements() -> bool:
    ...
    """
    return bool(
        os.getenv("BULLWHIP_INTERACTIVE")
        or os.getenv("BULLWHIP_GATEWAY_SESSION")
        or os.getenv("BULLWHIP_EXEC_ASK")
    )
