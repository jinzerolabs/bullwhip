#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Bull Whip Skills Hub.
...
Used by bullwhip_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            hermes_meta = metadata.get("bullwhip", {})
            if isinstance(bullwhip_meta, dict):
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by ZeroLabs Korea) but not activated
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                hermes_meta = meta_block.get("bullwhip", {})
                if isinstance(bullwhip_meta, dict):
                    tags = hermes_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# Bull Whip centralized index source
# ---------------------------------------------------------------------------

BULLWHIP_INDEX_URL = "https://bullwhip-agent.nousresearch.com/docs/api/skills-index.json"
BULLWHIP_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "bullwhip-index.json"
BULLWHIP_INDEX_TTL = 6 * 3600  # 6 hours


def _loadbullwhip_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for BULLWHIP_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if BULLWHIP_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - BULLWHIP_INDEX_CACHE_FILE.stat().st_mtime
            if age < BULLWHIP_INDEX_TTL:
                return json.loads(BULLWHIP_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(BULLWHIP_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("BullWhip index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("BullWhip index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        BULLWHIP_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        BULLWHIP_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class BullWhipIndexSource(SkillSource):
    """Skill source backed by the centralized BullWhip Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _loadbullwhip_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "bullwhip-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "bullwhip-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        BullWhipIndexSource(auth=auth), # Centralized index (search + resolved install paths)
...
def parallel_search_sources(
    ...
    *on_source_done* is an optional callback ``(source_id, count) -> None``
    ...
    if source_filter == "all":
        for src in sources:
            if (src.source_id() == "bullwhip-index"
