...
This module provides the glue:
    - ``BullWhipTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Bull Whip Agent"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses BULLWHIP_HOME so each profile gets its own OAuth tokens.
    Layout: ``BULLWHIP_HOME/mcp-tokens/``
    """
    try:
        from bullwhip_constants import get_bullwhip_home
        base = Path(get_bullwhip_home())
    except ImportError:
        base = Path(os.environ.get("BULLWHIP_HOME", str(Path.home() / ".bullwhip")))
...
# ---------------------------------------------------------------------------
# Bull WhipTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class BullWhipTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        BULLWHIP_HOME/mcp-tokens/<server_name>.json         -- tokens
        BULLWHIP_HOME/mcp-tokens/<server_name>.client.json   -- client info
    """
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Bull Whip.</p></body></html>"
            ) if code else (
...
def remove_oauth_tokens(server_name: str) -> None:
    ...
    storage = BullWhipTokenStorage(server_name)
    storage.remove()
...
def build_oauth_auth(
    ...
    # --- Client metadata ---
    client_name = cfg.get("client_name", "Bull Whip Agent")
    scope = cfg.get("scope")
