...
Configuration is loaded from ~/.bullwhip/config.yaml under the 'tts:' key.
...
def _get_default_output_dir() -> str:
    ...
    return str(getbullwhip_dir("cache/audio", "audio_cache"))
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.bullwhip/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.bullwhip/config.yaml.
    ...
    try:
        ...
        logger.debug("bullwhip_cli.config not available, using default TTS config")
        return {}
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.bullwhip/config.yaml (tts: section).
    ...
    platform = get_session_env("BULLWHIP_SESSION_PLATFORM", "").lower()
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run: pip install 'bullwhip-agent[mistral]'"
                }, ensure_ascii=False)
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run bullwhip setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": "Optional custom file path to save the audio. Defaults to ~/.bullwhip/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }
