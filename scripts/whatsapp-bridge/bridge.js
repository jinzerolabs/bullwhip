#!/usr/bin/env node
/**
 * Bull Whip Agent WhatsApp Bridge
 *
 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.bullwhip/whatsapp/session
 */
...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.bullwhip', 'whatsapp', 'session'));
const IMAGE_CACHE_DIR = path.join(process.env.HOME || '~', '.bullwhip', 'image_cache');
const DOCUMENT_CACHE_DIR = path.join(process.env.HOME || '~', '.bullwhip', 'document_cache');
const AUDIO_CACHE_DIR = path.join(process.env.HOME || '~', '.bullwhip', 'audio_cache');
const PAIR_ONLY = args.includes('--pair-only');
const WHATSAPP_MODE = getArg('mode', process.env.WHATSAPP_MODE || 'self-chat'); // "bot" or "self-chat"
const ALLOWED_USERS = parseAllowedUsers(process.env.WHATSAPP_ALLOWED_USERS || '');
const DEFAULT_REPLY_PREFIX = '🐂 *Bull Whip Agent*\n────────────\n';
...
async function startSocket() {
  ...
  sock = makeWASocket({
    ...
    browser: ['Bull Whip Agent', 'Chrome', '120.0'],
