#!/usr/bin/env python3
"""Bull Whip Agent Release Script
...
VERSION_FILE = REPO_ROOT / "bullwhip_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "bullwhip@nousresearch.com": "NousResearch",
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "bullwhip@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/bullwhip-agent",
    ...
    lines.append(f"# Bull Whip Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Bull Whip Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Bull Whip Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Bull Whip Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Bull Whip Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Bull Whip Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Bull Whip Agent v{new_version} ({calver_date})' "
