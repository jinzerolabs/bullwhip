#!/bin/bash
# ============================================================================
# Bull Whip Agent Installer
# ============================================================================
# Installation script for Linux, macOS, and Android/Termux.
# Uses uv for desktop/server installs and Python's stdlib venv + pip on Termux.
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/NousResearch/bullwhip-agent/main/scripts/install.sh | bash
#
# Or with options:
...
# Configuration
REPO_URL_SSH="git@github.com:NousResearch/bullwhip-agent.git"
REPO_URL_HTTPS="https://github.com/NousResearch/bullwhip-agent.git"
BULLWHIP_HOME="$HOME/.bullwhip"
INSTALL_DIR="${BULLWHIP_INSTALL_DIR:-$BULLWHIP_HOME/bullwhip-agent}"
PYTHON_VERSION="3.11"
NODE_VERSION="22"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --dir)
            ...
            ;;
        -h|--help)
            echo "Bull Whip Agent Installer"
            echo ""
            ...
            echo "  --dir PATH     Installation directory (default: ~/.bullwhip/bullwhip-agent)"
...
get_hermes_command_path() {
    ...
    if [ -x "$link_dir/bullwhip" ]; then
        echo "$link_dir/bullwhip"
    else
        echo "bullwhip"
    fi
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  irm https://raw.githubusercontent.com/NousResearch/bullwhip-agent/main/scripts/install.ps1 | iex"
            exit 1
            ;;
        *)
            OS="unknown"
...
check_node() {
    ...
    if [ -x "$BULLWHIP_HOME/node/bin/node" ]; then
        export PATH="$BULLWHIP_HOME/node/bin:$PATH"
        local found_ver=$("$BULLWHIP_HOME/node/bin/node" --version)
...
install_node() {
    ...
    log_info "Extracting to ~/.bullwhip/node/..."
    ...
    # Place into ~/.bullwhip/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$BULLWHIP_HOME/node"
    mkdir -p "$BULLWHIP_HOME"
    mv "$extracted_dir" "$BULLWHIP_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$BULLWHIP_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$BULLWHIP_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$BULLWHIP_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$BULLWHIP_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$BULLWHIP_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.bullwhip/node/"
    HAS_NODE=true
}
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Bull Whip Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="bullwhip-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
...
setup_path() {
    log_info "Setting up bullwhip command..."

    if [ "$USE_VENV" = true ]; then
        BULLWHIP_BIN="$INSTALL_DIR/venv/bin/bullwhip"
    else
        BULLWHIP_BIN="$(which bullwhip 2>/dev/null || echo "")"
        if [ -z "$BULLWHIP_BIN" ]; then
            log_warn "bullwhip not found on PATH after install"
            return 0
        fi
    fi

    # Verify the entry point script was actually generated
    if [ ! -x "$BULLWHIP_BIN" ]; then
        log_warn "bullwhip entry point not found at $BULLWHIP_BIN"
    ...
    # Create a user-facing shim for the bullwhip command.
    mkdir -p "$command_link_dir"
    ln -sf "$BULLWHIP_BIN" "$command_link_dir/bullwhip"
    log_success "Symlinked bullwhip → $command_link_display_dir/bullwhip"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "bullwhip command ready"
        return 0
    fi
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Bull Whip Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Bull Whip Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so bullwhip works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.bullwhip directory structure (config at top level, code in subdir)
    mkdir -p "$BULLWHIP_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills,whatsapp/session}

    # Create .env at ~/.bullwhip/.env (top level, easy to find)
    if [ ! -f "$BULLWHIP_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$BULLWHIP_HOME/.env"
            log_success "Created ~/.bullwhip/.env from template"
        else
            touch "$BULLWHIP_HOME/.env"
            log_success "Created ~/.bullwhip/.env"
        fi
    else
        log_info "~/.bullwhip/.env already exists, keeping it"
    fi

    # Create config.yaml at ~/.bullwhip/config.yaml (top level, easy to find)
    if [ ! -f "$BULLWHIP_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$BULLWHIP_HOME/config.yaml"
            log_success "Created ~/.bullwhip/config.yaml from template"
        fi
    else
        log_info "~/.bullwhip/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$BULLWHIP_HOME/SOUL.md" ]; then
        cat > "$BULLWHIP_HOME/SOUL.md" << 'SOUL_EOF'
# Bull Whip Agent Persona
...
-->
SOUL_EOF
        log_success "Created ~/.bullwhip/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.bullwhip/"

    # Seed bundled skills into ~/.bullwhip/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.bullwhip/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.bullwhip/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$BULLWHIP_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$BULLWHIP_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.bullwhip/skills/"
        fi
    fi
}
...
run_setup_wizard() {
    ...
    if ! [ -e /dev/tty ]; then
        log_info "Setup wizard skipped (no terminal available). Run 'bullwhip setup' after install."
        return 0
    fi
    ...
    # Run bullwhip setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m bullwhip_cli.main setup < /dev/tty
    else
        python -m bullwhip_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$BULLWHIP_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Bull Whip to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$BULLWHIP_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'bullwhip whatsapp' to pair via QR code..."
            echo ""
            read -p "Pair WhatsApp now? [Y/n] " -n 1 -r
            echo
            if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
                BULLWHIP_CMD="$(get_hermes_command_path)"
                $BULLWHIP_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'bullwhip whatsapp' to pair."
        fi
    fi

    if ! [ -e /dev/tty ]; then
        log_info "Gateway setup skipped (no terminal available). Run 'bullwhip gateway install' later."
        return 0
    fi
    ...
    if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $BULLWHIP_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $BULLWHIP_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: bullwhip gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: bullwhip gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $BULLWHIP_CMD gateway > "$BULLWHIP_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.bullwhip/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: bullwhip gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: bullwhip gateway"
    fi
}
...
print_success() {
    ...
    # Show file locations
    echo -e "${CYAN}${BOLD}📁 Your files (all in ~/.bullwhip/):${NC}"
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    ~/.bullwhip/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  ~/.bullwhip/.env"
    echo -e "   ${YELLOW}Data:${NC}      ~/.bullwhip/cron/, sessions/, logs/"
    echo -e "   ${YELLOW}Code:${NC}      ~/.bullwhip/bullwhip-agent/"
    echo ""
    ...
    echo ""
    echo -e "   ${GREEN}bullwhip${NC}              Start chatting"
    echo -e "   ${GREEN}bullwhip setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}bullwhip config${NC}       View/edit configuration"
    echo -e "   ${GREEN}bullwhip config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}bullwhip gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}bullwhip update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'bullwhip' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'bullwhip' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
