# Migrating from OpenClaw to Bull Whip Agent

This guide covers how to import your OpenClaw settings, memories, skills, and API keys into Bull Whip Agent.

## Three Ways to Migrate

### 1. Automatic (during first-time setup)

When you run `bullwhip setup` for the first time and Bull Whip detects `~/.openclaw`, it automatically offers to import your OpenClaw data before configuration begins. Just accept the prompt and everything is handled for you.

### 2. CLI Command (quick, scriptable)

```bash
bullwhip claw migrate                      # Preview then migrate (always shows preview first)
bullwhip claw migrate --dry-run            # Preview only, no changes
bullwhip claw migrate --preset user-data   # Migrate without API keys/secrets
bullwhip claw migrate --yes                # Skip confirmation prompt
```
...
```
> Migrate my OpenClaw setup to Bull Whip
```
...
|------|--------|-------------|
| SOUL.md | `~/.openclaw/workspace/SOUL.md` | `~/.bullwhip/SOUL.md` |
| Memory entries | `~/.openclaw/workspace/MEMORY.md` | `~/.bullwhip/memories/MEMORY.md` |
| User profile | `~/.openclaw/workspace/USER.md` | `~/.bullwhip/memories/USER.md` |
| Skills | `~/.openclaw/workspace/skills/` | `~/.bullwhip/skills/openclaw-imports/` |
| Command allowlist | `~/.openclaw/workspace/exec_approval_patterns.yaml` | Merged into `~/.bullwhip/config.yaml` |
| Messaging settings | `~/.openclaw/config.yaml` (TELEGRAM_ALLOWED_USERS, MESSAGING_CWD) | `~/.bullwhip/.env` |
| TTS assets | `~/.openclaw/workspace/tts/` | `~/.bullwhip/tts/` |
...
| Item | Source | Destination |
|------|--------|-------------|
| Telegram bot token | `openclaw.json` channels config | `~/.bullwhip/.env` |
| OpenRouter API key | `.env`, `openclaw.json`, or `openclaw.json["env"]` | `~/.bullwhip/.env` |
| OpenAI API key | `.env`, `openclaw.json`, or `openclaw.json["env"]` | `~/.bullwhip/.env` |
| Anthropic API key | `.env`, `openclaw.json`, or `openclaw.json["env"]` | `~/.bullwhip/.env` |
| ElevenLabs API key | `.env`, `openclaw.json`, or `openclaw.json["env"]` | `~/.bullwhip/.env` |
...
- **TTS provider**: OpenClaw renamed "edge" to "microsoft" — both are recognized and mapped to Bull Whip' "edge"
...
By default, the migration **will not overwrite** existing Bull Whip data:

- **SOUL.md** — skipped if one already exists in `~/.bullwhip/`
...
- **API keys** — skipped if the key is already set in `~/.bullwhip/.env`
...
For executed migrations, the full report is saved to `~/.bullwhip/migration/openclaw/<timestamp>/`.

## Post-Migration Notes

- **Skills require a new session** — imported skills take effect after restarting your agent or starting a new chat.
- **WhatsApp requires re-pairing** — WhatsApp uses QR-code pairing, not token-based auth. Run `bullwhip whatsapp` to pair.
- **Archive cleanup** — after migration, you'll be offered to rename `~/.openclaw/` to `.openclaw.pre-migration/` to prevent state confusion. You can also run `bullwhip claw cleanup` later.
...
```bash
bullwhip claw migrate --source /path/to/.openclaw
```

### "Migration script not found"
The migration script ships with Bull Whip Agent. If you installed via pip (not git clone), the `optional-skills/` directory may not be present. Install the skill from the Skills Hub:
```bash
bullwhip skills install openclaw-migration
```

### Memory overflow
If your OpenClaw MEMORY.md or USER.md exceeds Bull Whip' character limits, excess entries are exported to an overflow file in the migration report directory. You can manually review and add the most important ones.
...
The migration checks all four. If keys use `source: "file"` or `source: "exec"` SecretRefs, they can't be resolved automatically — add them via `bullwhip config set`.
