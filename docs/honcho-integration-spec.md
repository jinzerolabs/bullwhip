...
Comparison of Bull Whip Agent vs. openclaw-honcho — and a porting spec for bringing Bull Whip patterns into other Honcho integrations.

---

## Overview

Two independent Honcho integrations have been built for two different agent runtimes: **Bull Whip Agent** (Python, baked into the runner) and **openclaw-honcho** (TypeScript plugin via hook/tool API). Both use the same Honcho peer paradigm — dual peer model, `session.context()`, `peer.chat()` — but they made different tradeoffs at every layer.

This document maps those tradeoffs and defines a porting spec: a set of Bull Whip-originated patterns, each stated as an integration-agnostic interface, that any Honcho integration can adopt regardless of runtime or language.

> **Scope** Both integrations work correctly today. This spec is about the delta — patterns in Bull Whip that are worth propagating and patterns in openclaw-honcho that Bull Whip should eventually adopt. The spec is additive, not prescriptive.

---

## Architecture comparison

### Bull Whip: baked-in runner
...
| Dimension | Bull Whip Agent | openclaw-honcho |
|---|---|---|
...
| **AI peer name in identity** | Replaces "Bull Whip Agent" in DEFAULT_AGENT_IDENTITY when configured. | Not implemented. |
...
Six patterns from Bull Whip are worth adopting in any Honcho integration. Each is described as an integration-agnostic interface.

**Bull Whip contributes:**
...
**openclaw-honcho contributes back (Bull Whip should adopt):**
...
- [ ] **honcho mode / sessions / map** — CLI parity with Bull Whip
...
Greenfield integration. Start from openclaw-honcho's architecture and apply all Bull Whip patterns from day one.
