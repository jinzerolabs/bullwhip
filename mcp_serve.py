"""
BullWhip MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (BullWhip-specific extra)

Usage:
    bullwhip mcp serve
    bullwhip mcp serve --verbose
...
logger = logging.getLogger("bullwhip.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using BULLWHIP_HOME."""
    try:
        from bullwhip_constants import get_bullwhip_home
        return get_bullwhip_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("BULLWHIP_HOME", Path.home() / ".bullwhip")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from bullwhip_constants import get_bullwhip_home
        directory_file = get_bullwhip_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("BULLWHIP_HOME", Path.home() / ".bullwhip")
...
class EventBridge:
    ...
    This is the Bull Whip equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from bullwhip_constants import get_bullwhip_home
            db_file = get_bullwhip_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("BULLWHIP_HOME", Path.home() / ".bullwhip")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Bull Whip MCP server with all tools registered."""
    if not _MCP_SERVER_AVAILABLE:
        raise ImportError(
            "MCP server requires the 'mcp' package. "
            "Install with: pip install 'bullwhip-agent[mcp]'"
        )

    mcp = FastMCP(
        "bullwhip",
        instructions=(
            "Bull Whip Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Bull Whip MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
