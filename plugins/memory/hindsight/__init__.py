...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID   — memory bank identifier (default: bullwhip)
  HINDSIGHT_BUDGET    — recall budget: low/mid/high (default: mid)
  HINDSIGHT_API_URL   — API endpoint
  HINDSIGHT_MODE      — cloud or local (default: cloud)

Or via $BULLWHIP_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $BULLWHIP_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_bullwhip_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "bullwhip": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "bullwhip"),
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Bull Whip Agent and the User"
    ...
    def save_config(self, values, bullwhip_home):
        """Write config to $BULLWHIP_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(bullwhip_home) / "hindsight"
    ...
    def post_setup(self, bullwhip_home: str, config: dict) -> None:
        ...
        from bullwhip_cli.memory_setup import _curses_select
        ...
        provider_config["bank_id"] = "bullwhip"
        provider_config["recall_budget"] = "mid"
        bank_id = "bullwhip"
        config["memory"]["provider"] = "hindsight"
        save_config(config)

        self.save_config(provider_config, bullwhip_home)

        if env_writes:
            env_path = Path(bullwhip_home) / ".env"
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name", "default": "bullwhip"},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Bull Whip Agent and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "bullwhip"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "bullwhip"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = self._config.get("banks", {}).get("bullwhip", {})
        self._bank_id = self._config.get("bank_id") or banks.get("bankId", "bullwhip")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Bull Whip Agent and the User")
