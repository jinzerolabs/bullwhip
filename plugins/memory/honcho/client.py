...
Resolution order for config file:
  1. $BULLWHIP_HOME/honcho.json  (instance-local, enables isolated BullWhip instances)
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active BullWhip profile.

    Resolution order:
      1. BULLWHIP_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``bullwhip.<profile>``
      3. Fallback: ``"bullwhip"`` (default profile)
    """
    explicit = os.environ.get("BULLWHIP_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $BULLWHIP_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.bullwhip/honcho.json          (default profile — shared host blocks live here)
      3. ~/.honcho/config.json          (global, cross-app interop)

    Returns the global path if none exist (for first-time setup writes).
    """
    local_path = get_bullwhip_home() / "honcho.json"
    if local_path.exists():
        return local_path

    # Default profile's config — host blocks accumulate here via setup/clone
    default_path = Path.home() / ".bullwhip" / "honcho.json"
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into Bull Whip system prompt
    ...
    # True when Honcho was explicitly configured for this host (hosts.bullwhip
    ...
    def from_global_config(
        ...
        Resolution: $BULLWHIP_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active BullWhip profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.bullwhip block or explicit enabled flag means the user
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. BullWhip session title (from /title command)
          3. per-session strategy — BullWhip session_id ({timestamp}_{hex})
        ...
        # per-session: inherit BullWhip session_id (new Honcho session each run)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'bullwhip honcho setup' or set HONCHO_API_KEY. "
