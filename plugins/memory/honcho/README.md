...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/bullwhip>
...
```bash
bullwhip honcho setup    # full interactive wizard (cloud or local)
bullwhip memory setup    # generic picker, also works
```

Or manually:
```bash
bullwhip config set memory.provider honcho
echo "HONCHO_API_KEY=your-key" >> ~/.bullwhip/.env
```
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$BULLWHIP_HOME/honcho.json` | Profile-local (isolated Bull Whip instances) |
| 2 | `~/.bullwhip/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Bull Whip profile: `bullwhip` (default) or `bullwhip.<profile>`.
...
Host key derivation: `BULLWHIP_HONCHO_HOST` env > active profile (`bullwhip.<profile>`) > `"bullwhip"`.
...
| `BULLWHIP_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `bullwhip honcho setup` | Full interactive setup wizard |
| `bullwhip honcho status` | Show resolved config for active profile |
| `bullwhip honcho enable` / `disable` | Toggle Honcho for active profile |
| `bullwhip honcho mode <mode>` | Change recall or observation mode |
| `bullwhip honcho peer --user <name>` | Update user peer name |
| `bullwhip honcho peer --ai <name>` | Update AI peer name |
| `bullwhip honcho tokens --context <N>` | Set context token budget |
| `bullwhip honcho tokens --dialectic <N>` | Set dialectic max chars |
| `bullwhip honcho map <name>` | Map current directory to a session name |
| `bullwhip honcho sync` | Create host blocks for all Bull Whip profiles |
