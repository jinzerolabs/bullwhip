...
_DEFAULT_CONTAINER_TAG = "bullwhip"
...
def _load_supermemory_config(bullwhip_home: str) -> dict:
    config = _default_config()
    config_path = Path(bullwhip_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, bullwhip_home: str) -> None:
    config_path = Path(bullwhip_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `bullwhip memory setup`.
        # All other options are documented for $BULLWHIP_HOME/supermemory.json
    ...
    def save_config(self, values, bullwhip_home):
        ...
        _save_supermemory_config(sanitized, bullwhip_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from bullwhip_constants import get_bullwhip_home
        self._bullwhip_home = kwargs.get("bullwhip_home") or str(get_bullwhip_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._bullwhip_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "bullwhip", "type": "conversation_turn"}
