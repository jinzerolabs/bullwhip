...
Config file: `$BULLWHIP_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `bullwhip` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `bullwhip-{identity}` → `bullwhip-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per Bull Whip profile:

```json
{
  "container_tag": "bullwhip-{identity}"
}
```

For a profile named `coder`, this resolves to `bullwhip-coder`. The default profile resolves to `bullwhip-default`. Without `{identity}`, all profiles share the same container.
