"""Centralized logging setup for Bull Whip Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.bullwhip/logs/`` (profile-aware via ``get_bullwhip_home()``).
...
from bullwhip_constants import get_config_path, get_bullwhip_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "bullwhip_session_injector", False):
    ...
    _session_record_factory.bullwhip_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``bullwhip logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway",),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("bullwhip_cli", "cli"),
    "cron": ("cron",),
}
...
def setup_logging(
    *,
    ...
    """Configure the Bull Whip logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    bullwhip_home
        Override for the Bull Whip home directory.  Falls back to
        ``get_bullwhip_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "bullwhip_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler.bullwhip_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the bullwhip group. However, both _open() (initial creation) and
    ...
    def __init__(self, *args, **kwargs):
        from bullwhip_cli.config import is_managed
