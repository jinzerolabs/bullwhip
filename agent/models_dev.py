...
Data resolution order (like TypeScript OpenCode):
  ...
  2. Disk cache (~/.bullwhip/models_dev_cache.json)
...
# ---------------------------------------------------------------------------
# Provider ID mapping: BullWhip ↔ models.dev
# ---------------------------------------------------------------------------

# Bull Whip provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → BullWhip (built lazily)
...
def _get_cache_path() -> Path:
    ...
    return get_bullwhip_home() / "models_dev_cache.json"
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a BullWhip provider ID to its models dict from models.dev.
...
def get_provider_info(provider_id: str) -> Optional[ProviderInfo]:
    ...
    Accepts either a BullWhip provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve BullWhip ID → models.dev ID
...
def get_model_info(
    ...
    Accepts BullWhip or models.dev provider ID.  Tries exact match then
