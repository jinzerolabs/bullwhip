...
from bullwhip_constants import get_config_path, get_skills_dir
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``BULLWHIP_PLATFORM`` or
            ``BULLWHIP_SESSION_PLATFORM`` env vars.  Falls back to the
    ...
    resolved_platform = (
        platform
        or os.getenv("BULLWHIP_PLATFORM")
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.bullwhip/skills/`` are silently skipped.
    """
    config_path = get_config_path()
    if not config_path.exists():
        return []
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.bullwhip/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    bullwhip = metadata.get("bullwhip") or {}
    if not isinstance(bullwhip, dict):
        bullwhip = {}
    return {
        "fallback_for_toolsets": bullwhip.get("fallback_for_toolsets", []),
        "requires_toolsets": bullwhip.get("requires_toolsets", []),
        "fallback_for_tools": bullwhip.get("fallback_for_tools", []),
        "requires_tools": bullwhip.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    bullwhip = metadata.get("bullwhip")
    if not isinstance(bullwhip, dict):
        return []
    raw = bullwhip.get("config")
    if not raw:
        return []
