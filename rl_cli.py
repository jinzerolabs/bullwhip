...
if tinker_atropos_dir.exists():
    ...
    os.environ['BULLWHIP_QUIET'] = '1'  # Disable temp subdirectory creation
    print(f"📂 Terminal working directory: {tinker_atropos_dir}")
else:
    # Fall back to bullwhip-agent directory if submodule not found
...
from bullwhip_constants import get_bullwhip_home, OPENROUTER_BASE_URL

DEFAULT_MODEL = "anthropic/claude-opus-4.5"
DEFAULT_BASE_URL = OPENROUTER_BASE_URL


def loadbullwhip_config() -> dict:
    """
...
def main(
    ...
    Args:
        ...
        model: Model to use for the agent (reads from ~/.bullwhip/config.yaml if not provided)
    ...
    """
    # Load config from ~/.bullwhip/config.yaml
    config = loadbullwhip_config()
    ...
    if check_server:
        ...
        if ok:
            ...
            if missing:
                ...
                print("   Add them to ~/.bullwhip/.env")
            else:
