...
title: "Git Worktrees"
description: "Run multiple Bull Whip agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Bull Whip Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Bull Whip so each session has a clean, isolated working directory.

## Why Use Worktrees with Bull Whip?

Bull Whip treats the **current working directory** as the project root:

- CLI: the directory where you run `bullwhip` or `bullwhip chat`
...
git worktree add ../repo-feature feature/bullwhip-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/bullwhip-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Bull Whip there:

```bash
cd ../repo-feature

# Start Bull Whip in the worktree
bullwhip
```
...
git worktree add ../repo-experiment-a feature/bullwhip-a
git worktree add ../repo-experiment-b feature/bullwhip-b
```
...
Each Bull Whip process:

- Works on its own branch (`feature/bullwhip-a` vs `feature/bullwhip-b`).
...
- Bull Whip checkpoint data under `~/.bullwhip/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Bull Whip experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/bullwhip-checkpoints-docs`, `feature/bullwhip-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Bull Whip from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `bullwhip -w` (Automatic Worktree Mode)

Bull Whip has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `bullwhip/bullwhip-<hash>`).
...
```bash
bullwhip -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `bullwhip -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Bull Whip session its own clean checkout.
