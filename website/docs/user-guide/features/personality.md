...
description: "Customize Bull Whip Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Bull Whip Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `BULLWHIP_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Bull Whip is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Bull Whip now seeds a default `SOUL.md` automatically in:

```text
~/.bullwhip/SOUL.md
```

More precisely, it uses the current instance's `BULLWHIP_HOME`, so if you run Bull Whip with a custom home directory, it will use:

```text
$BULLWHIP_HOME/SOUL.md
```

### Important behavior

- **SOUL.md is the agent's primary identity.** It occupies slot #1 in the system prompt, replacing the hardcoded default identity.
- Bull Whip creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Bull Whip loads `SOUL.md` only from `BULLWHIP_HOME`
- Bull Whip does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Bull Whip falls back to a built-in default identity
...
If Bull Whip loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `BULLWHIP_HOME`, the personality belongs to the Bull Whip instance itself.

That also makes it easier to teach users:
- "Edit `~/.bullwhip/SOUL.md` to change Bull Whip' default personality."
...
- how Bull Whip should handle uncertainty, disagreement, or ambiguity
...
## What Bull Whip injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Bull Whip falls back to a built-in default identity ("You are Bull Whip Agent, an intelligent AI assistant created by ZeroLabs Korea..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Bull Whip ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Bull Whip, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Bull Whip its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.bullwhip/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.bullwhip/SOUL.md`
...
- [SOUL.md Guide](/docs/guides/use-soul-with-bullwhip)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Bull Whip speaks
- `display.skin` and `/skin` affect how Bull Whip looks in the terminal
