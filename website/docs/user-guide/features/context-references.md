...
Type `@` followed by a reference to inject content directly into your message. Bull Whip expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Bull Whip env: `$BULLWHIP_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$BULLWHIP_HOME/skills/.hub/`
