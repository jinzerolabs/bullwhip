...
description: "Connect Bull Whip Agent to external tool servers via MCP — and control exactly which MCP tools Bull Whip loads"
---

# MCP (Model Context Protocol)

MCP lets Bull Whip Agent connect to external tool servers so the agent can use tools that live outside Bull Whip itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.

If you have ever wanted Bull Whip to use a tool that already exists somewhere else, MCP is usually the cleanest way to do it.

## What MCP gives you

- Access to external tool ecosystems without writing a native Bull Whip tool first
- Local stdio servers and remote HTTP MCP servers in the same config
- Automatic tool discovery and registration at startup
- Utility wrappers for MCP resources and prompts when supported by the server
- Per-server filtering so you can expose only the MCP tools you actually want Bull Whip to see
...
```bash
cd ~/.bullwhip/bullwhip-agent
uv pip install -e ".[mcp]"
```

2. Add an MCP server to `~/.bullwhip/config.yaml`:
...
4. Ask Bull Whip to use the MCP-backed capability.
...
Bull Whip will discover the MCP server's tools and use them like any other tool.
...
HTTP MCP servers are remote endpoints Bull Whip connects to directly.
...
- you do not want Bull Whip spawning a local subprocess for that integration

## Basic configuration reference

Bull Whip reads MCP config from `~/.bullwhip/config.yaml` under `mcp_servers`.
...
| `enabled` | bool | If `false`, Bull Whip skips the server entirely |
...
## How Bull Whip registers MCP tools

Bull Whip prefixes MCP tools so they do not collide with built-in names:
...
In practice, you usually do not need to call the prefixed name manually — Bull Whip sees the tool and chooses it during normal reasoning.

## MCP utility tools

When supported, Bull Whip also registers utility tools around MCP resources and prompts:
...
- Bull Whip only registers resource utilities if the MCP session actually supports resource operations
- Bull Whip only registers prompt utilities if the MCP session actually supports prompt operations

So a server that exposes callable tools but no resources/prompts will not get those extra wrappers.

## Per-server filtering

You can control which tools each MCP server contributes to Bull Whip, allowing fine-grained management of your tool namespace.
...
If `enabled: false`, Bull Whip skips the server completely and does not even attempt a connection.
...
You can also separately disable Bull Whip-added utility wrappers:
...
If your config filters out all callable tools and disables or omits all supported utilities, Bull Whip does not create an empty runtime MCP toolset for that server.
...
Bull Whip discovers MCP servers at startup and registers their tools into the normal tool registry.

### Dynamic Tool Discovery

MCP servers can notify Bull Whip when their available tools change at runtime by sending a `notifications/tools/list_changed` notification. When Bull Whip receives this notification, it automatically re-fetches the server's tool list and updates the registry — no manual `/reload-mcp` required.
...
For stdio servers, Bull Whip does not blindly pass your full shell environment.
...
Then verify your config and restart Bull Whip.
...
Because Bull Whip now only registers those wrappers when both are true:
...
MCP servers can request LLM inference from Bull Whip via the `sampling/createMessage` protocol. This allows an MCP server to ask Bull Whip to generate text on its behalf — useful for servers that need LLM capabilities but don't have their own model access.
...
## Running Bull Whip as an MCP server

In addition to connecting **to** MCP servers, Bull Whip can also **be** an MCP server. This lets other MCP-capable agents (Claude Code, Cursor, Codex, or any MCP client) use Bull Whip's messaging capabilities — list conversations, read message history, and send messages across all your connected platforms.

### When to use this

- You want Claude Code, Cursor, or another coding agent to send and read Telegram/Discord/Slack messages through Bull Whip
- You want a single MCP server that bridges to all of Bull Whip's connected messaging platforms at once
- You already have a running Bull Whip gateway with connected platforms
...
Add Bull Whip to your MCP client config. For example, in Claude Code's `~/.claude/claude_desktop_config.json`:
...
Or if you installed Bull Whip in a specific location:

```json
{
  "mcpServers": {
    "bullwhip": {
      "command": "/home/user/.bullwhip/bullwhip-agent/venv/bin/bullwhip",
      "args": ["mcp", "serve"]
    }
  }
}
...
The MCP server exposes 10 tools, matching OpenClaw's channel bridge surface plus a Bull Whip-specific channel browser:
...
The MCP server includes a live event bridge that polls Bull Whip's session database for new messages. This gives MCP clients near-real-time awareness of incoming conversations:
...
```bash
bullwhip mcp serve              # Normal mode
bullwhip mcp serve --verbose    # Debug logging on stderr
```

### How it works

The MCP server reads conversation data directly from Bull Whip's session store (`~/.bullwhip/sessions/sessions.json` and the SQLite database). A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same `send_message` infrastructure as the Bull Whip agent itself.
