...
Bull Whip has two hook systems that run custom code at key lifecycle points:

| System | Registered via | Runs in | Use case |
|--------|---------------|---------|----------|
| **[Gateway hooks](#gateway-event-hooks)** | `HOOK.yaml` + `handler.py` in `~/.bullwhip/hooks/` | Gateway only | Logging, alerts, webhooks |
...
Each hook is a directory under `~/.bullwhip/hooks/` containing two files:
...
LOG_FILE = Path.home() / ".bullwhip" / "hooks" / "my-hook" / "activity.log"
...
The gateway ships with a built-in `boot-md` hook that looks for `~/.bullwhip/BOOT.md` on every startup. If the file exists, the agent runs its instructions in a background session. No installation needed — just create the file.

**Create `~/.bullwhip/BOOT.md`:**

```markdown
# Startup Checklist

1. Check if any cron jobs failed overnight — run `bullwhip cron list`
...
```yaml
# ~/.bullwhip/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.bullwhip/hooks/long-task-alert/handler.py
import os
import httpx
...
```yaml
# ~/.bullwhip/hooks/command-logger/HOOK.yaml
name: command-logger
...
```python
# ~/.bullwhip/hooks/command-logger/handler.py
import json
from datetime import datetime
from pathlib import Path

LOG = Path.home() / ".bullwhip" / "logs" / "command_usage.jsonl"
...
```yaml
# ~/.bullwhip/hooks/session-webhook/HOOK.yaml
...
```python
# ~/.bullwhip/hooks/session-webhook/handler.py
import httpx

WEBHOOK_URL = "https://your-service.example.com/bullwhip-events"
...
1. On gateway startup, `HookRegistry.discover_and_load()` scans `~/.bullwhip/hooks/`
...
**Where context is injected:** Always the **user message**, never the system prompt. This preserves the prompt cache — the system prompt stays identical across turns, so cached tokens are reused. The system prompt is Bull Whip's territory (model guidance, tool enforcement, personality, skills). Plugins contribute context alongside the user's input.
...
See the **[Build a Plugin guide](/docs/guides/build-a-bullwhip-plugin)** for the full walkthrough including tool schemas, handlers, and advanced hook patterns.
