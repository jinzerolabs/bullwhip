...
description: "Overview of Bull Whip Agent's tools — what's available, how toolsets work, and terminal backends"
---
...
Bull Whip ships with a broad built-in tool registry covering web search, browser automation, terminal execution, file editing, memory, delegation, RL training, messaging delivery, Home Assistant, and more.
...
bullwhip chat --toolsets "web,terminal"
...
See [Toolsets Reference](/docs/reference/toolsets-reference) for the full set, including platform presets such as `bullwhip-cli`, `bullwhip-telegram`, and dynamic MCP toolsets like `mcp-<server>`.
...
```
```bash
# Set credentials in ~/.bullwhip/.env
...
# Configure
bullwhip config set terminal.backend singularity
bullwhip config set terminal.singularity_image ~/python.sif
```
...
modal setup
bullwhip config set terminal.backend modal
```
...
If a command needs sudo, you'll be prompted for your password (cached for the session). Or set `SUDO_PASSWORD` in `~/.bullwhip/.env`.

:::warning
On messaging platforms, if sudo fails, the output includes a tip to add `SUDO_PASSWORD` to `~/.bullwhip/.env`.
:::
