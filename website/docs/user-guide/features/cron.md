...
Schedule tasks to run automatically with natural language or cron expressions. Bull Whip exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
:::warning
Cron-run sessions cannot recursively create more cron jobs. Bull Whip disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
bullwhip cron create "every 2h" "Check server status"
bullwhip cron create "every 1h" "Summarize new feed items" --skill blogwatcher
bullwhip cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill find-nearby \
  --name "Skill combo"
```
...
Ask Bull Whip normally:

```text
Every morning at 9am, check Hacker News for AI news and send me a summary on Telegram.
```

Bull Whip will use the unified `cronjob` tool internally.
...
```bash
bullwhip cron edit <job_id> --schedule "every 4h"
bullwhip cron edit <job_id> --prompt "Use the revised task"
bullwhip cron edit <job_id> --skill blogwatcher --skill find-nearby
bullwhip cron edit <job_id> --add-skill find-nearby
bullwhip cron edit <job_id> --remove-skill blogwatcher
bullwhip cron edit <job_id> --clear-skills
```
...
```bash
bullwhip cron list
bullwhip cron pause <job_id>
bullwhip cron resume <job_id>
bullwhip cron run <job_id>
bullwhip cron remove <job_id>
bullwhip cron status
bullwhip cron tick
```
...
sudo bullwhip gateway install --system   # Linux: boot-time system service for servers
bullwhip gateway             # Or run in foreground
...
On each tick Bull Whip:

1. loads jobs from `~/.bullwhip/cron/jobs.json`
...
A file lock at `~/.bullwhip/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.
...
| `"local"` | Save to local files only (`~/.bullwhip/cron/output/`) | Default on CLI |
...
If the agent's final response starts with `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.bullwhip/cron/output/`), but no message is sent to the delivery target.
...
Or set the `BULLWHIP_CRON_SCRIPT_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 120s default.
...
The agent's final response is automatically delivered — you do **not** need to include `send_message` in the cron prompt for that same destination. If a cron run calls `send_message` to the exact target the scheduler will already deliver to, Bull Whip skips that duplicate send and tells the model to put the user-facing content in the final response instead. Use `send_message` only for additional or different targets.
...
Jobs are stored in `~/.bullwhip/cron/jobs.json`. Output from job runs is saved to `~/.bullwhip/cron/output/{job_id}/{timestamp}.md`.
