...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Bull Whip Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Bull Whip Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Bull Whip](/docs/guides/use-voice-mode-with-bullwhip).

## Prerequisites

Before using voice features, make sure you have:

1. **Bull Whip Agent installed** — `pip install bullwhip-agent` (see [Installation](/docs/getting-started/installation))
2. **An LLM provider configured** — run `bullwhip model` or set your preferred provider credentials in `~/.bullwhip/.env`
3. **A working base setup** — run `bullwhip` to verify the agent responds to text before enabling voice

:::tip
The `~/.bullwhip/` directory and default `config.yaml` are created automatically the first time you run `bullwhip`. You only need to create `~/.bullwhip/.env` manually for API keys.
:::
...
pip install "bullwhip-agent[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
pip install "bullwhip-agent[all]"
```
...
```bash
bullwhip                # Start the interactive CLI
```
...
1. Start the CLI with `bullwhip` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.bullwhip/config.yaml` (default: `ctrl+b`).
:::
...
```bash
bullwhip gateway        # Start the gateway (connects to configured platforms)
bullwhip gateway setup  # Interactive setup wizard for first-time configuration
```
...
:::tip
To disable the mention requirement in server channels, add to `~/.bullwhip/.env`:
```bash
...
```bash
bullwhip gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.bullwhip/.env`
...
- Review gateway logs: `tail -f ~/.bullwhip/logs/gateway.log`
