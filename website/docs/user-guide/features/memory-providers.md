...
Bull Whip Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
bullwhip memory setup      # interactive picker + configuration
bullwhip memory status     # check what's active
bullwhip memory off        # disable external provider
```

You can also select the active memory provider via `bullwhip plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.bullwhip/config.yaml`:
...
When a memory provider is active, Bull Whip automatically:
...
**Setup Wizard:**
```bash
bullwhip honcho setup        # (legacy command) 
# or
bullwhip memory setup        # select "honcho"
```

**Config:** `$BULLWHIP_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$BULLWHIP_HOME/honcho.json` > `~/.bullwhip/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/bullwhip-ai/bullwhip-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/bullwhip).
...
:::tip Migrating from `bullwhip honcho`
If you previously used `bullwhip honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-agent / Profiles:**

Each Bull Whip profile gets its own Honcho AI peer while sharing the same workspace -- all profiles see the same user representation, but each agent builds its own identity and observations.

```bash
bullwhip profile create coder --clone   # creates honcho peer "coder", inherits config from default
```

What `--clone` does: creates a `bullwhip.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The peer is eagerly created in Honcho so it exists before first message.

For profiles created before Honcho was set up:

```bash
bullwhip honcho sync   # scans all profiles, creates host blocks for any missing ones
```

This inherits settings from the default `bullwhip` host block and creates new AI peers for each profile. Idempotent -- skips profiles that already have a host block.
...
See the [config reference](https://github.com/bullwhip-ai/bullwhip-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/bullwhip).
...
bullwhip memory setup    # select "openviking"
# Or manually:
bullwhip config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.bullwhip/.env
```
...
**Setup:**
```bash
bullwhip memory setup    # select "mem0"
# Or manually:
bullwhip config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.bullwhip/.env
```

**Config:** `$BULLWHIP_HOME/mem0.json`

| Key | Default | Description |
|-----|---------|-------------|
| `user_id` | `bullwhip-user` | User identifier |
| `agent_id` | `bullwhip` | Agent identifier |
...
**Setup:**
```bash
bullwhip memory setup    # select "hindsight"
# Or manually:
bullwhip config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.bullwhip/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p bullwhip ui start`

**Config:** `$BULLWHIP_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `bullwhip` | Memory bank identifier |
...
See [plugin README](https://github.com/ZeroLabsKorea/bullwhip-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
bullwhip memory setup    # select "holographic"
# Or manually:
bullwhip config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.bullwhip-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$BULLWHIP_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
bullwhip memory setup    # select "retaindb"
# Or manually:
bullwhip config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.bullwhip/.env
```
...
# Then configure Bull Whip
bullwhip memory setup    # select "byterover"
# Or manually:
bullwhip config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$BULLWHIP_HOME/byterover/` (profile-scoped)
...
**Setup:**
```bash
bullwhip memory setup    # select "supermemory"
# Or manually:
bullwhip config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.bullwhip/.env
```

**Config:** `$BULLWHIP_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `bullwhip` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `bullwhip-{identity}` → `bullwhip-coder`) to isolate memories per Bull Whip profile
...
- **Local storage providers** (Holographic, ByteRover) use `$BULLWHIP_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$BULLWHIP_HOME/` so each profile has its own credentials
