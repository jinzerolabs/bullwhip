...
Connect Bull Whip to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between Bull Whip and the BlueBubbles server
...
Or set environment variables directly in `~/.bullwhip/.env`:
...
When someone messages your iMessage, Bull Whip automatically sends them a pairing code. Approve it with:
```bash
bullwhip pairing approve bluebubbles <CODE>
```
Use `bullwhip pairing list` to see pending codes and approved users.

**Pre-authorize specific users** (in `~/.bullwhip/.env`):
```bash
BLUEBUBBLES_ALLOWED_USERS=user@icloud.com,+15551234567
```

**Open access** (in `~/.bullwhip/.env`):
```bash
...
Bull Whip will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.

## How It Works

```
iMessage → Messages.app → BlueBubbles Server → Webhook → Bull Whip
Bull Whip → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** Bull Whip sends messages via the BlueBubbles REST API.
...
### Chat Addressing
You can address chats by email or phone number — Bull Whip resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
...
- Check `bullwhip logs gateway` for webhook errors (or `bullwhip logs -f` to follow in real-time)
