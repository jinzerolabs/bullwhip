...
sidebar_position: 8
title: "Open WebUI"
description: "Connect Open WebUI to Bull Whip Agent via the OpenAI-compatible API server"
---

# Open WebUI Integration

[Open WebUI](https://github.com/open-webui/open-webui) (126k★) is the most popular self-hosted chat interface for AI. With Bull Whip Agent's built-in API server, you can use Open WebUI as a polished web frontend for your agent — complete with conversation management, user accounts, and a modern chat interface.
...
flowchart LR
    ...
    B["bullwhip-agent<br/>gateway API server<br/>port 8642"]
    A -->|POST /v1/chat/completions| B
    B -->|SSE streaming response| A
```

Open WebUI connects to Bull Whip Agent's API server just like it would connect to OpenAI. Your agent handles the requests with its full toolset — terminal, file operations, web search, memory, skills — and returns the final response.

Open WebUI talks to Bull Whip server-to-server, so you do not need `API_SERVER_CORS_ORIGINS` for this integration.
...
### 2. Start Bull Whip Agent gateway
...
Go to **http://localhost:3000**. Create your admin account (the first user becomes admin). You should see your agent in the model dropdown (named after your profile, or **bullwhip-agent** for the default profile). Start chatting!
...
Your agent model should now appear in the model dropdown (named after your profile, or **bullwhip-agent** for the default profile).
...
This is the default and requires no extra configuration. Open WebUI sends standard OpenAI-format requests and Bull Whip Agent responds accordingly. Each request includes the full conversation history.
...
2. Edit your bullwhip-agent connection
3. Change **API Type** from "Chat Completions" to **"Responses (Experimental)"**
4. Save

With the Responses API, Open WebUI sends requests in the Responses format (`input` array + `instructions`), and Bull Whip Agent can preserve full tool call history across turns via `previous_response_id`.
...
2. Bull Whip Agent creates an AIAgent instance with its full toolset
...
|----------|-------------|
| `OPENAI_API_BASE_URL` | Bull Whip Agent's API URL (include `/v1`) |
...
- **Check model listing**: `curl http://localhost:8642/v1/models` should return a list with `bullwhip-agent`
...
Bull Whip Agent may be executing multiple tool calls (reading files, running commands, searching the web) before producing its final response. This is normal for complex queries. The response appears all at once when the agent finishes.

### "Invalid API key" errors

Make sure your `OPENAI_API_KEY` in Open WebUI matches the `API_SERVER_KEY` in Bull Whip Agent.

## Multi-User Setup with Profiles

To run separate Bull Whip instances per user — each with their own config, memory, and skills — use [profiles](/docs/user-guide/features/profiles). Each profile runs its own API server on a different port and automatically advertises the profile name as the model in Open WebUI.

### 1. Create profiles and configure API servers

```bash
bullwhip profile create alice
bullwhip -p alice config set API_SERVER_ENABLED true
bullwhip -p alice config set API_SERVER_PORT 8643
bullwhip -p alice config set API_SERVER_KEY alice-secret

bullwhip profile create bob
bullwhip -p bob config set API_SERVER_ENABLED true
bullwhip -p bob config set API_SERVER_PORT 8644
bullwhip -p bob config set API_SERVER_KEY bob-secret
```

### 2. Start each gateway

```bash
bullwhip -p alice gateway &
bullwhip -p bob gateway &
```
...
The model dropdown will show `alice` and `bob` as distinct models. You can assign models to Open WebUI users via the admin panel, giving each user their own isolated Bull Whip agent.
...
```bash
bullwhip -p alice config set API_SERVER_MODEL_NAME "Alice's Agent"
```
:::
