...
sidebar_position: 9
title: "Matrix"
description: "Set up Bull Whip Agent as a Matrix bot"
---

# Matrix Setup

Bull Whip Agent integrates with Matrix, the open, federated messaging protocol. Matrix lets you run your own homeserver or use a public one like matrix.org — either way, you keep control of your communications. The bot connects via the `mautrix` Python SDK, processes messages through the Bull Whip Agent pipeline (including tool use, memory, and reasoning), and responds in real time. It supports text, file attachments, images, audio, video, and optional end-to-end encryption (E2EE).

Bull Whip works with any Matrix homeserver — Synapse, Conduit, Dendrite, or matrix.org.

Before setup, here's the part most people want to know: how Bull Whip behaves once it's connected.

## How Bull Whip Behaves

| Context | Behavior |
|---------|----------|
| **DMs** | Bull Whip responds to every message. No `@mention` needed. Each DM has its own session. Set `MATRIX_DM_MENTION_THREADS=true` to start a thread when the bot is `@mentioned` in a DM. |
| **Rooms** | By default, Bull Whip requires an `@mention` to respond. Set `MATRIX_REQUIRE_MENTION=false` or add room IDs to `MATRIX_FREE_RESPONSE_ROOMS` for free-response rooms. Room invites are auto-accepted. |
| **Threads** | Bull Whip supports Matrix threads (MSC3440). If you reply in a thread, Bull Whip keeps the thread context isolated from the main room timeline. Threads where the bot has already participated do not require a mention. |
| **Auto-threading** | By default, Bull Whip auto-creates a thread for each message it responds to in a room. This keeps conversations isolated. Set `MATRIX_AUTO_THREAD=false` to disable. |
| **Shared rooms with multiple users** | By default, Bull Whip isolates session history per user inside the room. Two people talking in the same room do not share one transcript unless you explicitly disable that. |
...
2. Choose a username like `bullwhip` — the full user ID will be `@bullwhip:your-server.org`.

### Option B: Use matrix.org or Another Public Homeserver

1. Go to [Element Web](https://app.element.io) and create a new account.
2. Pick a username for your bot (e.g., `bullwhip-bot`).

### Option C: Use Your Own Account

You can also run Bull Whip as your own user. This means the bot posts as you — useful for personal assistants.

## Step 2: Get an Access Token

Bull Whip needs an access token to authenticate with the homeserver. You have two options:
...
Instead of providing an access token, you can give Bull Whip the bot's user ID and password. Bull Whip will log in automatically on startup. This is simpler but means the password is stored in your `.env` file.

```bash
MATRIX_USER_ID=@bullwhip:your-server.org
MATRIX_PASSWORD=your-password
```

## Step 3: Find Your Matrix User ID

Bull Whip Agent uses your Matrix User ID to control who can interact with the bot. Matrix User IDs follow the format `@username:server`.
...
## Step 4: Configure Bull Whip Agent
...
Add the following to your `~/.bullwhip/.env` file:
...
# MATRIX_USER_ID=@bullwhip:matrix.example.org
...
Optional behavior settings in `~/.bullwhip/config.yaml`:
...
:::tip
You can run `bullwhip gateway` in the background or as a systemd service for persistent operation. See the deployment docs for details.
:::

## End-to-End Encryption (E2EE)

Bull Whip supports Matrix end-to-end encryption, so you can chat with your bot in encrypted rooms.
...
# Or install with bullwhip extras
pip install 'bullwhip-agent[matrix]'
```
...
Add to your `~/.bullwhip/.env`:

```bash
MATRIX_ENCRYPTION=true
```

When E2EE is enabled, Bull Whip:

- Stores encryption keys in `~/.bullwhip/platforms/matrix/store/` (legacy installs: `~/.bullwhip/matrix/store/`)
...
On each startup, if `MATRIX_RECOVERY_KEY` is set, Bull Whip imports cross-signing keys from the homeserver's secure secret storage and signs the current device. This is idempotent and safe to leave enabled permanently.

:::warning
If you delete the `~/.bullwhip/platforms/matrix/store/` directory, the bot loses its encryption keys. You'll need to verify the device again in your Matrix client. Back up this directory if you want to preserve encrypted sessions.
:::
...
If you previously used Bull Whip with `MATRIX_ENCRYPTION=true` and are upgrading to
...
1. **Generate a new access token** to get a fresh device ID. The simplest way:
   ...
   curl -X POST https://your-server/_matrix/client/v3/login \
     ...
     -d '{
       "type": "m.login.password",
       "identifier": {"type": "m.id.user", "user": "@bullwhip:your-server.org"},
       "password": "***",
       "initial_device_display_name": "Bull Whip Agent"
     }'
   ```

   Copy the new `access_token` and update `MATRIX_ACCESS_TOKEN` in `~/.bullwhip/.env`.

2. **Delete old encryption state**:

   ```bash
   rm -f ~/.bullwhip/platforms/matrix/store/crypto.db
   rm -f ~/.bullwhip/platforms/matrix/store/crypto_store.*
   ```

3. **Set your recovery key** (if you use cross-signing — most Element users do). Add to `~/.bullwhip/.env`:
...
a working E2EE setup with a previous version of Bull Whip and are upgrading.
...
services:
  bullwhip-matrix:
    ...
    environment:
      ...
      MATRIX_ENCRYPTION: "true"
      MATRIX_DEVICE_ID: "BULLWHIP_BOT"

      # Proxy mode — forward to host agent
      GATEWAY_PROXY_URL: "http://192.168.1.100:8642"
      GATEWAY_PROXY_KEY: "your-secret-key-here"
    volumes:
      - ./matrix-store:/root/.bullwhip/platforms/matrix/store
```
...
RUN pip install 'bullwhip-agent[matrix]'

CMD ["bullwhip", "gateway"]
```
...
| Setting | Description |
|---------|-------------|
| `GATEWAY_PROXY_URL` | URL of the remote Bull Whip API server (e.g., `http://192.168.1.100:8642`) |
...
:::tip
Session continuity is maintained via the `X-Bull Whip-Session-Id` header. The host's API server tracks sessions by this ID, so conversations persist across messages just like they would with a local agent.
:::
...
**Fix**: The sync loop automatically retries every 5 seconds on error. Check the Bull Whip logs for sync-related warnings. If the bot consistently falls behind, ensure your homeserver has adequate resources.

### Bot is offline

**Cause**: The Bull Whip gateway isn't running, or it failed to connect.

**Fix**: Check that `bullwhip gateway` is running. Look at the terminal output for error messages. Common issues: wrong homeserver URL, expired access token, homeserver unreachable.

### "User not allowed" / Bot ignores you

**Cause**: Your User ID isn't in `MATRIX_ALLOWED_USERS`.

**Fix**: Add your User ID to `MATRIX_ALLOWED_USERS` in `~/.bullwhip/.env` and restart the gateway. Use the full `@user:server` format.
...
For more information on securing your Bull Whip Agent deployment, see the [Security Guide](../security.md).
...
- **Media support**: Bull Whip can send and receive images, audio, video, and file attachments. Media is uploaded to your homeserver using the Matrix content repository API.
