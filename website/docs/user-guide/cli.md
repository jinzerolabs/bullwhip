...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Bull Whip Agent terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Bull Whip Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.
...
bullwhip chat --model "anthropic/claude-sonnet-4"

# With a specific provider
bullwhip chat --provider nous        # Use Nous Portal
bullwhip chat --provider openrouter  # Force OpenRouter
...
bullwhip chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
bullwhip --continue             # Resume the most recent CLI session (-c)
bullwhip --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
bullwhip chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
bullwhip -w                         # Interactive mode in worktree
bullwhip -w -q "Fix issue #123"     # Single query in worktree
```

## Interface Layout

<img className="docs-terminal-figure" src="/img/docs/cli-layout.svg" alt="Stylized preview of the Bull Whip CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Bull Whip CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
When resuming a previous session (`bullwhip -c` or `bullwhip --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Bull Whip to background (Unix only). Run `fg` in the shell to resume. |
| `Tab` | Accept auto-suggestion (ghost text) or autocomplete slash commands |

## Slash Commands

Type `/` to see the autocomplete dropdown. Bull Whip supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Bull Whip replies |
...
```bash
bullwhip -s bullwhip-agent-dev,github-auth
bullwhip chat -s github-pr-workflow -s github-auth
```

Bull Whip loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.bullwhip/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.bullwhip/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Bull Whip..."
  # Add your own!
```
...
On Unix systems, press **`Ctrl+Z`** to suspend Bull Whip to the background — just like any terminal process. The shell prints a confirmation:

```
Bull Whip Agent has been suspended. Run `fg` to bring Bull Whip Agent back.
```
...
Resume this session with:
  bullwhip --resume 20260225_143052_a1b2c3
...
```bash
bullwhip --continue                          # Resume the most recent CLI session
bullwhip -c                                  # Short form
bullwhip -c "my project"                     # Resume a named session (latest in lineage)
bullwhip --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
bullwhip --resume "refactoring auth"         # Resume by title
bullwhip -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `bullwhip sessions rename <id> <title>` from the command line. Use `bullwhip sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Bull Whip's SQLite state database under `~/.bullwhip/state.db`. The database keeps:
...
Bull Whip immediately confirms the task and gives you back the prompt:
