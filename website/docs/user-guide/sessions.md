...
Bull Whip Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.bullwhip/state.db`) — structured session metadata with FTS5 full-text search
2. **JSONL transcripts** (`~/.bullwhip/sessions/`) — raw conversation transcripts including tool calls (gateway)
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`bullwhip` or `bullwhip chat`) |
...
bullwhip -c "my project"

# If there are lineage variants (my project, my project #2, my project #3),
# this automatically resumes the most recent one
bullwhip -c "my project"   # → resumes "my project #3"
```
...
bullwhip --resume 20250305_091523_a1b2c3d4
bullwhip -r 20250305_091523_a1b2c3d4

# Resume by title
bullwhip --resume "refactoring auth"

# Or with the chat subcommand
bullwhip chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `bullwhip sessions list`.

### Conversation Recap on Resume

When you resume a session, Bull Whip displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Bull Whip session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.bullwhip/config.yaml`:
...
Bull Whip automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `bullwhip sessions list` or `bullwhip sessions browse`.
...
```bash
bullwhip sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Bull Whip creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`bullwhip -c "my project"`), it automatically picks the most recent session in the lineage.
...
Bull Whip provides a full set of session management commands via `bullwhip sessions`:
...
# Filter by platform
bullwhip sessions list --source telegram

# Show more sessions
bullwhip sessions list --limit 50
```
...
bullwhip sessions export backup.jsonl

# Export sessions from a specific platform
bullwhip sessions export telegram-history.jsonl --source telegram

# Export a single session
bullwhip sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
bullwhip sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
bullwhip sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
bullwhip sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
bullwhip sessions prune

# Custom age threshold
bullwhip sessions prune --older-than 30

# Only prune sessions from a specific platform
bullwhip sessions prune --source telegram --older-than 60

# Skip confirmation
bullwhip sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`bullwhip insights`](/docs/reference/cli-commands#bullwhip-insights).
...
When Bull Whip cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Bull Whip uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Bull Whip in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.bullwhip/state.db` | All session metadata + messages with FTS5 |
| Gateway transcripts | `~/.bullwhip/sessions/` | JSONL transcripts per session + sessions.json index |
| Gateway index | `~/.bullwhip/sessions/sessions.json` | Maps session keys to active session IDs |
...
bullwhip sessions delete <session_id>
