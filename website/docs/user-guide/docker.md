...
sidebar_position: 7
title: "Docker"
description: "Running Bull Whip Agent in Docker and using Docker as a terminal backend"
---

# Bull Whip Agent — Docker

There are two distinct ways Docker intersects with Bull Whip Agent:

1. **Running Bull Whip IN Docker** — the agent itself runs inside a container (this page's primary focus)
...
If this is your first time running Bull Whip Agent, create a data directory on the host and start the container interactively to run the setup wizard:

```sh
mkdir -p ~/.bullwhip
docker run -it --rm \
  -v ~/.bullwhip:/opt/data \
  zerolabskorea/bullwhip-agent setup
```

This drops you into the setup wizard, which will prompt you for your API keys and write them to `~/.bullwhip/.env`. You only need to do this once. It is highly recommended to set up a chat system for the gateway to work with at this point.
...
docker run -d \
  ...
  -v ~/.bullwhip:/opt/data \
  -p 8642:8642 \
  zerolabskorea/bullwhip-agent gateway run
```
...
```sh
docker run -d \
  --name bullwhip-dashboard \
  --restart unless-stopped \
  -v ~/.bullwhip:/opt/data \
  -p 9119:9119 \
  -e GATEWAY_HEALTH_URL=http://$HOST_IP:8642 \
  zerolabskorea/bullwhip-agent dashboard
```
...
docker run -it --rm \
  -v ~/.bullwhip:/opt/data \
  zerolabskorea/bullwhip-agent
```

## Persistent volumes

The `/opt/data` volume is the single source of truth for all Bull Whip state. It maps to your host's `~/.bullwhip/` directory and contains:

| Path | Contents |
|------|----------|
| `.env` | API keys and secrets |
| `config.yaml` | All Bull Whip configuration |
...
:::warning
Never run two Bull Whip **gateway** containers against the same data directory simultaneously — session files and memory stores are not designed for concurrent write access. Running a dashboard container alongside the gateway is safe since the dashboard only reads data.
:::
...
services:
  bullwhip:
    image: zerolabskorea/bullwhip-agent:latest
    container_name: bullwhip
    restart: unless-stopped
    command: gateway run
    ports:
      - "8642:8642"
  ...
  dashboard:
    image: zerolabskorea/bullwhip-agent:latest
    container_name: bullwhip-dashboard
    restart: unless-stopped
    ...
    volumes:
      - ~/.bullwhip:/opt/data
    environment:
      - GATEWAY_HEALTH_URL=http://bullwhip:8642
    networks:
      - bullwhip-net
    depends_on:
      - bullwhip
...
The Bull Whip container needs moderate resources. Recommended minimums:
...
- Python 3 with all Bull Whip dependencies (`pip install -e ".[all]"`)
...
- Then runs `bullwhip` with whatever arguments you pass
...
```sh
docker pull zerolabskorea/bullwhip-agent:latest
docker rm -f bullwhip
docker run -d \
  --name bullwhip \
  --restart unless-stopped \
  -v ~/.bullwhip:/opt/data \
...
Check logs: `docker logs bullwhip`. Common causes:
- Missing or invalid `.env` file — run interactively first to complete setup
- Port conflicts if running with exposed ports

### "Permission denied" errors

The container runs as root by default. If your host `~/.bullwhip/` was created by a non-root user, permissions should work. If you get errors, ensure the data directory is writable:

```sh
chmod -R 755 ~/.bullwhip
```
...
```sh
docker restart bullwhip
```

### Checking container health

```sh
docker logs --tail 50 bullwhip          # Recent logs
docker run -it --rm zerolabskorea/bullwhip-agent:latest version     # Verify version
docker stats bullwhip                    # Resource usage
```
