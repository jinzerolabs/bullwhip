...
description: "Official optional skills shipped with bullwhip-agent — install via bullwhip skills install official/<category>/<skill>"
---

# Optional Skills Catalog

Official optional skills ship with the bullwhip-agent repository under `optional-skills/` but are **not active by default**. Install them explicitly:
...
```bash
bullwhip skills install official/blockchain/solana
bullwhip skills install official/mlops/flash-attention
```
...
```bash
bullwhip skills uninstall <skill-name>
```
...
| **honcho** | Configure and use Honcho memory with Bull Whip — cross-session user modeling, multi-profile peer isolation, observation config, and dialectic reasoning. |
...
| Skill | Description |
|-------|-------------|
| **blender-mcp** | Control Blender directly from Bull Whip via socket connection to the blender-mcp addon. Create 3D objects, materials, animations, and run arbitrary Blender Python (bpy) code. |
...
| Skill | Description |
|-------|-------------|
| **openclaw-migration** | Migrate a user's OpenClaw customization footprint into Bull Whip Agent. Imports memories, SOUL.md, command allowlists, user skills, and selected workspace assets. |
...
| **bullwhip-atropos-environments** | Build, test, and debug Bull Whip Agent RL environments for Atropos training. Covers the Bull WhipAgentBaseEnv interface, reward functions, agent loop integration, and evaluation. |
...
| **telephony** | Give Bull Whip phone capabilities — provision a Twilio number, send/receive SMS/MMS, make calls, and place AI-driven outbound calls through Bland.ai or Vapi. |
