...
description: "Frequently asked questions and solutions to common issues with Bull Whip Agent"
---
...
### What LLM providers work with Bull Whip?

Bull Whip Agent works with any OpenAI-compatible API. Supported providers include:

- **[OpenRouter](https://openrouter.ai/)** — access hundreds of models through one API key (recommended for flexibility)
- **Nous Portal** — ZeroLabs Korea's own inference endpoint
...
Set your provider with `bullwhip model` or by editing `~/.bullwhip/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.

### Does it work on Windows?

**Not natively.** Bull Whip Agent requires a Unix-like environment. On Windows, install [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) and run Bull Whip from inside it. The standard install command works perfectly in WSL2:
...
Yes — Bull Whip now has a tested Termux install path for Android phones.
...
API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). Bull Whip Agent does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.bullwhip/`.

### Can I use it offline / with local models?

Yes. Run `bullwhip model`, select **Custom endpoint**, and enter your server's URL:
...
Bull Whip persists the endpoint, provider, and base URL in `config.yaml` so it survives restarts. If your local server has exactly one model loaded, `/model custom` auto-detects it. You can also set `provider: custom` in config.yaml — it's a first-class provider, not an alias for anything else.
...
:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 16384`), make sure to set the matching context length in Bull Whip — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::

:::tip Timeouts with local models
Bull Whip auto-detects local endpoints and relaxes streaming timeouts (read timeout raised from 120s to 1800s, stale stream detection disabled). If you still hit timeouts on very large contexts, set `BULLWHIP_STREAM_READ_TIMEOUT=1800` in your `.env`. See the [Local LLM guide](../guides/local-llm-on-mac.md#timeouts) for details.
:::

### How much does it cost?

Bull Whip Agent itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same Bull Whip Agent instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
Yes. Import the `AIAgent` class and use Bull Whip programmatically:

```python
from run_agent import AIAgent

agent = AIAgent(model="openrouter/nous/bullwhip-3-llama-3.1-70b")
...
#### `bullwhip: command not found` after installation
...
**Cause:** Bull Whip requires Python 3.11 or newer.
...
sudo rm /usr/local/bin/bullwhip
...
bullwhip config show

# Re-configure your provider
bullwhip model

# Or set directly
bullwhip config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```

:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check `~/.bullwhip/.env` for conflicting entries.
:::
...
# Set a valid model
bullwhip config set BULLWHIP_MODEL openrouter/nous/bullwhip-3-llama-3.1-70b

# Or specify per-session
bullwhip chat --model openrouter/meta-llama/llama-3.1-70b-instruct
```
...
- Using `bullwhip chat --provider <alternative>` to route to a different backend

#### Context length exceeded

**Cause:** The conversation has grown too long for the model's context window, or Bull Whip detected the wrong context length for your model.
...
bullwhip chat --model openrouter/google/gemini-3-flash-preview
```

If this happens on the first long conversation, Bull Whip may have the wrong context length for your model. Check what it detected:
...
**Cause:** Bull Whip detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — Bull Whip never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
- Or switch to the terminal interface for administrative tasks: `bullwhip chat`
...
# Start the gateway
bullwhip gateway start

# Check logs for errors
cat ~/.bullwhip/logs/gateway.log | tail -50
```
...
**Solution:**
- Verify your bot token is valid with `bullwhip gateway setup`
- Check gateway logs: `cat ~/.bullwhip/logs/gateway.log | tail -50`
...
Configure in `~/.bullwhip/config.yaml` under your gateway's settings. See the [Messaging docs](../user-guide/messaging/index.md).
...
pip install "bullwhip-agent[telegram]"   # or [discord], [slack], [whatsapp]
...
#### WSL: Gateway keeps disconnecting or `bullwhip gateway start` fails
...
bullwhip gateway run

# Option 2: Persistent via tmux (survives terminal close)
tmux new -s bullwhip 'bullwhip gateway run'
# Reattach later: tmux attach -t bullwhip

# Option 3: Background via nohup
nohup bullwhip gateway run > ~/.bullwhip/logs/gateway.log 2>&1 &
```
...
1. Create a task that runs `wsl -d Ubuntu -- bash -lc 'bullwhip gateway run'`
...
**Solution:** The gateway captures your shell PATH when you run `bullwhip gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:

```bash
bullwhip gateway install    # Re-snapshots your current PATH
bullwhip gateway start      # Detects the updated plist and reloads
```

You can verify the plist has the correct PATH:
```bash
/usr/libexec/PlistBuddy -c "Print :EnvironmentVariables:PATH" \
  ~/Library/LaunchAgents/ai.bullwhip.gateway.plist
```
...
**Solution:**
- Try a faster/smaller model: `bullwhip chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `bullwhip chat -t "terminal"`
...
cd ~/.bullwhip/bullwhip-agent && uv pip install -e ".[mcp]"
...
Verify your `~/.bullwhip/config.yaml` MCP configuration:
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
bullwhip config show | grep -A 12 mcp_servers

# Restart Bull Whip or reload MCP after config changes
bullwhip chat
```

See also:
- [MCP (Model Context Protocol)](/docs/user-guide/features/mcp)
- [Use MCP with Bull Whip](/docs/guides/use-mcp-with-bullwhip)
...
:::warning
If an MCP server crashes mid-request, Bull Whip will report a timeout. Check the server's own logs (not just Bull Whip logs) to diagnose the root cause.
:::
...
No. Each profile has its own memory store, session database, and skills directory. They are completely isolated. If you want to start a new profile with existing memories and sessions, use `bullwhip profile create newname --clone-all` to copy everything from the current profile.

### What happens when I run `bullwhip update`?

`bullwhip update` pulls the latest code and reinstalls dependencies **once** (not per-profile). It then syncs updated skills to all profiles automatically. You only need to run `bullwhip update` once — it covers every profile on the machine.
...
bullwhip profile export work ./work-backup.tar.gz

# Copy the file to the target machine, then:
bullwhip profile import ./work-backup.tar.gz work
```
...
There is no hard limit. Each profile is just a directory under `~/.bullwhip/profiles/`. The practical limit depends on your disk space and how many concurrent gateways your system can handle (each gateway is a lightweight Python process). Running dozens of profiles is fine; each idle profile uses no resources.
...
**Solution: Delegation config.** Bull Whip can route subagents to a different model automatically. Set this in `~/.bullwhip/config.yaml`:
...
Now when you tell Bull Whip "write me a Twitter thread about X" and it spawns a `delegate_task` subagent, that subagent runs on Gemini instead of your main model. Your primary conversation stays on GPT-5.4.
...
**Scenario:** In OpenClaw, you had multiple independent agents bound to specific WhatsApp chats — one for a family shopping list group, another for your private chat. Can Bull Whip do this?

**Current limitation:** Bull Whip profiles each require their own WhatsApp number/session. You cannot bind multiple profiles to different chats on the same WhatsApp number — the WhatsApp bridge (Baileys) uses one authenticated session per number.
...
**Scenario:** You see gateway exec logs, Bull Whip reasoning, and tool call details in Telegram instead of just the final output.
...
**Scenario:** Telegram has a 100 slash command limit, and your skills are pushing past it. You want to disable skills you don't need on Telegram, but `bullwhip skills config` settings don't seem to take effect.

**Solution:** Use `bullwhip skills config` to disable skills per-platform. This writes to `config.yaml`:
...
After changing this, **restart the gateway** (`bullwhip gateway restart` or kill and relaunch). The Telegram bot command menu rebuilds on startup.
...
**Current behavior:** Bull Whip creates sessions keyed by user ID on most platforms, so each person gets their own conversation context. This is by design for privacy and context isolation.
...
### Exporting Bull Whip to another machine
...
1. Install Bull Whip Agent on the new machine:
   ```bash
   curl -fsSL https://raw.githubusercontent.com/ZeroLabsKorea/bullwhip-agent/main/scripts/install.sh | bash
   ```

2. Copy your entire `~/.bullwhip/` directory **except** the `bullwhip-agent` subdirectory (that's the code repo — the new install has its own):
   ```bash
   # On the source machine
   rsync -av --exclude='bullwhip-agent' ~/.bullwhip/ newmachine:~/.bullwhip/
   ```

   Or use profile export/import:
   ```bash
   # On source machine
   bullwhip profile export default ./bullwhip-backup.tar.gz

   # On target machine
   bullwhip profile import ./bullwhip-backup.tar.gz default
   ```

3. On the new machine, run `bullwhip setup` to verify API keys and provider config are working. Re-authenticate any messaging platforms (especially WhatsApp, which uses QR pairing).

The `~/.bullwhip/` directory contains everything: `config.yaml`, `.env`, `SOUL.md`, `memories/`, `skills/`, `state.db` (sessions), `cron/`, and any custom plugins. The code itself lives in `~/.bullwhip/bullwhip-agent/` and is installed fresh.

### Permission denied when reloading shell after install

**Scenario:** After running the Bull Whip installer, `source ~/.zshrc` gives a permission denied error.

**Cause:** This usually happens when `~/.zshrc` (or `~/.bashrc`) has incorrect file permissions, or when the installer couldn't write to it cleanly. It's not a Bull Whip-specific issue — it's a shell config permissions problem.
...
bullwhip config show | head -20

# Re-run model selection
bullwhip model

# Or test with a known-good model
bullwhip chat -q "hello" --model anthropic/claude-sonnet-4.6
```
...
1. **Search existing issues:** [GitHub Issues](https://github.com/ZeroLabsKorea/bullwhip-agent/issues)
2. **Ask the community:** [ZeroLabs Korea Discord](https://discord.gg/zerolabskorea)
3. **File a bug report:** Include your OS, Python version (`python3 --version`), Bull Whip version (`bullwhip --version`), and the full error message
