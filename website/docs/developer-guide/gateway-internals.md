...
The messaging gateway is the long-running process that connects Bull Whip to 14+ external messaging platforms through a unified architecture.
...
1. `resolve_command()` from `bullwhip_cli/commands.py` maps input to canonical name (handles aliases, prefix matching)
...
|--------|-----------------|
| `~/.bullwhip/.env` | API keys, bot tokens, platform credentials |
| `~/.bullwhip/config.yaml` | Model settings, tool configuration, display options |
...
Hooks are discovered from `gateway/builtin_hooks/` (always active) and `~/.bullwhip/hooks/` (user-installed). Each hook is a directory with a `HOOK.yaml` manifest and `handler.py`.
...
- `bullwhip gateway start` / `bullwhip gateway stop` — manual control
- `systemctl` (Linux) or `launchctl` (macOS) — service management
- PID file at `~/.bullwhip/gateway.pid` — profile-scoped process tracking

**Profile-scoped vs global**: `start_gateway()` uses profile-scoped PID files. `bullwhip gateway stop` stops only the current profile's gateway. `bullwhip gateway stop --all` uses global `ps aux` scanning to kill all gateway processes (used during updates).
