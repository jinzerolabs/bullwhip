...
description: "How Bull Whip builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

Bull Whip deliberately separates:
...
1. agent identity — `SOUL.md` from `BULLWHIP_HOME` when available, otherwise falls back to `DEFAULT_AGENT_IDENTITY` in `prompt_builder.py`
...
```
# Layer 1: Agent Identity (from ~/.bullwhip/SOUL.md)
You are Bull Whip, an AI assistant created by ZeroLabs Korea.
...
`SOUL.md` lives at `~/.bullwhip/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_hermes_md(cwd_path)       # 1. .bullwhip.md / BULLWHIP.md (walks to git root)
    ...
    # SOUL.md from BULLWHIP_HOME (independent of project context)
    if not skip_soul:
...
|----------|-------|-------------|-------|
| 1 | `.bullwhip.md`, `BULLWHIP.md` | CWD up to git root | Bull Whip-native project config |
...
- **YAML frontmatter stripped** — `.bullwhip.md` frontmatter is removed (reserved for future config overrides)
...
1. `.bullwhip.md` / `BULLWHIP.md` (walks to git root)
