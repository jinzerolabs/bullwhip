...
Bull Whip Agent uses a SQLite database (`~/.bullwhip/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `bullwhip_state.py`


## Architecture Overview

```
~/.bullwhip/state.db (SQLite, WAL mode)
...
Multiple bullwhip processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.bullwhip/state.db
...
Default path: `~/.bullwhip/state.db`

This is derived from `bullwhip_constants.get_bullwhip_home()` which resolves to
`~/.bullwhip/` by default, or the value of `BULLWHIP_HOME` environment variable.
