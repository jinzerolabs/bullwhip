...
description: "How Bull Whip stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `bullwhip_cli/cron.py` | CLI `bullwhip cron` subcommands |
...
Jobs are stored in `~/.bullwhip/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `bullwhip cron` commands are run or during active CLI sessions.
...
```python
# ~/.bullwhip/scripts/check_competitors.py
...
2. **Environment variable** — `BULLWHIP_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.bullwhip/cron/output/` |
...
The `bullwhip cron` CLI provides direct job management:

```bash
bullwhip cron list                    # Show all jobs
bullwhip cron create                  # Interactive job creation (alias: add)
bullwhip cron edit <job_id>           # Edit job configuration
bullwhip cron pause <job_id>          # Pause a running job
bullwhip cron resume <job_id>         # Resume a paused job
bullwhip cron run <job_id>            # Trigger immediate execution
bullwhip cron remove <job_id>         # Delete a job
```
