...
description: "How to add a new inference provider to Bull Whip Agent — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

Bull Whip can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `bullwhip model` menu entries
...
1. `bullwhip_cli/auth.py` decides how credentials are found.
2. `bullwhip_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `bullwhip_cli/models.py` and `bullwhip_cli/main.py` make the provider show up in the CLI. (`bullwhip_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
1. `bullwhip_cli/auth.py`
2. `bullwhip_cli/models.py`
3. `bullwhip_cli/runtime_provider.py`
4. `bullwhip_cli/main.py`
...
:::tip
`bullwhip_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `bullwhip setup`.
:::
...
- `PROVIDER_REGISTRY` in `bullwhip_cli/auth.py`
- `_PROVIDER_LABELS` in `bullwhip_cli/models.py`
- `_PROVIDER_ALIASES` in both `bullwhip_cli/auth.py` and `bullwhip_cli/models.py`
- CLI `--provider` choices in `bullwhip_cli/main.py`
...
## Step 2: Add auth metadata in `bullwhip_cli/auth.py`
...
- What env vars should Bull Whip check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `bullwhip_cli/models.py`
...
## Step 4: Resolve runtime data in `bullwhip_cli/runtime_provider.py`
...
Be careful with API-key precedence. Bull Whip already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `bullwhip_cli/main.py`

A provider is not discoverable until it shows up in the interactive `bullwhip model` flow.

Update these in `bullwhip_cli/main.py`:
...
:::tip
`bullwhip_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `bullwhip model` and `bullwhip setup` automatically.
:::
...
When you add a native provider, double-check that Bull Whip is only sending fields that provider actually understands.
...
```bash
source venv/bin/activate
python -m bullwhip_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
```bash
source venv/bin/activate
python -m bullwhip_cli.main model
python -m bullwhip_cli.main setup
```
...
- [ ] `ProviderConfig` added in `bullwhip_cli/auth.py`
- [ ] aliases added in `bullwhip_cli/auth.py` and `bullwhip_cli/models.py`
- [ ] model catalog added in `bullwhip_cli/models.py`
- [ ] runtime branch added in `bullwhip_cli/runtime_provider.py`
- [ ] CLI wiring added in `bullwhip_cli/main.py` (setup.py inherits automatically)
...
### 7. Updating `bullwhip model` but not `bullwhip setup`
