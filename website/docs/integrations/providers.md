...
This page covers setting up inference providers for Bull Whip Agent — from cloud APIs like OpenRouter and Anthropic, to self-hosted endpoints like Ollama and vLLM, to advanced routing and fallback configurations. You need at least one provider configured to use Bull Whip.

## Inference Providers

You need at least one way to connect to an LLM. Use `bullwhip model` to switch providers and models interactively, or configure directly:

| Provider | Setup |
|----------|-------|
| **Nous Portal** | `bullwhip model` (OAuth, subscription-based) |
| **OpenAI Codex** | `bullwhip model` (ChatGPT OAuth, uses Codex models) |
| **GitHub Copilot** | `bullwhip model` (OAuth device code flow, `COPILOT_GITHUB_TOKEN`, `GH_TOKEN`, or `gh auth token`) |
| **GitHub Copilot ACP** | `bullwhip model` (spawns local `copilot --acp --stdio`) |
| **Anthropic** | `bullwhip model` (Claude Pro/Max via Claude Code auth, Anthropic API key, or manual setup-token) |
| **OpenRouter** | `OPENROUTER_API_KEY` in `~/.bullwhip/.env` |
| **AI Gateway** | `AI_GATEWAY_API_KEY` in `~/.bullwhip/.env` (provider: `ai-gateway`) |
| **z.ai / GLM** | `GLM_API_KEY` in `~/.bullwhip/.env` (provider: `zai`) |
| **Kimi / Moonshot** | `KIMI_API_KEY` in `~/.bullwhip/.env` (provider: `kimi-coding`) |
| **Kimi / Moonshot (China)** | `KIMI_CN_API_KEY` in `~/.bullwhip/.env` (provider: `kimi-coding-cn`; aliases: `kimi-cn`, `moonshot-cn`) |
| **Arcee AI** | `ARCEEAI_API_KEY` in `~/.bullwhip/.env` (provider: `arcee`; aliases: `arcee-ai`, `arceeai`) |
| **MiniMax** | `MINIMAX_API_KEY` in `~/.bullwhip/.env` (provider: `minimax`) |
| **MiniMax China** | `MINIMAX_CN_API_KEY` in `~/.bullwhip/.env` (provider: `minimax-cn`) |
| **Alibaba Cloud** | `DASHSCOPE_API_KEY` in `~/.bullwhip/.env` (provider: `alibaba`, aliases: `dashscope`, `qwen`) |
| **Kilo Code** | `KILOCODE_API_KEY` in `~/.bullwhip/.env` (provider: `kilocode`) |
| **Xiaomi MiMo** | `XIAOMI_API_KEY` in `~/.bullwhip/.env` (provider: `xiaomi`, aliases: `mimo`, `xiaomi-mimo`) |
| **OpenCode Zen** | `OPENCODE_ZEN_API_KEY` in `~/.bullwhip/.env` (provider: `opencode-zen`) |
| **OpenCode Go** | `OPENCODE_GO_API_KEY` in `~/.bullwhip/.env` (provider: `opencode-go`) |
| **DeepSeek** | `DEEPSEEK_API_KEY` in `~/.bullwhip/.env` (provider: `deepseek`) |
| **Hugging Face** | `HF_TOKEN` in `~/.bullwhip/.env` (provider: `huggingface`, aliases: `hf`) |
| **Google / Gemini** | `GOOGLE_API_KEY` (or `GEMINI_API_KEY`) in `~/.bullwhip/.env` (provider: `gemini`) |
| **Custom Endpoint** | `bullwhip model` → choose "Custom endpoint" (saved in `config.yaml`) |
...
:::info Codex Note
The OpenAI Codex provider authenticates via device code (open a URL, enter a code). Bull Whip stores the resulting credentials in its own auth store under `~/.bullwhip/auth.json` and can import existing Codex CLI credentials from `~/.codex/auth.json` when present. No Codex CLI installation is required.
:::
...
bullwhip chat --provider anthropic --model claude-sonnet-4-6

# Preferred: authenticate through `bullwhip model`
# Bull Whip will use Claude Code's credential store directly when available
bullwhip model

# Manual override with a setup-token (fallback / legacy)
export ANTHROPIC_TOKEN=***  # setup-token or manual OAuth token
bullwhip chat --provider anthropic

# Auto-detect Claude Code credentials (if you already use Claude Code)
bullwhip chat --provider anthropic  # reads Claude Code credential files automatically
```

When you choose Anthropic OAuth through `bullwhip model`, Bull Whip prefers Claude Code's own credential store over copying the token into `~/.bullwhip/.env`. That keeps refreshable Claude credentials refreshable.
...
Bull Whip supports GitHub Copilot as a first-class provider with two modes:
...
```bash
bullwhip chat --provider copilot --model gpt-5.4
```
...
If no token is found, `bullwhip model` offers an **OAuth device code login** — the same flow used by the Copilot CLI and opencode.
...
|------|--------|------------|
| OAuth token | `gho_` | `bullwhip model` → GitHub Copilot → Login with GitHub |
...
If your `gh auth token` returns a `ghp_*` token, use `bullwhip model` to authenticate via OAuth instead.
:::
...
```bash
bullwhip chat --provider copilot-acp --model copilot-acp
...
| `BULLWHIP_COPILOT_ACP_COMMAND` | Override the Copilot CLI binary path (default: `copilot`) |
| `BULLWHIP_COPILOT_ACP_ARGS` | Override ACP args (default: `--acp --stdio`) |
...
```bash
# z.ai / ZhipuAI GLM
bullwhip chat --provider zai --model glm-5
# Requires: GLM_API_KEY in ~/.bullwhip/.env

# Kimi / Moonshot AI (international: api.moonshot.ai)
bullwhip chat --provider kimi-coding --model kimi-for-coding
# Requires: KIMI_API_KEY in ~/.bullwhip/.env

# Kimi / Moonshot AI (China: api.moonshot.cn)
bullwhip chat --provider kimi-coding-cn --model kimi-k2.5
# Requires: KIMI_CN_API_KEY in ~/.bullwhip/.env

# MiniMax (global endpoint)
bullwhip chat --provider minimax --model MiniMax-M2.7
# Requires: MINIMAX_API_KEY in ~/.bullwhip/.env

# MiniMax (China endpoint)
bullwhip chat --provider minimax-cn --model MiniMax-M2.7
# Requires: MINIMAX_CN_API_KEY in ~/.bullwhip/.env

# Alibaba Cloud / DashScope (Qwen models)
bullwhip chat --provider alibaba --model qwen3.5-plus
# Requires: DASHSCOPE_API_KEY in ~/.bullwhip/.env

# Xiaomi MiMo
bullwhip chat --provider xiaomi --model mimo-v2-pro
# Requires: XIAOMI_API_KEY in ~/.bullwhip/.env

# Arcee AI (Trinity models)
bullwhip chat --provider arcee --model trinity-large-thinking
# Requires: ARCEEAI_API_KEY in ~/.bullwhip/.env
```
...
When using the Z.AI / GLM provider, Bull Whip automatically probes multiple endpoints (global, China, coding variants) to find one that accepts your API key. You don't need to set `GLM_BASE_URL` manually — the working endpoint is detected and cached automatically.
:::

### xAI (Grok) Prompt Caching

When using xAI as a provider (any base URL containing `x.ai`), Bull Whip automatically enables prompt caching by sending the `x-grok-conv-id` header with every API request. This routes requests to the same server within a conversation session, allowing xAI's infrastructure to reuse cached system prompts and conversation history.
...
bullwhip chat --provider huggingface --model Qwen/Qwen3-235B-A22B-Thinking-2507
# Requires: HF_TOKEN in ~/.bullwhip/.env

# Short alias
bullwhip chat --provider hf --model deepseek-ai/DeepSeek-V3.2
```
...
Bull Whip Agent works with **any OpenAI-compatible API endpoint**. If a server implements `/v1/chat/completions`, you can point Bull Whip at it. This means you can use local models, GPU inference servers, multi-provider routers, or any third-party API.
...
`OPENAI_BASE_URL` and `LLM_MODEL` in `.env` are **removed**. Neither is read by any part of Bull Whip — `config.yaml` is the single source of truth for model and endpoint configuration. If you have stale entries in your `.env`, they are automatically cleared on the next `bullwhip setup` or config migration. Use `bullwhip model` or edit `config.yaml` directly.
:::
...
When switching providers, Bull Whip persists the base URL and provider to config so the change survives restarts. When switching away from a custom endpoint to a built-in provider, the stale base URL is automatically cleared.
...
**You cannot set context length through the OpenAI-compatible API** (`/v1/chat/completions`). It must be configured server-side or via a Modelfile. This is the #1 source of confusion when integrating Ollama with tools like Bull Whip.
:::
...
| Flag | Purpose |
|------|---------|
| `--enable-auto-tool-choice` | Required for `tool_choice: "auto"` (the default in Bull Whip) |
| `--tool-call-parser <name>` | Parser for the model's tool call format |

Supported parsers: `bullwhip` (Qwen 2.5, Bull Whip 2/3), `llama3_json` (Llama 3.x), `mistral`, `deepseek_v3`, `deepseek_v31`, `xlam`, `pythonic`. Without these flags, tool calls won't work — the model will output tool calls as text.
...
Then configure Bull Whip to point at it:
...
Without `--jinja`, llama-server ignores the `tools` parameter entirely. The model will try to call tools by writing JSON in its response text, but Bull Whip won't recognize it as a tool call — you'll see raw JSON like `{"name": "web_search", ...}` printed as a message instead of an actual search.

Native tool calling support (best performance): Llama 3.x, Qwen 2.5 (including Coder), Bull Whip 2/3, Mistral, DeepSeek, Functionary. All other models use a generic handler that works but may be less efficient. See the [llama.cpp function calling docs](https://github.com/ggml-org/llama.cpp/blob/master/docs/function-calling.md) for the full list.
...
**Tool calling:** Supported since LM Studio 0.3.6. Models with native tool-calling training (Qwen 2.5, Llama 3.x, Mistral, Bull Whip) are auto-detected and shown with a tool badge. Other models use a generic fallback that may be less reliable.

---

### WSL2 Networking (Windows Users)

Since Bull Whip Agent requires a Unix environment, Windows users run it inside WSL2. If your model server (Ollama, LM Studio, etc.) runs on the **Windows host**, you need to bridge the network gap — WSL2 uses a virtual network adapter with its own subnet, so `localhost` inside WSL2 refers to the Linux VM, **not** the Windows host.
...
Use that IP in your Bull Whip config:
...
If you get a JSON response listing your models, you're good. Use that same URL as the `base_url` in your Bull Whip config.

---

### Troubleshooting Local Models

These issues affect **all** local inference servers when used with Bull Whip.

#### "Connection refused" from WSL2 to a Windows-hosted model server

If you're running Bull Whip inside WSL2 and your model server on the Windows host, `http://localhost:<port>` won't work in WSL2's default NAT networking mode. See [WSL2 Networking](#wsl2-networking-windows-users) above for the fix.
...
| **vLLM** | Add `--enable-auto-tool-choice --tool-call-parser bullwhip` |
...
**Cause:** Context window is too small. When the conversation exceeds the context limit, most servers silently drop older messages. Bull Whip's system prompt + tool schemas alone can use 4k–8k tokens.

**Diagnosis:**

```bash
# Check what Bull Whip thinks the context is
...
Bull Whip auto-detects context length from your server's `/v1/models` endpoint. If the server reports a low value (or doesn't report one at all), Bull Whip uses the model's declared limit which may be wrong.
...
**Possible causes:**
1. **Low output cap (`max_tokens`) on the server** — SGLang defaults to 128 tokens per response. Set `--default-max-tokens` on the server or configure Bull Whip with `model.max_tokens` in config.yaml. Note: `max_tokens` controls response length only — it is unrelated to how long your conversation history can be (that is `context_length`).
2. **Context exhaustion** — The model filled its context window. Increase `model.context_length` or enable [context compression](/docs/user-guide/configuration#context-compression) in Bull Whip.
...
Then configure Bull Whip with `bullwhip model` → Custom endpoint → `http://localhost:4000/v1`.
...
Then configure Bull Whip with `bullwhip model` → Custom endpoint → `http://localhost:8402/v1` → model name `blockrun/auto`.
...
Configure any of these with `bullwhip model` → Custom endpoint, or in `config.yaml`:
...
**`context_length`** is the **total context window** — the combined budget for input *and* output tokens (e.g. 200,000 for Claude Opus 4.6). Bull Whip uses this to decide when to compress history and to validate API requests.
...
Bull Whip uses a multi-source resolution chain to detect the correct context window for your model and provider:
...
`bullwhip model` will prompt for context length when configuring a custom endpoint. Leave it blank for auto-detection.
...
custom_providers:
  - name: local
    base_url: http://localhost:8080/v1
    # api_key omitted — Bull Whip uses "no-key-required" for keyless local servers
  - name: work
...
You can also select named custom providers from the interactive `bullwhip model` menu.
...
:::tip
You can switch between providers at any time with `bullwhip model` — no restart required. Your conversation history, memory, and skills carry over regardless of which provider you use.
:::
...
2. Point Bull Whip at your instance (no API key needed):
   ```bash
   bullwhip config set FIRECRAWL_API_URL http://localhost:3002
   ```
...
When using OpenRouter, you can control how requests are routed across providers. Add a `provider_routing` section to `~/.bullwhip/config.yaml`:
...
Configure a backup provider:model that Bull Whip switches to automatically when your primary model fails (rate limits, server errors, auth failures):
...
Optional cheap-vs-strong routing lets Bull Whip keep your main model for complex work while sending very short/simple turns to a cheaper model.
...
How it works:
- If a turn is short, single-line, and does not look code/tool/debug heavy, Bull Whip may route it to `cheap_model`
- If the turn looks complex, Bull Whip stays on your primary model/provider
- If the cheap route cannot be resolved cleanly, Bull Whip falls back to the primary model automatically
