...
sidebar_position: 2
title: "Installation"
description: "Install Bull Whip Agent on Linux, macOS, WSL2, or Android via Termux"
---

# Installation

Get Bull Whip Agent up and running in under two minutes with the one-line installer, or follow the manual steps for full control.
...
Bull Whip now ships a Termux-aware installer path too:
...
:::warning Windows
Native Windows is **not supported**. Please install [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) and run Bull Whip Agent from there. The install command above works inside WSL2.
:::

### What the Installer Does

The installer handles everything automatically — all dependencies (Python, Node.js, ripgrep, ffmpeg), the repo clone, virtual environment, global `bullwhip` command setup, and LLM provider configuration. By the end, you're ready to chat.
...
bullwhip gateway setup  # Set up messaging platforms
bullwhip config set     # Set individual config values
bullwhip setup          # Or run the full setup wizard to configure everything at once
```
...
:::tip
You do **not** need to activate the venv to use `bullwhip`. The entry point has a hardcoded shebang pointing to the venv Python, so it works globally once symlinked.
:::
...
mkdir -p ~/.bullwhip/{cron,sessions,logs,memories,skills,pairing,hooks,image_cache,audio_cache,whatsapp/session}
...
Open `~/.bullwhip/.env` and add at minimum an LLM provider key:
...
```bash
bullwhip config set OPENROUTER_API_KEY sk-or-v1-your-key-here
```

### Step 8: Add `bullwhip` to Your PATH
...
```bash
bullwhip model       # Select your LLM provider and model
```

### Step 10: Verify the Installation

```bash
bullwhip version    # Check that the command is available
bullwhip doctor     # Run diagnostics to verify everything is working
bullwhip status     # Check your configuration
bullwhip chat -q "Hello! What tools do you have available?"
```
...
# Make bullwhip available globally
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/bullwhip" ~/.local/bin/bullwhip
...
| Problem | Solution |
|---------|----------|
| `bullwhip: command not found` | Reload your shell (`source ~/.bashrc`) or check PATH |
| `API key not set` | Run `bullwhip model` to configure your provider, or `bullwhip config set OPENROUTER_API_KEY your_key` |
| Missing config after update | Run `bullwhip config check` then `bullwhip config migrate` |

For more diagnostics, run `bullwhip doctor` — it will tell you exactly what's missing and how to fix it.
