...
description: "Run Bull Whip Agent directly on an Android phone with Termux"
---

# Bull Whip on Android with Termux

This is the tested path for running Bull Whip Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Bull Whip from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Bull Whip now ships a Termux-aware installer path:
...
- links `bullwhip` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `bullwhip` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/bullwhip" "$PREFIX/bin/bullwhip"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `bullwhip` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.bullwhip/.env`.
...
### `bullwhip doctor` says ripgrep or Node is missing
