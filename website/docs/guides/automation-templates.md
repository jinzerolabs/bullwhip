...
Copy-paste recipes for common automation patterns. Each template uses Bull Whip's built-in [cron scheduler](/docs/user-guide/features/cron) for time-based triggers and [webhook platform](/docs/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`bullwhip webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `bullwhip webhook subscribe`) |
...
```bash
bullwhip cron create "0 2 * * *" \
  "You are a project manager triaging the ZeroLabsKorea/bullwhip-agent GitHub repo.

1. Run: gh issue list --repo ZeroLabsKorea/bullwhip-agent --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
bullwhip webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
bullwhip cron create "0 9 * * 1" \
  "Scan the ZeroLabsKorea/bullwhip-agent repo for documentation drift.

1. Run: gh pr list --repo ZeroLabsKorea/bullwhip-agent --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (bullwhip_cli/commands.py, bullwhip_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (bullwhip_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
bullwhip cron create "0 6 * * *" \
  "Run a dependency security audit on the bullwhip-agent project.

1. cd ~/.bullwhip/bullwhip-agent && source .venv/bin/activate
...
```bash
bullwhip webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
bullwhip webhook subscribe alert-triage \
...
```python title="~/.bullwhip/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Bull Whip-Monitor/1.0"})
...
```bash
bullwhip cron create "every 30m" \
  ...
  --script ~/.bullwhip/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
bullwhip cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Bull Whip Agent development." \
...
```bash
bullwhip webhook subscribe github-issues \
  --events "issues" \
...
```bash
bullwhip webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
bullwhip webhook subscribe stripe-payments \
...
```bash
bullwhip cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the bullwhip-agent codebase.
