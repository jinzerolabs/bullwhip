...
description: "Diagnose and fix common Bull Whip cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`bullwhip gateway` or `bullwhip serve`). For one-off debugging, you can manually trigger a tick with `bullwhip cron tick`.
...
```bash
date
bullwhip cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.bullwhip/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.bullwhip/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.bullwhip/.env` |
...
| `local` | Write access to `~/.bullwhip/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `bullwhip cron list` for updated `last_error` field (if available).
...
```bash
bullwhip skills list
```

Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `bullwhip skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-daily-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `bullwhip skills list`.
...
2. `~/.bullwhip/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `bullwhip cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Bull Whip config directory). Verify:
```bash
ls ~/.bullwhip/scripts/your-script.py   # Must exist
bullwhip cron edit <job_id> --script ~/.bullwhip/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `bullwhip skills install <skill-name>`.
...
```bash
ps aux | grep bullwhip
# Kill duplicate processes, keep only one
```

### Check 4: Permissions on jobs.json

Jobs are stored in `~/.bullwhip/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.bullwhip/cron/jobs.json
chmod 600 ~/.bullwhip/cron/jobs.json   # Your user should own it
```
...
```bash
bullwhip cron list                    # Show all jobs, states, next_run times
bullwhip cron run <job_id>            # Schedule for next tick (for testing)
bullwhip cron edit <job_id>           # Fix configuration issues
bullwhip logs                         # View recent Bull Whip logs
bullwhip skills list                  # Verify installed skills
```
...
1. Run the job with `bullwhip cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.bullwhip/logs/agent.log` for scheduler messages and `~/.bullwhip/logs/errors.log` for warnings
3. Open an issue at [github.com/ZeroLabsKorea/bullwhip-agent](https://github.com/ZeroLabsKorea/bullwhip-agent) with:
