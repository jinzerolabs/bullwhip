...
This tutorial walks you through setting up a Telegram bot powered by Bull Whip Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Bull Whip Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/docs/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.bullwhip/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Bull Whip itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Bull Whip Assistant`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Bull Whip Agent. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.bullwhip/.env`:
...
```
[Gateway] Starting Bull Whip Gateway...
...
# View live logs
journalctl --user -u bullwhip-gateway -f
...
bullwhip gateway start
bullwhip gateway stop
tail -f ~/.bullwhip/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `bullwhip gateway install` to update the plist.
:::
...
```bash
bullwhip gateway stop && bullwhip gateway start
```
...
bullwhip pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.bullwhip/.env`:
...
Control how much detail the bot shows when using tools. In `~/.bullwhip/config.yaml`:
...
Customize how the bot communicates by editing `~/.bullwhip/SOUL.md`:

For a full guide, see [Use SOUL.md with Bull Whip](/docs/guides/use-soul-with-bullwhip).
...
```bash
# From the CLI
bullwhip cron list          # View all scheduled jobs
bullwhip cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u bullwhip-gateway` (Linux) or `~/.bullwhip/logs/gateway.log` (macOS) |
| Cron job output | `~/.bullwhip/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.bullwhip/cron/jobs.json` |
| Pairing data | `~/.bullwhip/pairing/` |
| Session history | `~/.bullwhip/sessions/` |
