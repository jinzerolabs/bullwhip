...
title: "Build a Bull Whip Plugin"
description: "Step-by-step guide to building a complete Bull Whip plugin with tools, hooks, data files, and skills"
---

# Build a Bull Whip Plugin

This guide walks through building a complete Bull Whip plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.
...
```bash
mkdir -p ~/.bullwhip/plugins/calculator
cd ~/.bullwhip/plugins/calculator
```
...
This tells Bull Whip: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
4. **Accept `**kwargs`:** Bull Whip may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `bullwhip my-plugin <subcommand>`)
- If this function crashes, the plugin is disabled but Bull Whip continues fine
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.bullwhip/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.bullwhip/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `bullwhip plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
This is the only hook whose return value matters. When a `pre_llm_call` callback returns a dict with a `"context"` key (or a plain string), Bull Whip injects that text into the **current turn's user message**. This is the mechanism for memory plugins, RAG integrations, guardrails, and any plugin that needs to provide the model with additional context.

#### Return format

```python
# Dict with context key
return {"context": "Recalled memories:\n- User prefers dark mode\n- Last project: bullwhip-agent"}
...
- **The system prompt is Bull Whip's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
Plugins can add their own `bullwhip <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for bullwhip my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: bullwhip my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for bullwhip my-plugin."""
...
After registration, users can run `bullwhip my-plugin status`, `bullwhip my-plugin config`, etc.
...
```toml
# pyproject.toml
[project.entry-points."bullwhip_agent.plugins"]
my-plugin = "my_plugin_package"
```

```bash
pip install bullwhip-plugin-calculator
# Plugin auto-discovered on next bullwhip startup
```
...
```python
# Wrong — will break if Bull Whip passes extra context
def handler(args):
    ...
