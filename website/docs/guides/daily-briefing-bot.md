...
2. **Bull Whip spins up** a fresh agent session with your prompt
...
- **Bull Whip Agent installed** — see the [Installation guide](/docs/getting-started/installation)
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.bullwhip/cron/output/` and you can read them anytime.
:::
...
Bull Whip will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by Bull Whip Agent
```
...
Just tell Bull Whip what you want:
...
Bull Whip will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell Bull Whip to delegate each topic to a sub-agent:
...
Bull Whip will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
