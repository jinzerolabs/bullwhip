# Contributing to Bull Whip Agent

Thank you for contributing to Bull Whip Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — Windows, macOS, different Linux distros, different terminal emulators. We want Bull Whip to work everywhere.
...
Bundled skills (in `skills/`) ship with every Bull Whip install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `bullwhip skills browse` (labeled "official") and install it with `bullwhip skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [ZeroLabs Korea Discord](https://discord.gg/ZeroLabsKorea). Users can install it with `bullwhip skills install`.
...
```bash
git clone --recurse-submodules https://github.com/ZeroLabsKorea/bullwhip-agent.git
cd bullwhip-agent
...
```bash
mkdir -p ~/.bullwhip/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.bullwhip/config.yaml
touch ~/.bullwhip/.env

# Add at minimum an LLM provider key:
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.bullwhip/.env
```
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/bullwhip" ~/.local/bin/bullwhip

# Verify
bullwhip doctor
bullwhip chat -q "Hello"
```
...
├── cli.py                    # BullWhipCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (bullwhip-cli, bullwhip-telegram, etc.)
├── bullwhip_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── bullwhip_cli/               # CLI command implementations
...
│
├── skills/                   # Bundled skills (copied to ~/.bullwhip/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── environments/             # RL training environments (Atropos integration)
├── tests/                    # Test suite
├── website/                  # Documentation site (bullwhip-agent.zerolabskorea.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.bullwhip/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.bullwhip/`)

| Path | Purpose |
|------|---------|
| `~/.bullwhip/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.bullwhip/.env` | API keys and secrets |
| `~/.bullwhip/auth.json` | OAuth credentials (Nous Portal) |
| `~/.bullwhip/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.bullwhip/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.bullwhip/state.db` | SQLite session database |
| `~/.bullwhip/sessions/` | JSON session logs |
| `~/.bullwhip/cron/` | Scheduled job data |
| `~/.bullwhip/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`bullwhip_state.py`) with full-text search and unique session titles. JSON logs go to `~/.bullwhip/sessions/`.
...
Four fields are supported under `metadata.bullwhip`:
...
The user may skip setup and keep loading the skill. Bull Whip only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `bullwhip setup` or update `~/.bullwhip/.env` locally.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Bull Whip tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `bullwhip --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Bull Whip uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.bullwhip/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `bullwhip_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `bullwhip_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Bull Whip runs on Linux, macOS, and Windows. When writing code that touches the OS:
...
Bull Whip has terminal access. Security matters.
...
2. **Test manually**: Run `bullwhip` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/ZeroLabsKorea/bullwhip-agent/issues)
- Include: OS, Python version, Bull Whip version (`bullwhip version`), full error traceback
...
- **Discord**: [discord.gg/ZeroLabsKorea](https://discord.gg/ZeroLabsKorea) — for questions, showcasing projects, and sharing skills
