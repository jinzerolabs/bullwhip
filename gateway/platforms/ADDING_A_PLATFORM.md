...
Checklist for integrating a new messaging platform into the Bull Whip gateway.
...
And add it to the `bullwhip-gateway` composite:

```python
"bullwhip-gateway": {
    "includes": [..., "bullwhip-your-platform"]
}
```
...
## 12. Status Display (`bullwhip_cli/status.py`)
...
## 13. Gateway Setup Wizard (`bullwhip_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ bullwhip_cli/ toolsets.py \
  --include="*.py" -l | sort -u
