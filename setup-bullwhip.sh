...
# 5. Symlinks the 'bullwhip' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}🐂 Bull Whip Agent Setup${NC}"
echo ""
...
# ============================================================================
# PATH setup — symlink bullwhip into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up bullwhip command..."

BULLWHIP_BIN="$SCRIPT_DIR/venv/bin/bullwhip"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$BULLWHIP_BIN" "$COMMAND_LINK_DIR/bullwhip"
echo -e "${GREEN}✓${NC} Symlinked bullwhip → $COMMAND_LINK_DISPLAY_DIR/bullwhip"
...
# ============================================================================
# Seed bundled skills into ~/.bullwhip/skills/
# ============================================================================

BULLWHIP_SKILLS_DIR="${BULLWHIP_HOME:-$HOME/.bullwhip}/skills"
mkdir -p "$BULLWHIP_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.bullwhip/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$BULLWHIP_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  bullwhip status        # Check configuration"
if is_termux; then
    echo "  bullwhip gateway       # Run gateway in foreground"
else
    echo "  bullwhip gateway install # Install gateway service (messaging + cron)"
fi
echo "  bullwhip cron list     # View scheduled jobs"
echo "  bullwhip doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m bullwhip_cli.main setup
fi
